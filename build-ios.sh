#!/bin/bash
# Build Maude for iOS Simulator (same as CI / command-line check).
set -euo pipefail
cd "$(dirname "$0")"
...
# closures may ship in Maude/ views. Disabled honest stubs (SOON chip +
# .disabled(true), JournalView "Scan the label" pattern) are exempt via an
# explicit `// HONEST-STUB` marker on the Button line.
if grep -rn -E 'Button *\{ *\}' Maude --include='*.swift' | grep -v 'HONEST-STUB'; then
...
echo "Building Maude → $DEST"
xcodebuild -scheme Maude -destination "$DEST" clean build | xcbeautify 2>/dev/null || xcodebuild -scheme Maude -destination "$DEST" clean build
