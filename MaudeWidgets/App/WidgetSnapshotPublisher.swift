...
//
// TARGET MEMBERSHIP: the **Maude iOS app target ONLY**. It references
// `AppState`, `TodaySignals` and `NudgeGuard`, none of which exist in an
// extension. (`MaudeWidgets/Shared/` is what the extensions get.)
//
...
nonisolated enum WidgetSnapshotComposer {
    ...
    /// `MaudeEditionWidget.kind` in the widget extension target.
    static let editionWidgetKind = "MaudeEditionWidget"
    ...
    static func compose(signals: TodaySignals?,
                        ...
                        now: Date = Date()) -> MaudeWidgetSnapshot? {
        guard let s = signals else { return nil }
        return MaudeWidgetSnapshot(
            derivedAt: now,
...
extension AppState {
    ...
    func publishWidgetSnapshot(now: Date = Date()) {
        ...
        if let snapshot {
            MaudeWidgetSnapshotStore.save(snapshot)
        } else {
            MaudeWidgetSnapshotStore.clear()
        }
        #if canImport(WidgetKit)
