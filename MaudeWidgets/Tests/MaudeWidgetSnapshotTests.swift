// MaudeWidgetSnapshotTests.swift — FR-WID-01 rail tests.
//
// TARGET MEMBERSHIP: **MaudeTests**. Add together with the two
// `MaudeWidgets/Shared/` files and `MaudeWidgets/App/WidgetSnapshotPublisher.swift`
...
final class MaudeWidgetSnapshotTests: XCTestCase {
    ...
    private func snapshot(_ s: TodaySignals? = nil) -> MaudeWidgetSnapshot? {
    ...
    func testT_WID_01_roundTripIsLossless() throws {
        ...
        let back = try decoder.decode(MaudeWidgetSnapshot.self,
    ...
    func testT_WID_01_unsupportedSchemaIsNotRendered() throws {
        let snap = try XCTUnwrap(snapshot())
        let future = MaudeWidgetSnapshot(schema: MaudeWidgetSnapshot.currentSchema + 1,
    ...
    func testT_WID_06_storeIsInertWithoutAnAppGroup() throws {
        // A bundle with no `MaudeAppGroup` key stands in for a target that was
        // not set up: reads are nil, writes report failure. Never a guessed group.
        XCTAssertNil(MaudeWidgetSnapshotStore.appGroupIdentifier(bundle: Bundle(for: type(of: self))))
        XCTAssertNil(MaudeWidgetSnapshotStore.load(from: nil))
        let snap = try XCTUnwrap(snapshot())
        XCTAssertFalse(MaudeWidgetSnapshotStore.save(snap, to: nil))
    }

    func testT_WID_06_saveLoadClearRoundTripInAnIsolatedSuite() throws {
        let suiteName = "maude.widget.tests.\(UUID().uuidString)"
        let store = try XCTUnwrap(UserDefaults(suiteName: suiteName))
        defer { UserDefaults().removePersistentDomain(forName: suiteName) }

        let snap = try XCTUnwrap(snapshot())
        XCTAssertTrue(MaudeWidgetSnapshotStore.save(snap, to: store))
        XCTAssertEqual(MaudeWidgetSnapshotStore.load(from: store), snap)
        MaudeWidgetSnapshotStore.clear(in: store)
        XCTAssertNil(MaudeWidgetSnapshotStore.load(from: store))
    }
}
