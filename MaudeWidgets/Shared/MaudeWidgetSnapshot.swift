// MaudeWidgetSnapshot.swift — the ONLY thing that crosses from the app into a
...
/// no Maude surface currently states. So this type deliberately does NOT claim
...
public nonisolated struct MaudeWidgetSnapshot: Codable, Equatable, Sendable {
    ...
    public init(schema: Int = MaudeWidgetSnapshot.currentSchema,
                derivedAt: Date,
...
/// Info.plist key `MaudeAppGroup`, which is set to `$(APP_GROUP)` — the single
...
public nonisolated enum MaudeWidgetSnapshotStore {

    /// Info.plist key carrying `$(APP_GROUP)`. See MaudeWidgets/SETUP.md §5.
    public static let appGroupInfoKey = "MaudeAppGroup"
    /// UserDefaults key inside the shared suite. Versioned with the schema.
    public static let defaultsKey = "maude.widget.snapshot.v1"
    ...
    public static func defaults(bundle: Bundle = .main) -> UserDefaults? {
        guard let id = appGroupIdentifier(bundle: bundle) else {
            ...
            print("[MaudeWidgets] Info.plist key '\(appGroupInfoKey)' missing or "
                  + "unexpanded — snapshot sharing is OFF. See MaudeWidgets/SETUP.md §5.")
            #endif
            return nil
        }
    ...
    public static func load(from store: UserDefaults? = defaults()) -> MaudeWidgetSnapshot? {
        guard let store, let data = store.data(forKey: defaultsKey) else { return nil }
        guard let snap = try? decoder.decode(MaudeWidgetSnapshot.self, from: data) else { return nil }
    ...
    @discardableResult
    public static func save(_ snapshot: MaudeWidgetSnapshot,
