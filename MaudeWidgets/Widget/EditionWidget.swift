...
// score opacity — a single number with hidden arithmetic. Maude's structural
...
nonisolated struct EditionProvider: TimelineProvider {
    ...
    func getTimeline(in context: Context, completion: @escaping (Timeline<EditionEntry>) -> Void) {
        let entry = EditionEntry(date: Date(), snapshot: MaudeWidgetSnapshotStore.load())
    ...
    static let galleryPreview = MaudeWidgetSnapshot(
        derivedAt: Date(),
        edition: .morning,
...
struct MaudeEditionWidget: Widget {
    var body: some WidgetConfiguration {
        StaticConfiguration(kind: MaudeEditionWidget.kind, provider: EditionProvider()) { entry in
            EditionWidgetView(entry: entry)
                .containerBackground(MaudeTheme.paper2, for: .widget)
        }
    ...
    static let kind = "MaudeEditionWidget"
}
