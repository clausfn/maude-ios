# Maude Design System — handoff bundle (A6 “Daylight”)
...
- `_ds_bundle.js` — compiled React components (Mark, Button, Card, ConsentChip, KpiTile, Insight, MetricRing, NudgeCard, …). Load after React: `window.MaudeDesignSystem_af5aa6.<Component>`.
- `assets/brand/` — iris logo (mark / mono / reversed / app icon), DfG logo, and the flat favicon set (`maude_favicon.svg` + `favicon-*.png`).
...
## For designers
Work inside the **Maude Design System** project and pick from the **Templates** list. To add a new canonical template, create `templates/<slug>/<Slug>.dc.html` with `<!-- @template name="…" description="…" -->` on line 1 and `<helmet><script src="./ds-base.js"></script></helmet>`.
