...
struct AppLockTests {
    ...
    private static let appFile   = "Maude/MaudeApp.swift"
    private static let lockFile  = "Maude/Views/Onboarding/AppLockSetupView.swift"
    /// Every surface that can turn the lock on.
    private static let toggleFiles = [
        "Maude/Views/Onboarding/AppLockSetupView.swift",
        "Maude/Views/SettingsView.swift",
        "Maude/Views/AccountSecurityView.swift",
    ]
    ...
    @Test func coldLaunchStartsLockedWhenEnabled() throws {
        ...
        #expect(src.range(of: pattern, options: .regularExpression) != nil,
                "MaudeApp must initialise isLocked from the persisted pref, or a cold launch flashes content")
    }
    ...
    @Test func backgroundingReArmsTheLock() throws {
        ...
        let onChange = try #require(
            ...
            "MaudeApp must observe scenePhase (lint is stale)")
    ...
    @Test func inactiveCoversTheAppSwitcherSnapshotWithoutPrompting() throws {
        ...
        let overlay = try #require(SourceLint.body(ofDeclarationContaining: ".overlay {", in: src),
                                   "MaudeApp must present the cover as an overlay (lint is stale)")
        ...
        let coverSrc = try source("Maude/Views/Onboarding/AppLockSetupView.swift")
    ...
    @Test func theLockOverlaysEverything() throws {
        let src = try source(Self.appFile)
        let overlay = try #require(SourceLint.body(ofDeclarationContaining: ".overlay {", in: src),
                                   "MaudeApp must present the lock as an overlay (lint is stale)")
    ...
    @Test func onlyAnAuthenticatedUnlockClearsTheLockInRelease() throws {
        ...
        #expect(!clears.isEmpty, "MaudeApp must have an unlock path (lint is stale)")
        for clear in clears {
    ...
    @Test func disablingTheLockIsNeverGated() throws {
        for file in ["Maude/Views/SettingsView.swift", "Maude/Views/AccountSecurityView.swift"] {
            let src = try source(file)
    ...
    @Test func faceIDUsageDescriptionIsDeclared() throws {
        let plist = try source("Maude/Info.plist")
