...
struct ReleasePostureTests {
    ...
    @Test func simulatedLoginsAreDebugGated() throws {
        try expectDebugGated(file: "Maude/Config.swift", marker: "dfgWalletLoginEnabled = true")
    ...
    @Test func sandboxWalletRailDetourIsRetired() throws {
        ...
        try expectDebugGated(file: "Maude/Config.swift",
                             marker: #"environment["MAUDE_WALLET_RAIL_URL"]"#)
    }

    @Test func walletRailRidesMainBackendByDefault() {
        // Runtime check (valid in any configuration when the env override is unset).
        guard ProcessInfo.processInfo.environment["MAUDE_WALLET_RAIL_URL"] == nil else { return }
    ...
    @Test func lv001InjectionIsDebugGated() throws {
        try expectDebugGated(file: "Maude/AppState.swift", marker: "LV001Dataset.passportStats")
    }

    @Test func mockWalletSeedingIsDebugGated() throws {
        // signInDemo seeds + loadWallet fallback — every reference gated.
        try expectDebugGated(file: "Maude/AppState.swift", marker: "MockData.walletGrants")
        try expectDebugGated(file: "Maude/AppState.swift", marker: "MockData.walletEvents")
        try expectDebugGated(file: "Maude/AppState.swift", marker: "MockData.demoStudy")
    }

    @Test func coldStartSeedsAreEmptyInRelease() throws {
        // Every MockData reference in the seed table must be DEBUG-only.
        try expectDebugGated(file: "Maude/ColdStartSeeds.swift", marker: "MockData.")
    }

    @Test func demoLoginButtonIsDebugGated() throws {
        try expectDebugGated(file: "Maude/Views/AuthView.swift", marker: "appState.signInDemo()")
    }
    ...
    @Test func journalAndVaultDemoSeedsAreDebugGated() throws {
        ...
        try expectDebugGated(file: "Maude/Views/JournalView.swift", marker: "demoSeed")
        try expectDebugGated(file: "Maude/Views/JournalView.swift", marker: "VaultDocument.demo")
    }
    ...
    @Test func privacyProofSheetHasNoDemoGrants() throws {
        ...
        let src = try source("Maude/Views/InAppPrivacyView.swift")
