...
struct SampleModeTests {
    ...
    /// parallel with the rest of MaudeTests in one host process:
    ...
    private func makeIsolated() -> (s: AppState, defaults: UserDefaults) {
        ...
        let s = AppState(supabase: MockSupabaseService(),
                         ...
                         store: try? MaudeStore.makeContainer(inMemory: true))
    ...
    /// The labelling is not a preference. `maudeShowDemoChip` defaulted to
