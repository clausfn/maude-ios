...
struct ChatGuardTests {
    ...
    func redTeamReturnsSafetyLine(prompt: String) {
        #expect(engine().respond(to: prompt) == MaudeChatCopy.safetyLine)
    }
    ...
    @Test func emptyPromptIsSafe() {
        #expect(engine().respond(to: "   ") == MaudeChatCopy.safetyLine)
    }
    ...
    @Test func driftingDiagnosisResponseIsBlocked() {
        ...
        #expect(ChatGuard.sanitizeOutput(drift) == MaudeChatCopy.safetyLine)
    }
    ...
    @Test func intendedPurposeHasNoDiseaseManagementClaim() {
        ...
        let blob = (MaudeChatCopy.intendedPurpose + " " + MaudeChatCopy.aiLabel
                    + " " + MaudeChatCopy.systemPrompt.replacingOccurrences(of: "MUST NOT", with: ""))
            .lowercased()
        ...
        for term in banned {
            #expect(!MaudeChatCopy.intendedPurpose.lowercased().contains(term),
                    "intended purpose contains banned term: \(term)")
        }
        #expect(MaudeChatCopy.intendedPurpose.lowercased().contains("not a medical device"))
        _ = blob
    }
    ...
    @Test func safetyLineIsStaticAndNonDynamic() {
        ...
        #expect(a == b)
        #expect(a == MaudeChatCopy.safetyLine)
    }
}
