...
struct ClinicalTIRZoneTests {
    ...
    @Test func theRampUsesTheFrozenPalette() {
        #expect(ramp[0].color == MaudeTheme.tirVeryLow)
        #expect(ramp[1].color == MaudeTheme.tirLow)
        #expect(ramp[2].color == MaudeTheme.tirTarget)
        #expect(ramp[3].color == MaudeTheme.tirHigh)
        #expect(ramp[4].color == MaudeTheme.tirVeryHigh)
    ...
    @Test func thereIsExactlyOneImplementationOfTheRamp() throws {
        ...
        let oura = try String(contentsOf: root.appendingPathComponent("Maude/Views/OuraComponents.swift"),
                              encoding: .utf8)
        ...
        let trends = try String(contentsOf: root.appendingPathComponent("Maude/Views/TrendsCharts.swift"),
                                encoding: .utf8)
        ...
        for token in ["tirVeryLow", "tirLow", "tirTarget", "tirHigh", "tirVeryHigh"] {
            #expect(!trends.contains("MaudeTheme.\(token)"),
