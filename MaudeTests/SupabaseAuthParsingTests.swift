...
struct SupabaseAuthParsingTests {
    ...
    @Test func buildsPasswordBody() throws {
        let data = SupabaseAuthClient.passwordBody(email: "claus@maude.app", password: "pw")
        let o = try #require(try JSONSerialization.jsonObject(with: data) as? [String: Any])
        #expect(o["email"] as? String == "claus@maude.app")
    ...
    @Test func parsesTokenResponse() {
        ...
        let json = Data("""
        {"access_token":"sb_at_123","token_type":"bearer","expires_in":3600,"refresh_token":"sb_rt_xyz","user":{"id":"u-9","email":"claus@maude.app"}}
        """.utf8)
        let r = SupabaseAuthClient.parseTokenResponse(json)
        #expect(r?.accessToken == "sb_at_123")
        #expect(r?.userID == "u-9")
        #expect(r?.email == "claus@maude.app")
    ...
    @Test func parsesUser() {
        let json = Data("""
        {"id":"u-9","aud":"authenticated","email":"claus@maude.app"}
        """.utf8)
