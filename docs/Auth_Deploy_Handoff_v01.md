# Auth Deploy Handoff — stand up `auth.maude.app` (GoTrue)
...
**Status flipped: `auth.maude.app` is UP.** Verified:

| Check | Result |
|---|---|
| `auth.maude.app` DNS (public) | ✅ resolves → `163.172.143.5` (sslip.io) |
| `auth.maude.app/health` + `/auth/v1/health` | ✅ 200, valid TLS, **GoTrue v2.189.0** |
| `/settings` | ✅ `email: true`, `disable_signup: false`, `mailer_autoconfirm: true` |
| `/.well-known/jwks.json` | `{"keys":[]}` → **HS256 (symmetric secret)** mode |
| `api.maude.app/health` / `/me` (no token) | ✅ 200 / ✅ 401 (guard live) |
...
1. **Confirm the backend ↔ GoTrue secret match.** Empty JWKS ⇒ HS256, so
   `api.maude.app` must have **`SUPABASE_JWT_SECRET` == GoTrue's `GOTRUE_JWT_SECRET`**
   ...
   GoTrue password-grant → `access_token` → `api.maude.app/me` returning **200**
   confirms it.
...
2. **Enable the Apple provider.** `/settings` shows `apple: false`, so
   ...
   `xyz.ppcn.maude`, or defer Apple (testers use email + demo).
...
TestFlight sign-in is blocked because `auth.maude.app` does not exist yet. Deploy
the self-hosted **GoTrue** (Supabase Auth) container in the EU and point
`auth.maude.app` at it, with a signing key that matches the backend. Until then,
...
| Host | Purpose | Result |
|---|---|---|
| `api.maude.app` | sovereign backend | ✅ Up — `GET /health` = 200 |
| `auth.maude.app` | self-hosted GoTrue (sign-in) | ❌ Down — no DNS record, doesn't resolve (confirmed on public `8.8.8.8`) |
...
`authURL = https://auth.maude.app`. With no endpoint there:

- ❌ Email/password and "Continue with Apple" fail (no token-exchange endpoint).
- ❌ Live **Care messaging** against `api.maude.app` won't work — it needs a bearer
...
2. **DNS / Edge route** for `auth.maude.app` → that container, with a managed TLS cert.
3. **Match the JWT signing config** so the backend accepts GoTrue's tokens (below).
   Enable the **Apple** provider (bundle `xyz.ppcn.maude`) and **email/password**.

## Backend JWT config it has to match

From `maude-backend/src/common/auth/auth.guard.ts`. The guard verifies
`Authorization: Bearer <jwt>` with **either** of these — set one in the
`api.maude.app` container env, matching GoTrue's signing key:

```
SUPABASE_JWT_SECRET=""     # HS256 symmetric secret — must equal GoTrue's GOTRUE_JWT_SECRET
SUPABASE_JWKS_URL=""       # asymmetric alt, e.g. https://auth.maude.app/auth/v1/.well-known/jwks.json
...
- `curl https://auth.maude.app/health` → 200, valid cert.
- A test email signup/login returns an access token, and that token is accepted by
  `api.maude.app` (a `/me`-type call returns 200, not 401).
...
curl -sS -o /dev/null -w "health: %{http_code}\n" https://auth.maude.app/health
curl -sS -o /dev/null -w "health(v1): %{http_code}\n" https://auth.maude.app/auth/v1/health
...
curl -sS -X POST "https://auth.maude.app/auth/v1/token?grant_type=password" \
...
curl -sS -o /dev/null -w "api /me: %{http_code}\n" \
  -H "Authorization: Bearer $TOKEN" https://api.maude.app/me

# (baseline, already green)
curl -sS -o /dev/null -w "api health: %{http_code}\n" https://api.maude.app/health
```
