# Liviqa — Ways of Working (workstreams, repos, environments, releases)
...
|---|---|---|---|
| Frontend / UX iteration | `clausfn/liviqa-ios` | iOS chat | tweak screen → TestFlight → feedback. **Backend-independent** (demo data). |
| Backend productionization | `clausfn/liviqa-backend` (+ `liviqa-b2b-console`) | B2B/console chat | auth, data, consent, deploy. |

Keep them separate. The contract between them is `Liviqa_iOS_Backend_Contract_v01`
...
In `liviqa-ios/Liviqa/Config.swift`:

- `authEnabled` — **false** now. Auth screen shows only "Enter Liviqa" (demo). Flip
...
- Backend selection: **Debug → `.mock`**, **Release/TestFlight → `sovereignProd`**,
  override via `LIVIQA_BACKEND` env (`sovereignLocal|sovereignStaging|sovereignProd|mock`).
...
|---|---|---|---|
| local | `localhost:3001`, `DEV_AUTH=true`, seed tokens | none | dev (`LIVIQA_BACKEND=sovereignLocal`) |
| **staging** | `sandbox.liviqa.app` | shared GoTrue | QA before promotion (`LIVIQA_BACKEND=sovereignStaging`) |
| **prod** | `api.liviqa.app` + `auth.liviqa.app`, `DEV_AUTH=false` | GoTrue (HS256) | TestFlight/App Store (`sovereignProd`, the Release default) |

Backend devs iterate on **staging** without risking **prod**. (As of 2026-06-04
`sandbox.liviqa.app` resolves but returns 403 — staging backend still needs a
healthy container; tracked with the backend chat.)

## Branch model

- **liviqa-ios:** `main` = last build blessed for TestFlight; `develop` = active UI
...
- **liviqa-backend:** `main` = **prod; the only branch you deploy from.** Feature
...
3. iOS verifies password-grant → `api.liviqa.app/me` = 200, flips `authEnabled=true`,
...
- **GitHub `clausfn/*` is the source of truth.** One canonical local clone per repo;
  iOS = `~/Developer/DataForGood/liviqa-ios`. Remove duplicate local copies (backend/
