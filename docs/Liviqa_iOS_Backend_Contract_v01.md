# Liviqa iOS ↔ Sovereign Backend — Integration Contract (v01)

_Version: 2026-06-03 · Owner: Data for Good. The single contract both `liviqa-ios` and `liviqa-backend` build to, so the citizen app (Claus on TestFlight) and the B2B console form one real loop. Reconciles what the iOS app does **today** with the Mode-A sharing model (`Liviqa_Consented_Sharing_and_Recipient_View_Model_v01`) and the backend spec (`Liviqa_Backend_Spec_v01`)._
...
The sovereign backend now exposes the surface `SupabaseServiceProtocol` needs, so `LiviqaBackendService` maps 1:1:
