...
1. **Authenticate with Supabase** (the `MaudeBackendService` / `SupabaseServiceProtocol`): point the Supabase client at the self-hosted URL (e.g. `https://auth.maude.app`), not `*.supabase.co`. Email/password (and Apple) sign-in via Supabase.
2. **Call the Maude API with the Supabase access token** as a bearer:
...
## Sandbox vs prod
- **Sandbox** (`sandbox.maude.app`, synthetic/Claus data): backend runs with `DEV_AUTH=true` and accepts seeded dev bearer tokens — you can develop against it without Supabase wired. The console uses a dev role-picker.
