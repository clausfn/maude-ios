...
struct ProviderResolutionTests {
    ...
    @Test func envForcesMock() {
        let kind = AppState.resolveProviderKind(arguments: [], env: ["LIVIQA_DATA": "mock"], realAvailable: true)
        #expect(kind == .mock)
    }
    ...
    @Test func envForcesHealthKitEvenWhenUnavailableFlag() {
        ...
        let kind = AppState.resolveProviderKind(arguments: [], env: ["LIVIQA_DATA": "healthKit"], realAvailable: false)
        #expect(kind == .healthKit)
    }
