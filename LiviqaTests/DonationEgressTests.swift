...
// `T-RSCH-05`, over the whole `Liviqa/Donation` directory plus the donor screen.
//
...
struct DonationEgressTests {
    ...
    private static let donationDirectory = "Liviqa/Donation"
    private static let donorScreen = "Liviqa/Views/DonorExportView.swift"
    ...
    @Test func theOnlyWriteIsTheSealedFile() throws {
        let src = try SourceLint.text("Liviqa/Donation/DonationExport.swift")
    ...
    @Test func donorBuildFlagIsCompileTimeGated() throws {
        let src = try SourceLint.text("Liviqa/Donation/DonationProgramme.swift")
        #expect(src.contains("#if LIVIQA_DONOR"), "the donor gate must be a compile-time condition")
        ...
        #expect(DonationProgramme.isDonorBuild == false,
                "a build without -D LIVIQA_DONOR must have no donor flow")
    }
    ...
    @Test func noProgrammeKeyMaterialIsCommitted() throws {
        ...
        let src = try SourceLint.text("Liviqa/Donation/DonationProgramme.swift")
        let env = SourceLint.matches("LIVIQA_DONATION_PUBKEY", in: src)
