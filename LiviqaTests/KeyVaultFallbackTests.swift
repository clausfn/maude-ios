...
struct KeyVaultFallbackTests {
    ...
    @Test func unusableKeyMaterialIsNeverSilentlyReplaced() throws {
        ...
        if ((try? store.read(account: "liviqa.device-key.v1")) ?? nil) != nil {
            try store.write(Data(repeating: 0x5A, count: 40), account: "liviqa.device-key.v1")
            ...
            #expect(((try? store.read(account: "liviqa.wrapped-dek.v1")) ?? nil) != nil)
    ...
    @Test func wrappedKeyWithoutItsDeviceKeyIsRefused() throws {
        ...
        let store = DeviceKeyFileStore(service: "test." + service, directory: dir)
        guard ((try? store.read(account: "liviqa.wrapped-dek.v1")) ?? nil) != nil else { return }
        try store.delete(account: "liviqa.device-key.v1")
    ...
    @Test func deviceFileStoreRoundTripsAndHidesItsAccountNames() throws {
        let dir = tempDir()
        let store = DeviceKeyFileStore(service: "dev.liviqa.app.keyvault", directory: dir)
        #expect(try store.read(account: "liviqa.device-key.v1") == nil)

        let material = Data((0 ..< 32).map { UInt8($0) })
        try store.write(material, account: "liviqa.device-key.v1")
        #expect(try store.read(account: "liviqa.device-key.v1") == material)

        let files = try FileManager.default
            .contentsOfDirectory(atPath: dir.appendingPathComponent("liviqa-keystore").path)
        #expect(files.count == 1)
        let name = try #require(files.first)
        #expect(!name.contains("device-key"))
        #expect(!name.contains("liviqa.device"))
        #expect(name.hasSuffix(".key"))

        try store.delete(account: "liviqa.device-key.v1")
        #expect(try store.read(account: "liviqa.device-key.v1") == nil)
        try store.delete(account: "liviqa.device-key.v1")   // deleting nothing is not an error
    }
