...
struct SampleModePostureTests {
    ...
    /// `liviqaShowDemoChip` defaulted to FALSE, so a person could switch the
    ...
    @Test func theDemoChipPreferenceNoLongerExists() {
        ...
        for file in SourceLint.swiftFiles(in: "Liviqa") {
            if !SourceLint.matches("liviqaShowDemoChip", in: file.source).isEmpty {
    ...
    @Test func theLabelIsNotGatedByAnyPreference() throws {
        let src = try source("Liviqa/Views/SampleModeBanner.swift")
    ...
    @Test func isDemoDataNoLongerMeansNoRealData() throws {
        let src = try source("Liviqa/AppState.swift")
    ...
    @Test func thereAreExactlyTwoDoorsIntoSampleMode() {
        ...
        #expect(Set(callers) == ["Liviqa/SampleMode.swift",                     // the definition
                                 "Liviqa/Views/Onboarding/OnboardingFlowView.swift",
    ...
    @Test func bothDoorsDeclareThemselves() throws {
        let onboarding = try source("Liviqa/Views/Onboarding/OnboardingFlowView.swift")
        let settings = try source("Liviqa/Views/SettingsView.swift")
    ...
    @Test func theSampleDatasetHasNoWayToPersistAnything() throws {
        let src = try source("Liviqa/Ingestion/SampleDataset.swift")
    ...
    @Test func theSampleModeStateMachineTouchesNoStore() throws {
        let src = try source("Liviqa/SampleMode.swift")
    ...
    @Test func homeNoLongerCarriesHardcodedStandInFigures() throws {
        let src = try source("Liviqa/Views/TodayView.swift")
