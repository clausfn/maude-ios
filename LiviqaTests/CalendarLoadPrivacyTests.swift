...
//
// The promise on the iOS permission prompt is "Liviqa reads how full your days
...
struct CalendarLoadPrivacyTests {
    ...
    private static let ingestorPath = "Liviqa/Ingestion/CalendarLoadIngestor.swift"
    ...
    @Test func nothingIsEverLoggedFromTheCalendarPath() throws {
        ...
        for path in [Self.ingestorPath,
                     "Liviqa/Context/CalendarLoad.swift",
                     "Liviqa/Context/CalendarLoadStore.swift",
                     "Liviqa/Context/CalendarLoadCopy.swift"] {
    ...
    @Test func onlyTheIngestorReadsCalendarEvents() {
        let files = SourceLint.swiftFiles(in: "Liviqa")
    ...
    @Test func eventKitIsImportedByTheReadSeamAndTheConsultWriterOnly() {
        // PlanConsultView WRITES one event the citizen asked Liviqa to create;
        // it never reads. Any third importer is a new, unreviewed calendar path.
        let importers = SourceLint.swiftFiles(in: "Liviqa")
            .filter { !SourceLint.matches(#"^\s*import EventKit"#, in: $0.source).isEmpty }
            .map(\.path)
            .sorted()
        #expect(importers == [Self.ingestorPath, "Liviqa/Views/PlanConsultView.swift"],
    ...
    @Test func theIntervalThatLeavesEventKitCarriesNoWords() throws {
        ...
        let src = try SourceLint.text("Liviqa/Context/CalendarLoad.swift")
    ...
    @Test func theUsageStringMakesThePromiseTheCodeKeeps() throws {
        let plistText = try SourceLint.text("Liviqa/Info.plist")
