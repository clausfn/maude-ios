...
// `LiviqaSundhedIngestClient.ingestSundhed`) deliberately stayed in the files
...
struct SundhedSinkTests {
    ...
    private static let capturePaths: [(file: String, sink: String)] = [
        ("Liviqa/Sundhed/SundhedImport.swift",     "private func save(_ s: SundhedDerivedSummary)"),
        ("Liviqa/Sundhed/SundhedWebSession.swift", "private func ingest(_ harvest: SundhedWebHarvest)"),
    ]
    ...
    @Test func noSundhedFileCallsTheIngestClient() {
        for file in SourceLint.swiftFiles(in: "Liviqa/Sundhed") {
    ...
    @Test func theIngestClientHasExactlyOneCallSiteInTheApp() {
        ...
        #expect(sites.first?.hasPrefix("Liviqa/AppState.swift:") == true,
    ...
    @Test func liveExtractionPathHasNoIngestEndpointInCode() throws {
        let src = try source("Liviqa/Sundhed/SundhedWebSession.swift")
    ...
    @Test func canonicalRecordStoreIsRegisteredOnDeviceWithoutCloudSync() throws {
        let src = try source("Liviqa/Models/Domain/Persistence.swift")
