...
//
// Store discipline mirrors LiviqaStore (Persistence.swift): SwiftData, on-device
...
// MARK: - Store (parallel container, same discipline as LiviqaStore)

/// The universal layer's own on-device SwiftData store. A SEPARATE container by
/// design this wave: `Persistence.swift` (LiviqaStore) is owned by concurrent
/// workflows, and separation is also the isolation guarantee — nothing that
/// queries LiviqaStore can ever fetch a universal row by accident.
enum UniversalHealthStore {
    ...
    /// Same construction rules as `LiviqaStore.makeContainer`: on-device only,
...
public nonisolated enum UniversalBrowse {
    ...
    /// it — reading back what was written, never an assessment by Liviqa.
