// CalendarLoadCopy.swift — every sentence Liviqa says about calendar load.
//
...
public nonisolated enum CalendarLoadCopy {
    ...
    /// The neutral sentence every fallback lands on. Says only that Liviqa has
    /// nothing to say — never a number, never a claim.
    public static var neutralFallback: String {
        String(localized: "Liviqa has nothing to say about this day's calendar.")
    }
    ...
    public static func stateLine(_ state: CalendarRowState) -> String {
        ...
        switch state {
        case .notConnected:
            text = String(localized: "Your calendar isn't connected, so this row is empty. Liviqa would read only how full your days are — never what is in them.")
        case .noEntriesAtAll:
        ...
        case .calibrating(let days):
            ...
            text = String(localized: "Liviqa has \(d) of your calendar so far. It needs a few days before it can tell a full day from your usual one.")
        case .ready:
    ...
    ///   explicit statement that Liviqa is not calling it a cause.
    public static func dayReadout(day: String,
                                  ...
                                  direction: Direction?,
                                  notable: Bool) -> String {
        let fallback = guarded(String(localized: "Liviqa has no calendar reading for \(day)."),
    ...
    /// The exact list of things Liviqa never reads. Every item here is a field
    ...
    public static var promise: String {
        String(localized: "Liviqa reads how full your days are, never what is in them.")
    }
}
