...
//
// The citizen picks a Sundhed.dk PDF (or text export), Liviqa parses it ON-DEVICE
...
// and mirrors the LiviqaBackendService networking style (Bearer from the Keychain
// SessionTokenStore, friendly error mapping). See INTEGRATION.md for the preferred
// production wiring (route through LiviqaBackendService to reuse token refresh).
import SwiftUI
#if os(iOS)
import UIKit
...
/// POST /ingest/sundhed route parity is verified on api.liviqa.app. This path is
...
/// in shipped builds until deliberately flipped. Local QA: `LIVIQA_SUNDHED_IMPORT=1`.
...
public enum SundhedIngestError: Error, LocalizedError {
    ...
    public var errorDescription: String? {
        switch self {
        case .notAvailable: return String(localized: "Importing Sundhed.dk data needs the live Liviqa backend.")
...
/// way LiviqaBackendService does (Keychain access token, falling back to the local
/// dev seed token), and maps transport errors to the same friendly copy.
public final class LiviqaSundhedIngestClient: SundhedIngesting, @unchecked Sendable {
    ...
    public init(session: URLSession = LiviqaSundhedIngestClient.defaultSession) {
    ...
    public func ingestSundhed(_ body: SundhedIngestBody) async throws {
        ...
        case 500...599:
            throw SundhedIngestError.server(String(localized: "Liviqa's servers are having trouble right now. Please try again in a few minutes."))
        default:
...
public struct SundhedImportView: View {
    ...
    private var ingestClient: SundhedIngesting {
        (appState.supabase as? SundhedIngesting) ?? LiviqaSundhedIngestClient()
    }
            ...
            VStack(alignment: .leading, spacing: 14) {
                ...
                HStack(spacing: 10) {
                    ...
                    Button {
                        ...
                        .foregroundStyle(.white)
                        .frame(maxWidth: .infinity).padding(.vertical, 12)
                        .background(s.isEmpty ? LiviqaTheme.ink4 : LiviqaTheme.moss)
                        .clipShape(RoundedRectangle(cornerRadius: 12))
                    }
                    .buttonStyle(.plain)
    ...
    private func medsSection(_ rows: [SundhedDerivedSummary.MedRow]) -> some View {
        VStack(alignment: .leading, spacing: 6) {
            Text("Medication (\(rows.count))").font(.lato(13, .bold)).foregroundStyle(LiviqaTheme.ink)
            ForEach(rows) { r in
                HStack {
                    Text(r.brand).font(.lato(13)).foregroundStyle(LiviqaTheme.ink2)
                    Spacer()
                    Text(r.atc).font(.lato(12, .bold)).foregroundStyle(LiviqaTheme.moss)
                }
            }
        }
    }
    ...
    private func conditionsSection(_ rows: [SundhedDerivedSummary.ConditionRow]) -> some View {
        VStack(alignment: .leading, spacing: 6) {
            Text("Diagnoses (\(rows.count))").font(.lato(13, .bold)).foregroundStyle(LiviqaTheme.ink)
            HStack {
                ForEach(rows) { r in
                    Text(r.code)
                        .font(.lato(12, .bold)).foregroundStyle(LiviqaTheme.ink)
