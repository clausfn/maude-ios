...
struct DonorExportView: View {
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Text("This is not part of using Liviqa, and nothing here happens on its own. The app sends nothing: it writes one encrypted file that only \(DonationProgramme.controller)'s two named custodians can open, and you hand that file over yourself.")
        ...
        VStack(alignment: .leading, spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    ForEach(DonationAssembler.Scope.allCases, id: \.rawValue) { scope in
                        Toggle(isOn: Binding(
                            ...
                            Text(scope.label).font(.lato(14.5, .semibold)).foregroundStyle(LiviqaTheme.ink)
                        }
            ...
            Button {
                ...
                Text("Record this agreement")
                    ...
                    .background(LiviqaTheme.invertBG.opacity(canRecord ? 1 : 0.45))
    ...
    private func activeGrantSection(_ grant: WalletGrant) -> some View {
        card {
            VStack(alignment: .leading, spacing: 6) {
                Text("Agreement \(DonationExport.reference(for: grant))")
                    .font(.liviqaMono(12)).foregroundStyle(LiviqaTheme.ink2)
            ...
            card {
                VStack(alignment: .leading, spacing: 8) {
                    Text("What this file would contain".uppercased())
                        .font(.liviqaKicker(10)).tracking(1.2).foregroundStyle(LiviqaTheme.ink3)
                    ForEach(countRows(payload), id: \.label) { row in
                        HStack {
                            Text(row.label).font(.lato(13)).foregroundStyle(LiviqaTheme.ink2)
                            Spacer()
                            Text(row.value).font(.liviqaMono(12)).foregroundStyle(LiviqaTheme.ink)
                        }
                    }
                    Divider().background(LiviqaTheme.line2).padding(.vertical, 2)
                    ForEach(DonationProgramme.includedItems, id: \.self) { item in
                        bullet(item, symbol: "checkmark", tint: LiviqaTheme.moss)
                    }
                }
            }
            ...
            card {
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    ForEach(DonationProgramme.excludedItems, id: \.self) { item in
                        bullet(item, symbol: "xmark", tint: LiviqaTheme.ink4)
                    }
                }
            }
        }
        ...
        Button {
            ...
            Text("Delete the file from this phone")
                .font(.lato(13.5, .semibold)).foregroundStyle(LiviqaTheme.ink2)
        ...
        Button { confirmingWithdraw = true } label: {
            Text("Withdraw from the programme")
                .font(.lato(13.5, .semibold)).foregroundStyle(LiviqaTheme.clayText)
                .frame(maxWidth: .infinity).padding(.vertical, 12)
        }
        .buttonStyle(.plain)
                ...
                ForEach(appState.donationExports) { record in
                    VStack(alignment: .leading, spacing: 3) {
                        ...
                        Text("\(record.rowCount) rows · \(record.byteCount / 1024) KB")
                            .font(.lato(12)).foregroundStyle(LiviqaTheme.ink2)
                        Text(record.sealedDigest.prefix(16) + "…")
                            .font(.liviqaMono(10)).foregroundStyle(LiviqaTheme.ink4)
                    }
    ...
    private var honestyCard: some View {
        card(background: LiviqaTheme.brass2, stroke: LiviqaTheme.brass.opacity(0.5)) {
            VStack(alignment: .leading, spacing: 8) {
                Text(DonationProgramme.pseudonymityNotice)
                    .font(.lato(12)).lineSpacing(2.5).foregroundStyle(LiviqaTheme.ink2)
                    .fixedSize(horizontal: false, vertical: true)
    ...
    private func refusalCard(_ text: String) -> some View {
        card(background: LiviqaTheme.paper2, stroke: LiviqaTheme.clayText.opacity(0.4)) {
    ...
    private func card<Content: View>(background: Color = LiviqaTheme.paper2,
                                     stroke: Color = LiviqaTheme.line,
