...
//
// Universality is the point. Liviqa already showed its work in three places (the
...
struct SeeWhyChip: View {
    ...
    var body: some View {
        Button(action: action) {
            ...
            .foregroundStyle(onTint ? Color.white : LiviqaTheme.moss)
            .padding(.horizontal, 11).padding(.vertical, 6)
            .background(Capsule().fill(onTint ? Color.white.opacity(0.18) : LiviqaTheme.moss2))
            .overlay(Capsule().stroke(onTint ? Color.white.opacity(0.30) : LiviqaTheme.moss3,
                                      lineWidth: 1))
        }
        .buttonStyle(.plain)
