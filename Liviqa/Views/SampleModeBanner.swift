...
// preference, `liviqaShowDemoChip`, which defaulted to FALSE. Synthetic values
...
struct SampleModeBanner: View {
    ...
    var body: some View {
        HStack(spacing: 10) {
            ...
            Button(action: onLeave) {
                Text("Leave")
                    ...
                    .padding(.vertical, 7)
                    .background(
                        Capsule().stroke(LiviqaTheme.clayText.opacity(0.55), lineWidth: 1.2))
            }
            .buttonStyle(.plain)
        ...
        .overlay(alignment: .top) { Rectangle().fill(LiviqaTheme.clay).frame(height: 2) }
...
extension View {
    ...
    func liviqaSampleModeBanner(_ appState: AppState,
