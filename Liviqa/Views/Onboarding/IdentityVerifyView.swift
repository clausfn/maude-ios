...
let ID_METHODS: [IDMethod] = [
    ...
    IDMethod(id: "none", tier: "floor",
             ...
             trust: "—",
             sub: String(localized: "You can still use Liviqa for yourself — only study/trial participation needs a uniqueness check."),
             icon: "xmark")
]
...
struct IdentityVerifyView: View {
                ...
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    HStack(alignment: .top, spacing: 12) {
                        OnbIconChip(systemName: "globe.europe.africa",
                                    color: LiviqaTheme.accentRecovery, side: 34, corner: 10)
                        VStack(alignment: .leading, spacing: 3) {
                            ...
                            Text("Choose how you'd like to verify.")
                                .font(.liviqaSerif(20, .bold, relativeTo: .title3))
                                .foregroundStyle(LiviqaTheme.ink)
                        ...
                        Button {
                            ...
                            HStack(spacing: 12) {
                                OnbIconChip(systemName: method.icon,
                                            color: LiviqaTheme.moss, side: 32)
                                VStack(alignment: .leading, spacing: 1) {
                                    ...
                                    Text(method.trust)
                                        ...
                                        .foregroundStyle(method.id == "dfg_mpc"
                                                         ? LiviqaTheme.moss : LiviqaTheme.ink3)
                                }
                                Spacer(minLength: 0)
                            ...
                            .overlay(RoundedRectangle(cornerRadius: 14)
                                .stroke(LiviqaTheme.moss, lineWidth: 1.5))
                        }
                        .buttonStyle(.plain)
                        ...
                        if open {
                            VStack(spacing: 0) {
                                ForEach(Array(ID_METHODS.enumerated()), id: \.element.id) { i, m in
                                    if i > 0 { Divider().overlay(LiviqaTheme.line) }
                                    Button {
                                        ...
                                        HStack(alignment: .top, spacing: 11) {
                                            OnbIconChip(systemName: m.icon,
                                                        color: LiviqaTheme.ink, side: 26, corner: 8)
                                        ...
                                        .background(m.id == "dfg_mpc"
                                                    ? LiviqaTheme.moss2 : LiviqaTheme.paper2)
                                    }
                                    .buttonStyle(.plain)
                                }
                            }
    ...
    private var mpcExplainer: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            Divider().overlay(LiviqaTheme.line).padding(.top, 9)

            Text("Runs on a self-hosted multi-party-computation network — governed by the Data for Good Foundation, with no third-party server in the data path. Used only to prevent duplicate trial entries, never for identification elsewhere in Liviqa.")
                .font(.lato(11))
                .lineSpacing(3.5)
                .foregroundStyle(LiviqaTheme.ink3)
                .fixedSize(horizontal: false, vertical: true)
    ...
    private var mpcSteps: [String] {
        [String(localized: "Your device takes a brief face scan — the image never leaves your phone."),
         ...
         String(localized: "The nodes jointly answer “has this fingerprint enrolled before?” using zero-knowledge proofs — no single node, including Liviqa or Data for Good, ever sees a usable face."),
         String(localized: "Liviqa receives one signed result — unique or duplicate — issued and verified through a credential service run independently of any single party.")]
    }
}
