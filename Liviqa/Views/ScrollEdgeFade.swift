...
// at all. `liviqaScrollEdgeSoft()` was meant to cover this, but it resolves to
...
//     sits on;
//   • its HEIGHT and its opaque-vs-ramp split come from `LiviqaWindowInsets.top`
...
// `LIVIQA_SCROLL_TO=bottom` snapshot hook still scrolls exactly as far as it did.
...
struct LiviqaScrollEdgeFade: ViewModifier {
    ...
    private var topBleed: CGFloat { reduceTransparency ? liviqaTopBleedFirm : liviqaTopBleed }
    private var bottomBleed: CGFloat { reduceTransparency ? liviqaBottomBleedFirm : liviqaBottomBleed }
        ...
        content
            ...
            .overlay(alignment: .top) {
                LinearGradient(stops: topStops(), startPoint: .top, endPoint: .bottom)
                    .frame(height: LiviqaWindowInsets.top + topBleed)
    ...
    private func topStops() -> [Gradient.Stop] {
        let solid = LiviqaWindowInsets.top
        let total = solid + topBleed
        guard total > 0 else { return [.init(color: LiviqaTheme.paper.opacity(0), location: 0)] }
        let s = min(0.999, max(0, solid / total))
        let ramp = 1 - s
        if reduceTransparency {
            return [.init(color: LiviqaTheme.paper, location: 0),
                    .init(color: LiviqaTheme.paper, location: s + ramp * 0.7),
                    .init(color: LiviqaTheme.paper.opacity(0), location: 1)]
        }
        return [
            .init(color: LiviqaTheme.paper, location: 0),
            .init(color: LiviqaTheme.paper, location: s),
            .init(color: LiviqaTheme.paper.opacity(0.82), location: s + ramp * 0.30),
            .init(color: LiviqaTheme.paper.opacity(0.48), location: s + ramp * 0.55),
            .init(color: LiviqaTheme.paper.opacity(0.18), location: s + ramp * 0.78),
            .init(color: LiviqaTheme.paper.opacity(0), location: 1)
        ]
    }
    ...
    private func bottomStops() -> [Gradient.Stop] {
        if reduceTransparency {
            return [.init(color: LiviqaTheme.paper.opacity(0), location: 0),
                    .init(color: LiviqaTheme.paper, location: 0.35),
                    .init(color: LiviqaTheme.paper, location: 1)]
        }
        ...
        return [
            .init(color: LiviqaTheme.paper.opacity(0), location: 0),
            .init(color: LiviqaTheme.paper.opacity(0.08), location: 0.45),
            .init(color: LiviqaTheme.paper.opacity(0.30), location: 0.68),
            .init(color: LiviqaTheme.paper.opacity(0.68), location: 0.86),
            .init(color: LiviqaTheme.paper, location: 1)
        ]
    }
}
...
extension View {
    ...
    /// either. Pairs with `liviqaScrollEdgeSoft()` — that one adds Apple's soft
