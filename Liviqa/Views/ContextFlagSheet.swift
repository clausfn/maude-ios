...
struct ContextFlagSheet: View {
        ...
        NavigationStack {
            ScrollView {
                VStack(alignment: .leading, spacing: 14) {
                    ...
                    if !past.isEmpty {
                        LiviqaSectionHeader(label: "Days you've marked")
    ...
    private var intro: some View {
        HStack(alignment: .top, spacing: 12) {
            Image(systemName: "point.topleft.down.to.point.bottomright.curvepath")
                ...
                .background(LiviqaTheme.accentFinance)
                .clipShape(RoundedRectangle(cornerRadius: 10))
            VStack(alignment: .leading, spacing: 6) {
                Text("When life explains it")
                    .font(.liviqaSerif(20)).kerning(-0.2)
                    .foregroundStyle(LiviqaTheme.ink)
                Text("Mark the days you're away, ill or off your routine. Liviqa keeps recording them exactly as they happen — it just stops reading them as a drift from your usual.")
    ...
    private func openCard(_ flag: ContextFlag) -> some View {
        ...
        .overlay(RoundedRectangle(cornerRadius: 14)
            .stroke(LiviqaTheme.accentFinance.opacity(0.35), lineWidth: 1))
    }
                ...
                Button {
                    ...
                    HStack(spacing: 12) {
                        Image(systemName: kind.systemImage)
                            .font(.system(size: 14, weight: .semibold))
                            .foregroundStyle(picked == kind ? .white : LiviqaTheme.accentFinance)
                            .frame(width: 32, height: 32)
                            .background(picked == kind ? LiviqaTheme.accentFinance
                                                       : LiviqaTheme.accentFinance.opacity(0.12))
                            .clipShape(RoundedRectangle(cornerRadius: 9))
                        VStack(alignment: .leading, spacing: 2) {
                            Text(kind.label)
                        ...
                        Image(systemName: picked == kind ? "checkmark.circle.fill" : "circle")
                            .font(.system(size: 16))
                            .foregroundStyle(picked == kind ? LiviqaTheme.accentFinance : LiviqaTheme.line)
                    }
                    .padding(12)
                    .frame(maxWidth: .infinity, alignment: .leading)
                    .background(LiviqaTheme.paper2)
                    ...
                    .overlay(RoundedRectangle(cornerRadius: 12)
                        .stroke(picked == kind ? LiviqaTheme.accentFinance.opacity(0.5)
                                               : LiviqaTheme.line2, lineWidth: 1))
                }
                .buttonStyle(.plain)
    ...
    private var noteField: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Text("Your words, kept on this phone for you. Liviqa never reads them.")
                .font(.lato(11)).foregroundStyle(LiviqaTheme.ink3)
        }
    }
    ...
    private var markButton: some View {
        Button {
            ...
            Text("Mark from today")
                .font(.lato(14.5, .bold))
                .foregroundStyle(picked == nil ? LiviqaTheme.ink4 : LiviqaTheme.invertFG)
                .frame(maxWidth: .infinity)
                .padding(.vertical, 13)
                .background(picked == nil ? LiviqaTheme.line2 : LiviqaTheme.invertBG)
                .clipShape(RoundedRectangle(cornerRadius: 12))
        }
        .buttonStyle(.plain)
        .disabled(picked == nil)
    ...
    private func historyRow(_ flag: ContextFlag) -> some View {
        HStack(spacing: 12) {
            Image(systemName: flag.kind.systemImage)
                ...
                .background(LiviqaTheme.accentFinance.opacity(0.12))
            ...
            Button {
                ...
                Text(confirmRemove == flag.id ? String(localized: "Remove?")
                    ...
                    .foregroundStyle(confirmRemove == flag.id ? LiviqaTheme.rust : LiviqaTheme.ink3)
            }
            .buttonStyle(.plain)
        }
        .padding(12)
