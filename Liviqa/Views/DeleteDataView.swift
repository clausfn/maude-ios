...
// everything Liviqa knows about you.") → plain-words body → "What gets erased"
...
struct DeleteDataView: View {
    ...
    private var confirmBody: some View {
        ...
        HStack(alignment: .top, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                Text("You are about to erase everything Liviqa knows about you.")
        ...
        Text("That means you will lose all your data — every reading, every pattern Liviqa learned, your whole history. Nothing can bring it back, and Liviqa starts from zero if you use it again. Are you sure?")
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            erasedRow(String(localized: "Consent receipts (kept only as long as the law requires, then erased)"),
                      icon: "clock", tint: LiviqaTheme.ink3)
        }
        .padding(14)
        .background(LiviqaTheme.paper2)
        .clipShape(RoundedRectangle(cornerRadius: 12))
        ...
        .clipShape(RoundedRectangle(cornerRadius: 14))
        .overlay(RoundedRectangle(cornerRadius: 14).stroke(LiviqaTheme.rust.opacity(0.25), lineWidth: 1))
    ...
    private var failedBody: some View {
        ...
        .background(LiviqaTheme.rust2)
        .clipShape(RoundedRectangle(cornerRadius: 14))
        .overlay(RoundedRectangle(cornerRadius: 14).stroke(LiviqaTheme.rust.opacity(0.3), lineWidth: 1))
        .padding(.top, 12)
    }
    ...
    private var doneBody: some View {
        HStack(spacing: 8) {
            Image(systemName: "checkmark.circle.fill")
                .foregroundStyle(LiviqaTheme.moss)
            Text(keepDocuments
                 ? "Your data has been deleted from Liviqa's servers and this device. Your saved documents stay locked on this phone."
                 : "Your data has been deleted from Liviqa's servers and this device.")
                .font(.footnote)
                .foregroundStyle(LiviqaTheme.ink3)
        }
        .frame(maxWidth: .infinity)
    ...
    private func erasedRow(_ text: String, first: Bool = false,
                           icon: String = "trash", tint: Color = LiviqaTheme.rust) -> some View {
        VStack(spacing: 0) {
            if !first { Divider().overlay(LiviqaTheme.line2) }
