...
public nonisolated enum LearnTopic: String, CaseIterable, Identifiable, Sendable {
    ...
    /// The evidence gate a correlation must clear before Liviqa asserts it.
...
enum LearnLibrary {
    ...
    private static func methodNote(
        ...
        LearnArticle(
            ...
            plainTitle: title,
            plainKicker: String(localized: "How Liviqa works this out"),
    ...
    static var usualBand: LearnArticle {
        methodNote(
            ...
            body: String(localized: "Liviqa takes the days it actually has for a signal, finds their middle, and adds the spread those days usually have. **That band is your usual.** Nothing in it comes from anyone else, and no day is invented to fill a gap — a day without a reading is simply not in the window."),
            ...
            inPlainWords: String(localized: "Liviqa averages your own recent days and adds their usual spread. Inside that band is your ordinary; outside it is worth a glance. You never need this page to use the app."),
    ...
    static var evidenceGate: LearnArticle {
        methodNote(
            topic: .evidenceGate,
            title: String(localized: "When Liviqa says a pattern is real"),
            verdict: String(localized: "A pattern has to clear three tests before you're told about it."),
            body: String(localized: "It needs enough paired days, a strong enough relationship, and a low enough chance of coincidence. **If any one of the three falls short, Liviqa says it is still learning instead of guessing.** Most of the time, that is what it says."),
            meaningHeadline: String(localized: "Together isn't because."),
            meaningBody: String(localized: "Two things moving together is not proof that one causes the other. The gate exists so you are never handed a story your own data cannot carry — and so the times Liviqa does speak up are worth reading."),
