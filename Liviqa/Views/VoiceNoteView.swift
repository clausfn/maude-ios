...
struct VoiceNoteView: View {
        ...
        VStack(spacing: 0) {
            ...
            VStack(spacing: 0) {
                ...
                if capture.phase == .denied {
                    Text("Liviqa needs microphone access for voice notes. You can allow it in iOS Settings → Liviqa.")
            ...
            HStack(spacing: 30) {
                ...
                Button { save() } label: {
                    if isSaving {
                        ...
                        Text("Save")
                            .font(.lato(13.5, .semibold))
                            .foregroundStyle(canSave ? LiviqaTheme.moss : LiviqaTheme.ink4)
                            .frame(minWidth: 56)
                    }
                }
                .buttonStyle(.plain)
        ...
        HStack(spacing: 4) {
            ForEach(Array(capture.levels.enumerated()), id: \.offset) { _, level in
                RoundedRectangle(cornerRadius: 2)
                    .fill(LiviqaTheme.accentSleep.opacity(capture.phase == .recording ? 1 : 0.35))
    ...
    private var mainButton: some View {
        Button {
            ...
            ZStack {
                Circle()
                    .fill(LiviqaTheme.accentHeart)
                    .frame(width: 66, height: 66)
                    .shadow(color: LiviqaTheme.accentHeart.opacity(0.4), radius: 10, y: 5)
                switch capture.phase {
                case .recording:
