...
struct ShareReceiptSheet: View {
    ...
    var body: some View {
        ...
        .task {
            #if DEBUG
            // Headless screenshot hook: LIVIQA_RECEIPT_OFFER=1 opens the demoted
            // wallet-offer section so the QR state can be captured deterministically.
            if ProcessInfo.processInfo.environment["LIVIQA_RECEIPT_OFFER"] == "1" {
    ...
    private func slipCard(_ slip: ReceiptSlipModel) -> some View {
        VStack(spacing: 0) {
            ...
            ZStack(alignment: .topTrailing) {
                ...
                LiviqaApertureMark(size: 90, reversed: true)
                    .opacity(0.16)
                    .offset(x: 14, y: -20)
        ...
        .overlay(RoundedRectangle(cornerRadius: 16).stroke(LiviqaTheme.brass.opacity(0.55), lineWidth: 1))
    ...
    private var credentialBody: some View {
        VStack(spacing: 16) {
            ...
            VStack(spacing: 7) {
                Text("Your Liviqa Citizen credential")
                    .font(.liviqaSerif(20))
                    .kerning(-0.2)
                    .foregroundStyle(LiviqaTheme.ink)
