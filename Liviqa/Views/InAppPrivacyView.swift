...
struct InAppPrivacyView: View {
    // Optional pattern (mirrors LiviqaAppBar): the proof sheet can render
                ...
                HStack(alignment: .top, spacing: 11) {
                    ...
                    VStack(alignment: .leading, spacing: 3) {
                        Text("Try it: turn on Airplane Mode.")
                            .font(.lato(15.5, .bold)).foregroundStyle(LiviqaTheme.ink)
                        Text("Liviqa keeps working — readings, insights, journal — because everything happens on this phone. The internet is only needed when you choose to share.")
                            .font(.lato(12.5)).lineSpacing(2).foregroundStyle(LiviqaTheme.ink2)
                            .fixedSize(horizontal: false, vertical: true)
                    }
                }
        ...
        VStack(alignment: .leading, spacing: 0) {
            HStack(spacing: 10) {
                RoundedRectangle(cornerRadius: 9)
                    .fill(grant.isActive ? LiviqaTheme.moss : LiviqaTheme.ink4)
                ...
                Text(grant.isActive ? "Active" : "Stopped")
                    ...
                    .foregroundStyle(grant.isActive ? .white : LiviqaTheme.ink2)
                    .padding(.horizontal, 8).padding(.vertical, 4)
                    .background(grant.isActive ? LiviqaTheme.moss : LiviqaTheme.line2)
                    .clipShape(Capsule())
            }
