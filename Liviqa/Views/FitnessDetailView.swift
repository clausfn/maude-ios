...
// built to the DFitness editorial anatomy (design_handoff_liviqa_a7: d-insights.jsx
                ...
                ForEach(Array(m.workouts.enumerated()), id: \.offset) { i, w in
                    HStack(spacing: 12) {
                        ...
                        VStack(alignment: .leading, spacing: 1) {
                            ...
                            Text(w.meta)
                                .font(.liviqaMono(11.5)).foregroundStyle(LiviqaTheme.ink3)
                        }
                        Spacer()
                    ...
                    .overlay(alignment: .top) {
                        if i > 0 { Divider().background(LiviqaTheme.line2) }
                    }
...
extension FitnessDetailView.Model {
    ...
    static func zoneItems(_ zones: [FitnessDetail.ZoneShare]) -> [ZoneShareItem] {
        let colors: [String: Color] = [
            ...
            "Z2 · endurance": LiviqaTheme.accentRecovery,
            "Z3 · tempo": LiviqaTheme.amber,
            "Z4 · threshold": LiviqaTheme.accentHeart,
        ]
        return zones.map {
            ZoneShareItem(name: $0.name,
                          minText: FitnessDeriver.durText($0.minutes),
                          frac: $0.minutes,
                          color: colors[$0.name] ?? LiviqaTheme.accentRecovery)
        }
    }
