        ...
        NavigationStack {
            ScrollView {
                VStack(alignment: .leading, spacing: 16) {
                    Text("A plain-text report of your last 14 nights: every sleep sample Apple Health holds (which app or device wrote it, its stage, start and end) next to the figures Liviqa derived from them — and, night by night, exactly which samples each total came from.")
                        .font(.footnote)
                    ...
                    if appState.isSampleMode {
                        Text("Sample data is on, so Liviqa is not reading your real Apple Health data right now. Leave sample data first — this report is about your own nights.")
                            .font(.caption)
                    ...
                    if failed {
                        Text("The report couldn't be built. Check that Liviqa has permission to read Sleep in the Health app, then try again.")
                            .font(.caption)
    ...
    private func generate() {
        ...
        Task { @MainActor in
            ...
            let url = FileManager.default.temporaryDirectory
                .appendingPathComponent("liviqa-sleep-diagnostics-\(stamp).txt")
            do {
