...
// Requires `Liviqa/Theme.swift` to be a member of that target too (SETUP.md §4)
// — every colour and face here is a LiviqaTheme token, no hex literals, so the
...
struct EditionRule: View {
    ...
    var body: some View {
        RoundedRectangle(cornerRadius: 1.5)
            .fill(edition == .evening ? LiviqaTheme.clay : LiviqaTheme.moss)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 1) {
            ...
            Text(chip.note ?? WidgetCopy.placeholderDash)
                .font(.lato(10))
                .foregroundStyle(chip.needsAttention ? LiviqaTheme.clayText : LiviqaTheme.ink2)
                .lineLimit(1)
                .minimumScaleFactor(0.8)
        }
        .accessibilityElement(children: .combine)
