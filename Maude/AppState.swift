...
final class AppState {
    ...
    // by `-uiTestAutoDemo` / `MAUDE_DATA=mock` for screenshots & tests).
    ...
    /// `MAUDE_DATA=mock`; forced real for `MAUDE_DATA=healthKit`; otherwise real
    ...
    static func resolveProviderKind(arguments: [String],
        ...
        #if DEBUG
        if arguments.contains("-uiTestAutoDemo") || env["MAUDE_DATA"] == "mock" { return .mock }
        if env["MAUDE_DATA"] == "healthKit" { return .healthKit }
        #endif
    ...
    private static func openStore() -> (container: ModelContainer?, diskFailed: Bool) {
        do {
            return (try MaudeStore.makeContainer(), false)
        } catch {
            print("‼️ MaudeStore: on-disk container failed to open (\(error)). Falling back to in-memory for this session.")
            return (try? MaudeStore.makeContainer(inMemory: true), true)
        }
    }
    ...
    /// Set by the avatar button in `MaudeAppBar`; MainTabView presents ProfileSheet.
    ...
    init(supabase: any SupabaseServiceProtocol = Config.makeService(),
        ...
        NotificationCenter.default.addObserver(forName: .maudePushToken, object: nil, queue: .main) { note in
        ...
        NotificationCenter.default.addObserver(forName: .maudeOpenResearch, object: nil, queue: .main) { _ in
        ...
        NotificationCenter.default.addObserver(forName: .maudeResearchReceived, object: nil, queue: .main) { _ in
        ...
        NotificationCenter.default.addObserver(forName: .maudeOpenAttention, object: nil, queue: .main) { note in
    ...
    /// Permanently erase everything Maude holds on this device (T-DEL-01,
    ...
    /// Keychain session token, and Maude's UserDefaults leftovers — then sign
    ...
    func deleteAllData(keepDocuments: Bool = false) async {
        ...
        if let container = modelContainer {
            ...
            for model in MaudeStore.models {
        ...
        await signOut()
        // 5. Maude UserDefaults leftovers.
    ...
    func exportMyData() async -> URL? {
        do {
            ...
            let url = FileManager.default.temporaryDirectory
                .appendingPathComponent("Maude-export-\(df.string(from: Date())).json")
    ...
    func contributeHealthResearch() async {
        ...
        let client = (supabase as? SundhedIngesting) ?? MaudeSundhedIngestClient()
        do {
    ...
    static let displayNameKey = "maude.profile.displayName"
    ...
    static let initialBackfillKey = "maude.backfill.initialDone"
    ...
    /// UC-21 — issue a Maude Share Receipt for a grant into the My DfG wallet.
    ...
    /// UC-A — issue the citizen's own "Maude Citizen" sign-in credential into
    /// their My DfG wallet (pseudonymous: role + member id + issue date only).
    /// Returns the wallet offer URL, or nil with `lastError` set.
    /// Expiry of the last-issued Maude Citizen credential (short-validity
    ...
    var citizenCredentialValidUntil: Date? {
        get {
            let t = UserDefaults.standard.double(forKey: "maude.citizenCred.validUntil")
            return t > 0 ? Date(timeIntervalSince1970: t) : nil
        }
        set { UserDefaults.standard.set(newValue?.timeIntervalSince1970 ?? 0, forKey: "maude.citizenCred.validUntil") }
    }

    @MainActor
    func issueCitizenCredential() async -> URL? {
        guard let sov = sovereign else {
            lastError = "Your Maude Citizen credential requires the sovereign backend."
            return nil
        }
        do {
