// MaudeBackendService.swift — EU-sovereign backend client (NestJS/Scaleway+Ory).
...
//
// FR-SHARE-02 · Maude_iOS_Backend_Contract_v01 §5 · openapi.yaml.
...
protocol SovereignSharing: Sendable {
    ...
    /// Issue a Maude Share Receipt (UC-21) for an active grant into the citizen's
    ...
    /// Issue the citizen's own "Maude Citizen" sign-in credential (UC-A) into
...
final class MaudeBackendService: SupabaseServiceProtocol, SovereignSharing, CareConnect, DataRights, PasswordRecovery, @unchecked Sendable {
    ...
    init(baseURL: URL, devToken: String? = nil, auth: SupabaseAuthClient? = nil,
         session: URLSession = MaudeBackendService.defaultSession) {
    ...
    func fetchActiveConsults() async throws -> [ConsultSummary] {
        ...
        return dtos.map { d in
            ConsultSummary(
                id: d.id,
                roomName: d.roomName ?? "maude-consult-\(d.id)",
    ...
    func joinConsult(id: String) async throws -> String {
        ...
        return "maude-consult-\(id)"     // deterministic room (contract §video)
    }
    ...
    private func sendRaw(_ path: String, method: String, bodyData: Data?, base: URL? = nil,
        ...
        case 500...599:
            ...
            throw SupabaseError.serverError(String(localized: "Maude's servers are having trouble right now. Please try again in a few minutes."))
        default:
...
/// exercise `MaudeBackendService.walletEvent(from:)` without URLSession.
