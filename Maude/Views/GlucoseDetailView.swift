...
// DGlucose editorial anatomy (design_handoff_maude_a7: screen-glucose.jsx +
...
struct GlucoseDetailView: View {
    ...
    private func hero(_ m: Model) -> some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            HStack(alignment: .lastTextBaseline, spacing: 6) {
                Text("\(m.statPct)")
                    .font(.maudeSerif(48, .bold, relativeTo: .largeTitle))
                    .foregroundStyle(.white)
                Text("% in range")
        ...
        .background(
            LinearGradient(colors: [MaudeTheme.accentGlucose, MaudeTheme.accentGlucose.opacity(0.9)],
                ...
                if !clinicalTIRZones {
                    HStack(spacing: 5) {
                        ...
                        RoundedRectangle(cornerRadius: 2)
                            .fill(MaudeTheme.moss.opacity(0.4))
    ...
    private var emptyState: some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack(spacing: 6) {
                Image(systemName: "drop.fill").font(.system(size: 12))
                    .foregroundStyle(MaudeTheme.accentGlucose)
                Text("GLUCOSE")
                    .font(.maudeKicker(10.5)).tracking(MaudeTheme.Tracking.kicker)
                    .foregroundStyle(MaudeTheme.ink3)
            }
...
struct GlucoseTIRBar: View {
    ...
    private var bands: [Band] {
        ...
        return [
            Band(id: 0, label: "Very low",  range: "<3.0",      pct: p[0], color: MaudeTheme.tirVeryLow,  hatch: true,  dots: false, isTarget: false),
            Band(id: 1, label: "Low",       range: "3.0–3.9",   pct: p[1], color: MaudeTheme.tirLow,      hatch: false, dots: false, isTarget: false),
            Band(id: 2, label: "In range",  range: "3.9–10.0",  pct: p[2], color: MaudeTheme.tirTarget,   hatch: false, dots: false, isTarget: true),
            Band(id: 3, label: "High",      range: "10.0–13.9", pct: p[3], color: MaudeTheme.tirHigh,     hatch: false, dots: false, isTarget: false),
            Band(id: 4, label: "Very high", range: ">13.9",     pct: p[4], color: MaudeTheme.tirVeryHigh, hatch: false, dots: true,  isTarget: false),
        ]
    }
                ...
                ForEach(bands) { b in
                    HStack(spacing: 6) {
                        ...
                        Text(b.label)
                            ...
                            .foregroundStyle(b.isTarget ? MaudeTheme.ink : MaudeTheme.ink3)
                            .lineLimit(1)
                        Spacer(minLength: 2)
                        Text(b.range)
                            .font(.maudeMono(10)).foregroundStyle(MaudeTheme.ink3)
                            .lineLimit(1)
                        Text("\(Int(b.pct.rounded()))%")
                            .font(.maudeMono(10).weight(.bold))
                            .foregroundStyle(b.isTarget ? MaudeTheme.moss : MaudeTheme.ink3)
            ...
            ZStack(alignment: .topLeading) {
                ...
                ForEach(Array(days.enumerated()), id: \.offset) { i, day in
                    ...
                    if clinical, day.hi > high {
                        ...
                        RoundedRectangle(cornerRadius: bw / 2)
                            .fill(MaudeTheme.clinRed.opacity(day.isToday ? 1 : 0.6))
                    ...
                    if clinical, day.lo < low {
                        ...
                        RoundedRectangle(cornerRadius: bw / 2)
                            .fill(MaudeTheme.clinRed.opacity(0.6))
                            .frame(width: bw, height: max(3, bot - top))
                            .position(x: cx, y: (top + bot) / 2)
                    }
                }
            ...
            ForEach(Array(days.enumerated()), id: \.offset) { _, day in
                VStack(spacing: 1) {
                    Text(day.label)
                        ...
                        .foregroundStyle(day.isToday ? MaudeTheme.ink : MaudeTheme.ink4)
                    Text("\(day.tirPct)%")
                        .font(.maudeMono(9))
                        .foregroundStyle(day.isToday ? MaudeTheme.ink3 : MaudeTheme.ink4)
                }
                .frame(maxWidth: .infinity)
            }
        }
...
struct GlucoseCompareRows: View {
    ...
    var color: Color = MaudeTheme.accentGlucose
