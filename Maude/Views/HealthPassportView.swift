...
struct HealthPassportView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 0) {
                    ...
                    // ── App bar ──
                    MaudeAppBar(title: "Health Passport", showMark: false)

                    VStack(alignment: .leading, spacing: 0) {

                        // ── ABOUT YOU (declared — user fills this in) ──
                        MaudeSectionHeader(label: "About you")
                        ...
                        // ── YOUR NUMBERS ──
                        MaudeSectionHeader(label: "Your numbers")
                        ...
                        MaudeSectionHeader(label: "Your wellness summary")
                        ...
                        // ── PRIVACY RECORD ──
                        MaudeSectionHeader(label: "Privacy record")
                        ...
                        VStack(alignment: .leading, spacing: 10) {
                            HStack(alignment: .top, spacing: 10) {
                                ...
                                VStack(alignment: .leading, spacing: 4) {
                                    ...
                                    Text("Your consent decisions are independently logged and cannot be edited by anyone — including Maude.")
                                        .font(.lato(12.5))
                                        .lineSpacing(2)
                        ...
                        // ── CONNECTED SOURCES ──
                        MaudeSectionHeader(label: "Connected sources")
                        ...
                        // ── Footer ──
                        Text("Your data belongs to you. Maude has no access to your raw records.")
                            .font(.caption)
                            .foregroundStyle(MaudeTheme.ink4)
    ...
    private var healthRecordSection: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            if isEmpty {
                MaudeSectionHeader(label: "Health record")
                recordEmptyState
            } else {
                // Lab results
                MaudeSectionHeader(label: "Lab results")
                recordCard {
                    ForEach(Array(labs.enumerated()), id: \.element.persistentModelID) { i, o in
                        if i > 0 { Divider().background(MaudeTheme.line2).padding(.leading, 14) }
                        labRow(o)
                    }
                }
                ...
                if !conds.isEmpty {
                    ...
                    MaudeSectionHeader(label: "Diagnoses")
                    if !major.isEmpty {
                        recordCard {
                        ...
                        Button { withAnimation { showMinorDiagnoses.toggle() } } label: {
                            HStack(spacing: 8) {
                                ...
                                Text("Past, minor & administrative entries (\(other.count))")
                                    .font(.lato(12.5, .semibold)).foregroundStyle(MaudeTheme.ink2)
                                Spacer()
                            }
                ...
                // Medicine
                if !meds.isEmpty {
                    MaudeSectionHeader(label: "Medicine")
                    recordCard {
    ...
    private var recordEmptyState: some View {
        HStack(alignment: .top, spacing: 10) {
            Image(systemName: "tray.and.arrow.down")
                .font(.lato(15)).foregroundStyle(MaudeTheme.ink4)
    ...
    private func labRow(_ o: HealthObservation) -> some View {
        HStack(spacing: 10) {
            ...
            Text("\(HealthDisplay.number(o.value)) \(o.unit)")
                .font(.maudeMono(15)).monospacedDigit()
                .foregroundStyle(MaudeTheme.ink)
        }
            ...
            VStack(alignment: .leading, spacing: 3) {
                ...
                HStack(spacing: 6) {
                    Text(c.icd10)
                        ...
                        .background(MaudeTheme.clay2).clipShape(Capsule())
    ...
    private func medRow(_ m: HealthMedication) -> some View {
        HStack(spacing: 10) {
            VStack(alignment: .leading, spacing: 2) {
                Text(m.name).font(.lato(13.5, .semibold)).foregroundStyle(MaudeTheme.ink).lineLimit(1)
                if let atc = m.atc, !atc.isEmpty {
                    Text(atc).font(.lato(11, .medium)).foregroundStyle(MaudeTheme.moss)
                }
            }
            Spacer()
            sourceChip(m.source)
    ...
    private func sourceChip(_ rawSource: String) -> some View {
        Text((HealthDataSource(rawValue: rawSource)?.displayLabel ?? rawSource).uppercased())
            .font(.maudeKicker(8.5)).tracking(0.5)
            .foregroundStyle(MaudeTheme.moss)
            .padding(.horizontal, 7).padding(.vertical, 3)
            .background(MaudeTheme.moss3).clipShape(Capsule())
    }
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            if let researchNote {
                Text(researchNote)
                    ...
                    .foregroundStyle(appState.researchContributed ? MaudeTheme.moss : MaudeTheme.rust)
                    .fixedSize(horizontal: false, vertical: true)
            }
    ...
    private var timeInRangeTile: some View {
        ...
        let valueColor: Color = tir >= 80 ? MaudeTheme.moss
                              : tir >= 60 ? MaudeTheme.clay
                              : MaudeTheme.rust
        ...
        Button {
            ...
            HStack(spacing: 12) {
                Image(systemName: icon)
                    .font(.lato(13))
                    .foregroundStyle(value != nil ? MaudeTheme.moss : MaudeTheme.ink4)
                    .frame(width: 20)
                VStack(alignment: .leading, spacing: 2) {
                    Text(label)
