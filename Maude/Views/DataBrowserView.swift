...
struct DataBrowserView: View {
    ...
    var body: some View {
        Group {
            switch phase {
            case .loading:
                ProgressView().tint(MaudeTheme.moss)
                    .frame(maxWidth: .infinity, maxHeight: .infinity)
            case .unavailable:
                stateCard(
                    icon: "heart.slash",
                    title: String(localized: "Apple Health is not available here"),
                    body: String(localized: "This device does not offer HealthKit, so there is nothing Maude could read."))
            case .empty, .reading:
                emptyState
            case .ready:
                list
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 4) {
            Text("Every data point Apple Health shares with Maude, exactly as it was recorded — read on this phone, uploaded nowhere.")
    ...
    private var footer: some View {
        HStack(alignment: .top, spacing: 9) {
            ...
            Text("You choose what Maude may read on Apple's permission sheet, and you can change it any time in the Health app. Deleting your Maude data removes every reading listed here.")
        ...
        ScrollView {
            VStack(spacing: 14) {
                stateCard(
                    ...
                    body: String(localized: "Maude can list every kind of data your Apple Health holds — each reading shown as it was recorded, with the app it came from. You choose exactly which kinds on Apple's permission sheet."))
                OnbPrimaryButton(
                    label: phase == .reading
                ...
                ForEach(slots) { slot in
                    if let v = slot.value {
                        RoundedRectangle(cornerRadius: 1)
                            .fill(MaudeTheme.fjordBright.opacity(0.75))
                            .frame(width: barW,
    ...
    var body: some View {
        HStack(spacing: 10) {
            VStack(alignment: .leading, spacing: 2) {
                ...
                Text("Every kind of data Apple Health shares with Maude, listed reading by reading.")
                    .font(.lato(12)).lineSpacing(2)
                    .foregroundStyle(MaudeTheme.ink2)
                    .fixedSize(horizontal: false, vertical: true)
            }
