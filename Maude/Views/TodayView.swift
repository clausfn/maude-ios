...
// Recreated from the locked design prototype (maude-design-system · Home C):
...
struct TodayView: View {
    ...
    ///
    /// DEBUG only: when `MAUDE_EDITION` forces an edition for snapshot QA, the
    /// greeting follows the FORCED edition instead of the wall clock, so a frame
    /// captured at 22:00 with `MAUDE_EDITION=day` doesn't open with "Good
    ...
    private var greetingHour: Int {
        ...
        #if DEBUG
        if let e = ProcessInfo.processInfo.environment["MAUDE_EDITION"] {
    ...
    var body: some View {
        #if DEBUG
        if let p = ProcessInfo.processInfo.environment["MAUDE_OPEN_PILLAR"],
           let pillar = WellnessPillar(rawValue: p) {
            MetricDetailView(pillar: pillar)
        } else if ProcessInfo.processInfo.environment["MAUDE_OPEN_TRENDS"] == "1" {
    ...
    private var mainBody: some View {
        ScrollViewReader { proxy in
            scrollBody
                ...
                .task {
                    if ProcessInfo.processInfo.environment["MAUDE_SCROLL_TO"] == "bottom" {
    ...
    private var scrollBody: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 0) {
                ...
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    Text(greetingLine)
                        .font(.maudeSerif(26)).kerning(-0.2)
        ...
        .maudeScrollEdgeSoft()   // iOS 26 + flag: title dissolves into the feed
        ...
        HStack(alignment: .center, spacing: 14) {
            VStack(alignment: .leading, spacing: 0) {
                ...
                Text(flag.kind.todayHeadline)
                    .font(.maudeSerif(23)).kerning(-0.2).lineSpacing(2)
                    .foregroundStyle(MaudeTheme.ink)
                    .fixedSize(horizontal: false, vertical: true)
                    .padding(.top, 8)
                RoundedRectangle(cornerRadius: 2)
                    .fill(MaudeTheme.accentFinance)
    ...
    private func contextQuietNote(_ flag: ContextFlag) -> some View {
        ...
        .padding(12)
        .frame(maxWidth: .infinity, alignment: .leading)
        .background(MaudeTheme.accentFinance.opacity(0.10))
        .clipShape(RoundedRectangle(cornerRadius: 14))
        .overlay(RoundedRectangle(cornerRadius: 14)
            .stroke(MaudeTheme.accentFinance.opacity(0.28), lineWidth: 1))
    ...
    private var storeDegradedBanner: some View {
        HStack(alignment: .top, spacing: 10) {
            Image(systemName: "externaldrive.badge.exclamationmark")
                .font(.lato(15)).foregroundStyle(MaudeTheme.rust)
            VStack(alignment: .leading, spacing: 2) {
                Text("Storage couldn't open — new data isn't being saved")
                    .font(.lato(13.5, .bold)).foregroundStyle(MaudeTheme.ink)
                Text("Anything already on this device is safe. Quit and reopen Maude to try again — if this keeps happening, tell us from Settings.")
                    .font(.lato(12.5)).lineSpacing(2)
                    .foregroundStyle(MaudeTheme.ink2)
            }
        }
        ...
        .clipShape(RoundedRectangle(cornerRadius: 14))
        .overlay(RoundedRectangle(cornerRadius: 14).stroke(MaudeTheme.rust.opacity(0.35), lineWidth: 1))
        .accessibilityElement(children: .combine)
    }
    ...
    private var sundhedConnectCard: some View {
        Button { showSundhedImport = true } label: {
            ...
            .background(MaudeTheme.paper2)
            .clipShape(RoundedRectangle(cornerRadius: 14))
            .overlay(RoundedRectangle(cornerRadius: 14).stroke(MaudeTheme.moss.opacity(0.35), lineWidth: 1))
        }
        .buttonStyle(.plain)
    }
    ...
    private var connectHealthHint: some View {
        HStack(alignment: .top, spacing: 10) {
            Image(systemName: "heart.text.square")
                .font(.lato(15)).foregroundStyle(MaudeTheme.moss)
            VStack(alignment: .leading, spacing: 2) {
                ...
                Button { onOpenSettings?() } label: {
                    ...
                    Text("Connect Apple Health in Settings — and make sure the data categories are turned ON →")
                        .font(.lato(12.5)).foregroundStyle(MaudeTheme.moss)
                        .multilineTextAlignment(.leading)
                }
                .buttonStyle(.plain)
            }
    ...
    private var baselineBuildingCard: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            Text("Learning your normal.")
                .font(.maudeSerif(20)).kerning(-0.2).lineSpacing(2)
                .foregroundStyle(MaudeTheme.ink)
                .padding(.top, 10)
            ...
            GeometryReader { geo in
                ZStack(alignment: .leading) {
                    ...
                    Capsule().fill(MaudeTheme.fjordBright)
                        .frame(width: geo.size.width * 0.18)
                }
            }
            .frame(height: 5)
            .padding(.top, 12)
            Text("Your readings are coming in. Until Maude has enough of your own days to compare against, nothing is shown as an insight — your first ones typically appear after about 3 days.")
        ...
        .overlay(RoundedRectangle(cornerRadius: 18).stroke(MaudeTheme.moss3, lineWidth: 1))
    ...
    private var attentionCard: some View {
        Button {
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                Text(heroHeadline)
                    .font(.maudeSerif(17)).kerning(-0.2)
                    .lineSpacing(2)
                    .multilineTextAlignment(.leading)
                    .foregroundStyle(MaudeTheme.ink)
                    .padding(.top, 9)
            ...
            .clipShape(RoundedRectangle(cornerRadius: 18))
            .overlay(RoundedRectangle(cornerRadius: 18).stroke(MaudeTheme.clay, lineWidth: 1.5))
        }
        .buttonStyle(.plain)
    }
    ...
    private var quietLine: some View {
        HStack(spacing: 10) {
            ZStack {
                Circle().stroke(MaudeTheme.moss, lineWidth: 1.6).frame(width: 19, height: 19)
                Image(systemName: "checkmark")
                    .font(.system(size: 9, weight: .bold))
                    .foregroundStyle(MaudeTheme.moss)
            }
            Text("Nothing needs your attention today.")
                .font(.maudeSerif(14, .regular))
                .italic()
                .foregroundStyle(MaudeTheme.ink2)
        }
        .frame(maxWidth: .infinity)
    ...
    private func insightCompareCard(_ cmp: TrendsPeriodCompare) -> some View {
        VStack(alignment: .leading, spacing: 0) {
            Text(String(localized: "Sleep · month to month").uppercased())
                ...
                .foregroundStyle(MaudeTheme.ink3)
            Text(cmp.sentence)
                .font(.maudeSerif(17)).kerning(-0.2).lineSpacing(2)
                .foregroundStyle(MaudeTheme.ink)
                .fixedSize(horizontal: false, vertical: true)
                .padding(.top, 8)
            InsightCompareBars(
            ...
            VStack(alignment: .leading, spacing: 0) {
                HStack(spacing: 6) {
                    Image(systemName: "calendar")
                        .font(.system(size: 11)).foregroundStyle(MaudeTheme.moss)
                    Text(String(localized: "This week").uppercased())
                        .font(.maudeKicker(10)).tracking(1.2)
                        .foregroundStyle(MaudeTheme.moss)
                    Spacer(minLength: 8)
                    Text(String(localized: "View full week"))
                        .font(.lato(12, .bold)).foregroundStyle(MaudeTheme.ink3)
                        .lineLimit(1)
                    Image(systemName: "arrow.right")
                        .font(.system(size: 10, weight: .bold)).foregroundStyle(MaudeTheme.ink3)
                }
                Text(weekHeadline)
                    .font(.maudeSerif(17)).kerning(-0.3).lineSpacing(2)
                    .multilineTextAlignment(.leading)
                    .foregroundStyle(MaudeTheme.ink)
                    .padding(.top, 9)
                if let spark = weekSparkline {
                    MiniSparkline(values: spark, tint: MaudeTheme.moss, height: 30)
                        .padding(.top, 11)
                }
            }
            .frame(maxWidth: .infinity, alignment: .leading)
    ...
    private func signalCard(_ card: HomeCard) -> some View {
        ...
        switch card.domain {
        case .sleep:
            SignalCardView(rule: MaudeTheme.accentSleep, icon: "moon.fill",
                           ...
                           onWhy: { seeWhy = signalWhy(.sleep, verdict: sleepVerdict,
                                                       ...
                                                       series: \.sleepWeek) })
        case .glucose:
            SignalCardView(rule: MaudeTheme.accentGlucose, icon: "drop.fill",
                           ...
                           onWhy: { seeWhy = signalWhy(.glucose, verdict: glucoseVerdict,
                                                       ...
                                                       series: \.inRangeWeek) })
        case .activity:
            SignalCardView(rule: MaudeTheme.fjordBright, icon: "figure.walk",
                           ...
                           onWhy: { seeWhy = SeeWhyExplainer.activityCard(
                               ...
                               coldStart: coldStart) })
        case .fitness:
            SignalCardView(rule: MaudeTheme.accentRecovery, icon: "figure.outdoor.cycle",
                           ...
                           onWhy: { seeWhy = SeeWhyExplainer.fitnessCard(
                               ...
                               coldStart: coldStart) })
        case .recovery:
            SignalCardView(rule: MaudeTheme.accentRecovery, icon: "waveform.path.ecg",
                           ...
                           onWhy: { seeWhy = signalWhy(.recovery, verdict: recoveryVerdict,
                                                       ...
                                                       series: \.hrvWeek) })
        case .heart:
            SignalCardView(rule: MaudeTheme.accentHeart, icon: "heart.fill",
                           ...
                           onWhy: { seeWhy = signalWhy(.heart, verdict: heartVerdict,
                                                       ...
                                                       series: \.rhrWeek) })
        case .body:
            SignalCardView(rule: MaudeTheme.accentBody, icon: "figure.arms.open",
    ...
    /// DEBUG env `MAUDE_EDITION=evening|day` forces either for snapshot QA.
        ...
        HStack(alignment: .center, spacing: 14) {
            VStack(alignment: .leading, spacing: 0) {
                ...
                Text(closingHeadline)
                    .font(.maudeSerif(23)).kerning(-0.2).lineSpacing(3)
                    .foregroundStyle(MaudeTheme.ink)
                    .padding(.top, 8)
                RoundedRectangle(cornerRadius: 2)
    ...
    private func partColor(_ domain: DayScoreComposer.Domain) -> Color {
        switch domain {
        case .sleep:    return MaudeTheme.accentSleep
        case .glucose:  return MaudeTheme.accentGlucose
        case .movement: return MaudeTheme.fjordBright
        case .recovery: return MaudeTheme.accentRecovery
        }
    }
        ...
        return HStack(spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 0) {
                Text(String(localized: "How today scored").uppercased())
                    ...
                    .foregroundStyle(MaudeTheme.ink3)
                Text(score.verdict)
                    .font(.maudeSerif(16.5)).lineSpacing(2)
                    .foregroundStyle(MaudeTheme.ink)
    ...
    private func monthTrendCard(_ trend: RecoveryTrend) -> some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            MonthTrendLine(data: trend.data, avg: trend.avg,
                           color: MaudeTheme.clay, color2: MaudeTheme.accentSleep,
    ...
    private var tomorrowHook: some View {
        VStack(alignment: .leading, spacing: 6) {
            Text(String(localized: "Tomorrow").uppercased())
                ...
                .foregroundStyle(MaudeTheme.ink3)
            Text(tomorrowLine)
                .font(.maudeSerif(14.5)).italic().lineSpacing(3)
                .foregroundStyle(MaudeTheme.ink2)
        }
        .frame(maxWidth: .infinity, alignment: .leading)
        .padding(.horizontal, 17).padding(.vertical, 14)
        .background(MaudeTheme.paper2.opacity(0.5))
    ...
    private func momentumStrip(_ items: [MomentumItem]) -> some View {
                ...
                ForEach(items) { item in
                    HStack(spacing: 4) {
                        if let up = item.up {
                            ...
                            Text("—").font(.lato(11)).foregroundStyle(MaudeTheme.ink3)
                        }
                        Text(item.text)
        ...
        .background(MaudeTheme.paper2.opacity(0.55))   // half-plate in BOTH modes
        .clipShape(RoundedRectangle(cornerRadius: 16))
        .overlay(RoundedRectangle(cornerRadius: 16).stroke(MaudeTheme.line, lineWidth: 1))
    }
...
private struct SignalCardView: View {
    ...
    var body: some View {
        ...
        .shadow(color: MaudeTheme.cardShadow, radius: 8, y: 4)
    }
    ...
    private var cardBody: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            Text(verdict)
                .font(.maudeSerif(15)).kerning(-0.1)
                .foregroundStyle(MaudeTheme.ink)
