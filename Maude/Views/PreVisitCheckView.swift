...
struct PreVisitCheckView: View {
    ...
    // expiring grant + derived summary (the payload Maude PRO renders).
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 14) {
                NavBackHeader(onBack: { dismiss() }) {
                    Text("Get ready").font(.lato(15, .black)).foregroundStyle(MaudeTheme.ink)
                }
                .padding(.top, 6)
                ...
                Text("A quick check so your video call with \(recipientName) starts smoothly.")
                    .font(.lato(13)).lineSpacing(2).foregroundStyle(MaudeTheme.ink3)
                ...
                Button { Task { await toggleShare() } } label: {
                    HStack(alignment: .top, spacing: 12) {
                        ZStack {
                            RoundedRectangle(cornerRadius: 6).fill(shareConsent ? MaudeTheme.moss : Color.clear)
                            RoundedRectangle(cornerRadius: 6).stroke(shareConsent ? MaudeTheme.moss : MaudeTheme.line, lineWidth: 1.5)
                        ...
                        VStack(alignment: .leading, spacing: 3) {
                            ...
                            Text("\(recipientName) sees only the derived summary you've consented to — never your raw data, which stays on this device.")
                                .font(.lato(12.5)).lineSpacing(2).foregroundStyle(MaudeTheme.ink3)
                                .fixedSize(horizontal: false, vertical: true).multilineTextAlignment(.leading)
                        }
                        Spacer(minLength: 0)
                    }
                    .padding(14)
                    .background(shareConsent ? MaudeTheme.moss2 : MaudeTheme.paper2)
                    .clipShape(RoundedRectangle(cornerRadius: 12))
                    .overlay(RoundedRectangle(cornerRadius: 12).stroke(shareConsent ? MaudeTheme.moss3 : MaudeTheme.line, lineWidth: shareConsent ? 1 : 0.5))
                }
                .buttonStyle(.plain)
                .disabled(arming)
                ...
                if arming {
                    ...
                    Text(armError).font(.lato(11.5)).foregroundStyle(MaudeTheme.rust)
                }

                // 2 — device & connection test
                kicker("Your camera, mic & connection").padding(.top, 6)
                VStack(spacing: 0) {
                    checkRow("Camera", "video.fill", cam, fix: "Enable camera access for Maude in Settings.")
                    Divider().background(MaudeTheme.line2).padding(.leading, 46)
                    checkRow("Microphone", "mic.fill", mic, fix: "Enable microphone access for Maude in Settings.")
                    Divider().background(MaudeTheme.line2).padding(.leading, 46)
                ...
                HStack(spacing: 9) {
                    Image(systemName: ready ? "checkmark.circle.fill" : "hourglass")
                        .font(.system(size: 15)).foregroundStyle(ready ? MaudeTheme.moss : MaudeTheme.ink4)
                    Text(ready ? (joinable != nil ? "You're ready to join."
                        ...
                        .font(.lato(12.5, .bold)).foregroundStyle(ready ? MaudeTheme.ink : MaudeTheme.ink3)
                        .fixedSize(horizontal: false, vertical: true)
                    Spacer(minLength: 0)
                }
                .padding(12)
                .frame(maxWidth: .infinity, alignment: .leading)
                .background(ready ? MaudeTheme.moss2 : MaudeTheme.paper2)
                .clipShape(RoundedRectangle(cornerRadius: 12))
                .overlay(RoundedRectangle(cornerRadius: 12).stroke(ready ? MaudeTheme.moss3 : MaudeTheme.line, lineWidth: ready ? 1 : 0.5))
                .padding(.top, 4)

                if ready, let sc = joinable {
                    // A7 canvas CTA — flows straight into the waiting room.
                    Button { enterWaiting = sc } label: {
                        Text("Enter waiting room").font(.lato(15, .bold)).foregroundStyle(MaudeTheme.invertFG)
                            .frame(maxWidth: .infinity).padding(.vertical, 15)
                            .background(MaudeTheme.invertBG).clipShape(RoundedRectangle(cornerRadius: 13))
                    }
                    .buttonStyle(.plain).padding(.top, 8)
                } else {
                    Button { dismiss() } label: {
                        Text("Done").font(.lato(15, .bold)).foregroundStyle(MaudeTheme.invertFG)
                            .frame(maxWidth: .infinity).padding(.vertical, 15)
                            .background(MaudeTheme.invertBG).clipShape(RoundedRectangle(cornerRadius: 13))
                    }
                    .buttonStyle(.plain).padding(.top, 8)
    ...
    private func checkRow(_ label: String, _ icon: String, _ status: Check, fix: String) -> some View {
        HStack(alignment: .top, spacing: 12) {
            Image(systemName: icon).font(.system(size: 15)).foregroundStyle(MaudeTheme.ink3)
                .frame(width: 22).padding(.top, 1)
            VStack(alignment: .leading, spacing: 2) {
                Text(label).font(.lato(14, .bold)).foregroundStyle(MaudeTheme.ink)
                Text(statusLabel(status)).font(.lato(12)).foregroundStyle(statusColor(status))
                if status == .fail || status == .warn {
                    Text(fix).font(.lato(11.5)).foregroundStyle(MaudeTheme.ink3)
    ...
    private func statusColor(_ s: Check) -> Color {
        switch s {
        case .ok: return MaudeTheme.moss
        case .warn: return MaudeTheme.clay
        case .fail: return MaudeTheme.rust
        case .checking: return MaudeTheme.ink4
        }
    }
    ...
    private func netCheck() async -> Check {
        ...
        let queue = DispatchQueue(label: "maude.previsit.net")
