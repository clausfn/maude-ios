    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 6) {
                ...
                Text(kicker.uppercased()).font(.maudeKicker(10)).tracking(1.2).foregroundStyle(accent)
            }
            Text(headline).font(.lato(15, .semibold)).foregroundStyle(MaudeTheme.ink)
                .fixedSize(horizontal: false, vertical: true)
            chart.frame(height: 88)
            Text(footer).font(.lato(11)).foregroundStyle(MaudeTheme.ink3)
                .fixedSize(horizontal: false, vertical: true)
        }
        .padding(16)
        ...
        GeometryReader { geo in
            ...
            ZStack(alignment: .topLeading) {
                Path { p in p.move(to: CGPoint(x: 0, y: y(labLevel, h))); p.addLine(to: CGPoint(x: w, y: y(labLevel, h))) }
                    .stroke(MaudeTheme.tirHigh, style: .init(lineWidth: 1.5, dash: [4, 3]))
                Text("lab HbA1c").font(.maudeMono(9)).foregroundStyle(MaudeTheme.tirHigh)
                ...
                .stroke(MaudeTheme.amber, style: .init(lineWidth: 2, lineCap: .round, lineJoin: .round))
                Text("your day-to-day").font(.maudeMono(9)).foregroundStyle(MaudeTheme.ink3)
        ...
        GeometryReader { geo in
            ...
            ZStack(alignment: .topLeading) {
                ForEach(strainDays, id: \.self) { d in
                    Rectangle().fill(MaudeTheme.accentFinance.opacity(0.18))
                ...
                .stroke(MaudeTheme.accentSleep, style: .init(lineWidth: 2, lineCap: .round, lineJoin: .round))
                Text("tighter-money days").font(.maudeMono(9)).foregroundStyle(MaudeTheme.accentFinance)
                    .position(x: 64, y: 10)
            }
        }
    }
            ...
            ZStack(alignment: .bottomLeading) {
                ForEach(0..<n, id: \.self) { i in
                    RoundedRectangle(cornerRadius: 4)
                        .fill(drinkNights.contains(i) ? MaudeTheme.amber : MaudeTheme.moss2)
                        .frame(width: bw, height: max(3, barH(rhr[i], h)))
                        .overlay(alignment: .top) {
                            if drinkNights.contains(i) {
                                Image(systemName: "fork.knife").font(.system(size: 8)).foregroundStyle(MaudeTheme.clayText)
                                    .offset(y: -12)
                            }
                        }
