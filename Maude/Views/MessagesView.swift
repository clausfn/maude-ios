...
struct CareUrgencyNote: View {
    ...
    var body: some View {
        ...
        .background(RoundedRectangle(cornerRadius: 14).fill(MaudeTheme.paper2.opacity(0.6)))
...
struct MessagesView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 0) {
                MaudeAppBar(title: "Care", showMark: false)
                ...
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    if appState.careConnect == nil && appState.careThreads.isEmpty {
                        emptyStateCard("Secure messaging with your care team becomes available once you're connected on the Maude network.")
                    } else {
                        scheduledSection
                        messagesSection
                    }
        ...
        #if DEBUG
        // Headless screenshot hooks — joins the MAUDE_* family:
        // MAUDE_TAB=care MAUDE_OPEN_CARE=<hero|plan|previsit|waiting|incoming|consult>
        .task {
            switch ProcessInfo.processInfo.environment["MAUDE_OPEN_CARE"] {
    ...
    private func liveHeroCard(_ c: ConsultSummary) -> some View {
            ...
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 3) {
                    ...
                    Text("\(c.recipientName) is ready for you")
                        .font(.maudeSerif(16.5)).kerning(-0.1)
                        .foregroundStyle(.white)
                }
                Spacer(minLength: 0)
            }
        ...
        .overlay(alignment: .topTrailing) {
            ...
            MaudeApertureMark(size: 130, reversed: true)
                .opacity(0.16)
                .offset(x: 34, y: -40)
    ...
    private var researchInviteCard: some View {
        Button { appState.showStudyConsent = true } label: {
            HStack(spacing: 12) {
                ZStack {
                    Circle().fill(MaudeTheme.amber2).frame(width: 38, height: 38)
                    Image(systemName: "bell.badge.fill").font(.system(size: 15)).foregroundStyle(MaudeTheme.clayText)
                }
                VStack(alignment: .leading, spacing: 2) {
                    Text("New research opportunity").font(.lato(14.5, .bold)).foregroundStyle(MaudeTheme.ink)
                    Text("\(appState.researchOpportunity?.name ?? "Study") · \(appState.researchOpportunity?.sponsor ?? "") · via Data for Good")
                        .font(.lato(12)).foregroundStyle(MaudeTheme.ink3).lineLimit(2).minimumScaleFactor(0.9)
                }
                Spacer()
                Image(systemName: "chevron.right").font(.system(size: 12, weight: .semibold)).foregroundStyle(MaudeTheme.ink4)
            }
            .padding(12)
            .background(MaudeTheme.paper2)
            .clipShape(RoundedRectangle(cornerRadius: 14))
            .overlay(RoundedRectangle(cornerRadius: 14).stroke(MaudeTheme.clay3, lineWidth: 1))
        }
        .buttonStyle(.plain)
    }
    ...
    private var planCard: some View {
        Button { showPlan = true } label: {
            HStack(spacing: 12) {
                ZStack {
                    Circle().fill(MaudeTheme.moss2).frame(width: 38, height: 38)
                    Image(systemName: "calendar.badge.plus").font(.system(size: 16)).foregroundStyle(MaudeTheme.moss)
                }
                VStack(alignment: .leading, spacing: 2) {
                    Text("Plan a consultation").font(.lato(14.5, .bold)).foregroundStyle(MaudeTheme.ink)
                    Text("Pick a time with \(careTeamName) — add it to your calendar")
                        .font(.lato(12)).foregroundStyle(MaudeTheme.ink3).lineLimit(1).minimumScaleFactor(0.9)
                }
                Spacer()
                Image(systemName: "chevron.right").font(.system(size: 12, weight: .semibold)).foregroundStyle(MaudeTheme.ink4)
            }
    ...
    @ViewBuilder private var scheduledSection: some View {
        if Config.videoConsultEnabled, appState.careConnect != nil {
            VStack(alignment: .leading, spacing: 8) {
                MaudeSectionHeader(label: "Scheduled")
    ...
    private func scheduledCard(_ sc: ScheduledConsult) -> some View {
                ...
                HStack(spacing: 6) {
                    Button {
                        ...
                        Text("Accept").font(.lato(11.5, .bold))
                            ...
                            .background(Capsule().fill(MaudeTheme.moss))
                    }.buttonStyle(.plain)
                    Button {
                        Task { @MainActor in
                        ...
                        Text("Decline").font(.lato(11.5, .bold))
                            ...
                            .overlay(Capsule().stroke(MaudeTheme.line2))
                    }.buttonStyle(.plain)
                }
        ...
        .overlay(RoundedRectangle(cornerRadius: 12).stroke(
            sc.status == "proposed:recipient" ? MaudeTheme.moss3 : MaudeTheme.line2, lineWidth: 0.5))
    }
    ...
    @ViewBuilder private var messagesSection: some View {
        MaudeSectionHeader(label: "Messages")
        if appState.careThreads.isEmpty {
            ...
            VStack(spacing: 0) {
                ForEach(Array(appState.careThreads.enumerated()), id: \.element.id) { idx, thread in
                    if idx > 0 {
                        Divider().background(MaudeTheme.line2).padding(.leading, 64)
                    }
                    NavigationLink {
    ...
    private func threadRow(_ t: CareThread) -> some View {
        HStack(spacing: 12) {
            ZStack {
                RoundedRectangle(cornerRadius: 12).fill(MaudeTheme.moss2).frame(width: 38, height: 38)
...
struct MessageThreadView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            MaudeAppBar(title: title, showMark: false, showsAvatar: false)
                .padding(.horizontal, 16)
            if let subtitle {
            ...
            ScrollViewReader { proxy in
                ScrollView {
                    LazyVStack(alignment: .leading, spacing: 8) {
                        if let error {
                            Text(error).font(.lato(12.5)).foregroundStyle(MaudeTheme.rust)
                                .padding(.vertical, 8)
                        }
                        if loadingThread && messages.isEmpty && error == nil {
                            HStack { Spacer(); ProgressView().tint(MaudeTheme.moss); Spacer() }
                                .padding(.vertical, 28)
    ...
    private func bubble(_ m: CareMessage) -> some View {
        HStack {
            ...
            Text(m.body)
                .font(.lato(14)).foregroundStyle(m.isMine ? .white : MaudeTheme.ink)
                .textSelection(.enabled)
                .padding(.horizontal, 12).padding(.vertical, 9)
                .background(RoundedRectangle(cornerRadius: 14)
                    .fill(m.isMine ? MaudeTheme.moss : MaudeTheme.paper2))
                .overlay(RoundedRectangle(cornerRadius: 14)
                    .stroke(m.isMine ? Color.clear : MaudeTheme.line2, lineWidth: 1))
            if !m.isMine { Spacer(minLength: 40) }
        }
    ...
    private var composer: some View {
        HStack(spacing: 10) {
            TextField("Message your care team…", text: $draft, axis: .vertical)
                .font(.lato(14))
                .foregroundStyle(MaudeTheme.ink)   // explicit: never follow system label colour
                .tint(MaudeTheme.moss)
                .focused($composerFocused)
                .padding(.horizontal, 12).padding(.vertical, 9)
                .background(RoundedRectangle(cornerRadius: 18).fill(MaudeTheme.paper2))
                .overlay(RoundedRectangle(cornerRadius: 18).stroke(MaudeTheme.line2, lineWidth: 1))
                .lineLimit(1...4)
