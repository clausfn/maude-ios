...
//   2) present an identity credential to Maude with SELECTIVE disclosure
...
//   4) control returns to Maude, "Verified with Partisia".
//
// DfG-branded on a fixed navy ground (constant in both app themes) so it reads as a
// distinct trusted authority — the DfG Wallet, not Maude — then hands back.
...
struct DfGWalletLoginView: View {
    ...
    /// UC-A on the first screen: issue the Maude Citizen credential from here
    ...
    init(onComplete: @escaping (String) -> Void, onCancel: @escaping () -> Void = {}) {
        ...
        switch ProcessInfo.processInfo.environment["MAUDE_WALLET_STEP"] {
    ...
    // Fixed DfG wallet palette (independent of the Maude app theme).
    ...
    var body: some View {
        ...
        .task {
            if ProcessInfo.processInfo.environment["MAUDE_WALLET_STEP"] == "verifying", step == .verifying, ref.isEmpty {
                runVerification()
            }
        }
        #endif
    ...
    private var footer: some View {
        HStack(spacing: 6) {
            ...
            Text("eIDAS 2.0 · secured by Partisia")
                .font(.maudeKicker(9.5)).tracking(0.6).foregroundStyle(sub)
        }
    }
    ...
    private var unlockStep: some View {
        VStack(spacing: 18) {
            ...
            VStack(spacing: 8) {
                ...
                Text("Sign in to Maude with the identity you control — nothing is revealed without your say-so.")
    ...
    private var presentStep: some View {
        VStack(spacing: 16) {
            VStack(spacing: 6) {
                Text("Maude is requesting").font(.maudeKicker(10)).tracking(1).foregroundStyle(sub)
            ...
            // proof that the Maude Citizen credential is stored in the wallet.
            card {
                rowHeader("FROM YOUR MY DFG WALLET", color: green)
                discloseRow("Maude Citizen credential", "Role + member ID — pseudonymous by design", on: true)
                ...
                if Config.walletIssuanceEnabled, appState?.sovereign != nil {
                    Button {
                        ...
                        HStack(spacing: 7) {
                            ...
                            Text("Not in your wallet yet? Get your Maude Citizen credential")
                                .font(.lato(11.5, .bold))
                        }
                        .foregroundStyle(green)
                    }
            ...
            card {
                ...
                HStack(alignment: .top, spacing: 10) {
                    ...
                    Text("Your name, date of birth and address are never revealed to Maude — only the facts above are proven.")
    ...
    private var verifyingStep: some View {
        VStack(spacing: 20) {
            ...
            VStack(alignment: .leading, spacing: 12) {
                progressRow("Requesting your Maude Citizen credential (OID4VP)", done: checkSignature)
    ...
    private var verifiedStep: some View {
        VStack(spacing: 16) {
            ...
            HStack(spacing: 8) {
                Text("CE REF").font(.maudeKicker(9)).tracking(1).foregroundStyle(sub)
                Text(ref).font(.maudeMono(12)).foregroundStyle(green)
            }
            .padding(.horizontal, 14).padding(.vertical, 9)
            .background(navy2).clipShape(Capsule())
            .overlay(Capsule().stroke(line, lineWidth: 1))

            primaryButton("Enter Maude", icon: "arrow.right") {
                onComplete(ref)
            }
        }
    }
    ...
    private func rowHeader(_ text: String, color: Color) -> some View {
        Text(text).font(.maudeKicker(9.5)).tracking(1).foregroundStyle(color)
    }
