...
// Source: _maude_design_reference/.../explorations/01-correlation-moment.html
import SwiftUI
...
struct ConfidenceChip: View {
    ...
    private var dot: Color {
        switch confidence {
        case .gated:    return MaudeTheme.moss
        case .emerging: return MaudeTheme.clay
        case .learning: return MaudeTheme.ink4
        }
    }
    private var bg: Color {
        switch confidence {
        case .gated:    return MaudeTheme.moss2
        case .emerging: return MaudeTheme.clay2
        case .learning: return MaudeTheme.paper
        }
    }
    private var fg: Color {
        switch confidence {
        case .gated:    return MaudeTheme.moss
        case .emerging: return MaudeTheme.clayText
        case .learning: return MaudeTheme.ink3
        }
    }
    ...
    var body: some View {
        ...
        .background(bg)
        .clipShape(Capsule())
        .overlay(Capsule().stroke(confidence == .learning ? MaudeTheme.line2 : .clear, lineWidth: 1))
    }
}
...
struct LeverCallout: View {
    ...
    var body: some View {
        HStack(alignment: .firstTextBaseline, spacing: 8) {
            Text("◆").font(.lato(12)).foregroundStyle(MaudeTheme.clay)
            Text(text)
...
struct SingleConfirmBar: View {
    ...
    var tint: Color = MaudeTheme.clay
    var valueText: Color = MaudeTheme.clayText

    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            Text(label.uppercased())
                .font(.maudeMono(9.5)).tracking(1)
                .foregroundStyle(MaudeTheme.ink3)

            HStack(spacing: 12) {
                GeometryReader { geo in
                    ZStack(alignment: .leading) {
                        RoundedRectangle(cornerRadius: 6).fill(MaudeTheme.line2)
                ...
                Text("\(Int((percent * 100).rounded()))%")
                    .font(.maudeMono(18)).foregroundStyle(valueText)
            }
            .padding(.top, 14)
...
struct BaselineProgressBar: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack {
                Text(label.uppercased())
                    .font(.maudeMono(9.5)).tracking(1).foregroundStyle(MaudeTheme.ink3)
                Spacer()
                Text("\(have) / \(need)")
                    .font(.maudeMono(9.5)).tracking(1).foregroundStyle(MaudeTheme.ink3)
            }
            GeometryReader { geo in
                ZStack(alignment: .leading) {
                    RoundedRectangle(cornerRadius: 6).fill(MaudeTheme.line2)
                    RoundedRectangle(cornerRadius: 6).fill(MaudeTheme.ink4)
...
struct ApertureArcGauge: View {
    ...
    var body: some View {
        GeometryReader { geo in
            ...
            let r = side / 2 - 16
            let dotTint = inRange ? MaudeTheme.moss : MaudeTheme.clay

            ZStack {
                // Track (open ring)
                arcPath(from: gap / 2, to: 2 * .pi - gap / 2, c: c, r: r)
                    .stroke(MaudeTheme.line, style: StrokeStyle(lineWidth: 13, lineCap: .round))
                // Your-normal band
                arcPath(from: theta(normalLow), to: theta(normalHigh), c: c, r: r)
                    .stroke(MaudeTheme.moss3, style: StrokeStyle(lineWidth: 13, lineCap: .round))
                // Today's marker
                Circle().fill(dotTint)
                    .frame(width: 18, height: 18)
                    .overlay(Circle().stroke(MaudeTheme.paper, lineWidth: 3.5))
