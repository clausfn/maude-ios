...
// EventKit write-only: Maude never reads the user's existing events.
import SwiftUI
import EventKit
...
struct PlanConsultView: View {
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                NavBackHeader(onBack: { dismiss() }) {
                    Text("Plan a consultation").font(.lato(15, .black)).foregroundStyle(MaudeTheme.ink)
                }
                .padding(.top, 6)
    ...
    private var picker: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            Text("When suits you for a video call?")
                .font(.maudeSerif(22)).kerning(-0.2).foregroundStyle(MaudeTheme.ink)
                .padding(.top, 5)
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 8) {
                    ForEach(days, id: \.self) { d in
                        ...
                        Button { day = d } label: {
                            VStack(spacing: 2) {
                                Text(d.formatted(.dateTime.weekday(.abbreviated))).font(.maudeKicker(9)).tracking(0.5)
                                Text(d.formatted(.dateTime.day())).font(.lato(15, .heavy))
                            }
                            .foregroundStyle(on ? MaudeTheme.invertFG : MaudeTheme.ink)
                            .frame(width: 46, height: 52)
                            .background(on ? MaudeTheme.invertBG : MaudeTheme.paper2)
                            .clipShape(RoundedRectangle(cornerRadius: 12))
                            .overlay(RoundedRectangle(cornerRadius: 12).stroke(on ? Color.clear : MaudeTheme.line, lineWidth: 1))
                        }
                        .buttonStyle(.plain)
                    }
                }
                ...
                ForEach(slots, id: \.self) { s in
                    ...
                    Button { if !busy { slot = s } } label: {
                        Text(s).font(.lato(14, .bold))
                            .strikethrough(busy, color: MaudeTheme.ink4)
                            .foregroundStyle(busy ? MaudeTheme.ink4 : (on ? .white : MaudeTheme.ink))
                            .padding(.horizontal, 14).padding(.vertical, 9)
                            .background(on ? MaudeTheme.moss : (busy ? MaudeTheme.line2.opacity(0.5) : MaudeTheme.paper2))
                            .clipShape(RoundedRectangle(cornerRadius: 11))
                            .overlay(RoundedRectangle(cornerRadius: 11).stroke(on ? Color.clear : MaudeTheme.line, lineWidth: 1))
                    }
                    .buttonStyle(.plain)
                    .disabled(busy)
                }
            ...
            Text("Suggested times — your clinician will confirm.")
                .font(.lato(11)).foregroundStyle(MaudeTheme.ink4).padding(.top, 5)
            ...
            VStack(alignment: .leading, spacing: 0) {
                HStack(spacing: 8) {
                    Image(systemName: "phone").font(.system(size: 13)).foregroundStyle(MaudeTheme.moss)
                    kicker("If the video drops")
                }
                TextField("Phone number (optional) — for fallback", text: $phone)
                    .keyboardType(.phonePad)
                    .font(.lato(13.5)).foregroundStyle(MaudeTheme.ink)
                    .padding(.horizontal, 14).padding(.vertical, 12)
                    .background(MaudeTheme.paper)
                    .clipShape(RoundedRectangle(cornerRadius: 11))
                    .overlay(RoundedRectangle(cornerRadius: 11).stroke(MaudeTheme.line, lineWidth: 1))
                    .padding(.top, 8)
                Text("\(recipientName) can call you if the connection fails. Stored on your device, shared only if the call drops.")
                    .font(.lato(11.5)).lineSpacing(2).foregroundStyle(MaudeTheme.ink3).padding(.top, 8)
            }
            .padding(14)
            .background(MaudeTheme.paper2)
            .clipShape(RoundedRectangle(cornerRadius: 13))
            .overlay(RoundedRectangle(cornerRadius: 13).stroke(MaudeTheme.line, lineWidth: 0.5))
            .padding(.top, 16)

            // Summary
            HStack(spacing: 8) {
                Image(systemName: "calendar").font(.system(size: 13)).foregroundStyle(MaudeTheme.moss)
                Text("\(startDate.formatted(.dateTime.weekday(.wide).day().month(.wide))) · \(slot)")
                    .font(.lato(14, .bold)).foregroundStyle(MaudeTheme.ink)
                Text("· \(duration) min").font(.lato(13)).foregroundStyle(MaudeTheme.ink3)
            }
            .padding(12)
            ...
            Button {
                ...
                Text(submitting ? "Requesting…" : "Request this consultation")
                    ...
                    .foregroundStyle(submitting ? MaudeTheme.primaryOffLabel : MaudeTheme.invertFG)
                    .frame(maxWidth: .infinity).padding(.vertical, 15)
                    .background(submitting ? MaudeTheme.primaryOffFill : MaudeTheme.invertBG)
                    .clipShape(RoundedRectangle(cornerRadius: 13))
            }
            .buttonStyle(.plain)
            .disabled(submitting)
    ...
    private var confirmation: some View {
        VStack(spacing: 0) {
            ZStack {
                Circle().fill(MaudeTheme.moss2).frame(width: 64, height: 64)
                Image(systemName: "checkmark").font(.system(size: 26, weight: .bold)).foregroundStyle(MaudeTheme.moss)
            }
            .padding(.top, 14)
            Text("Request sent.")
                .font(.maudeSerif(23)).kerning(-0.2).foregroundStyle(MaudeTheme.ink).padding(.top, 16)
            Text("\(recipientName) will confirm your \(startDate.formatted(.dateTime.weekday(.wide).day().month())) · \(slot) consultation. We'll notify you here.")
                ...
                .foregroundStyle(MaudeTheme.ink2).padding(.top, 10).padding(.horizontal, 8)

            if let error {
                Text(error).font(.lato(12.5)).foregroundStyle(MaudeTheme.rust).padding(.top, 10)
            }
            ...
            Button { addToCalendar() } label: {
                ...
                .foregroundStyle(calendarAdded ? MaudeTheme.moss : .white)
                .frame(maxWidth: .infinity).padding(.vertical, 14)
                .background(calendarAdded ? MaudeTheme.moss2 : MaudeTheme.moss)
                .clipShape(RoundedRectangle(cornerRadius: 13))
            }
            .buttonStyle(.plain)
            ...
            Button { showCheck = true } label: {
                ...
                .background(MaudeTheme.paper2)
                .clipShape(RoundedRectangle(cornerRadius: 13))
                .overlay(RoundedRectangle(cornerRadius: 13).stroke(MaudeTheme.line, lineWidth: 1))
            }
            .buttonStyle(.plain)
            .padding(.top, 10)

            Button { dismiss() } label: {
                Text("Done").font(.lato(14, .bold)).foregroundStyle(MaudeTheme.ink2)
    ...
    private func kicker(_ t: String) -> some View {
        Text(t.uppercased()).font(.maudeKicker(10)).tracking(1.2).foregroundStyle(MaudeTheme.ink3)
            .frame(maxWidth: .infinity, alignment: .leading)
    }
    ...
    private func addToCalendar() {
        ...
        let done: (Bool, Error?) -> Void = { granted, _ in
            DispatchQueue.main.async {
                ...
                ev.title = "Maude video consultation · \(recipientName)"
                // App deep link (Stage C): tapping the event reopens the consult in Maude.
                let deepLink = "maude://consult"
                ev.url = URL(string: deepLink)
                ev.notes = "Secure video consultation in Maude.\nOpen in the app: \(deepLink)"
