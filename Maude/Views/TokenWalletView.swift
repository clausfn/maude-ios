...
struct TokenWalletView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 0) {
                ...
                .background(MaudeTheme.paper2.opacity(0.7))
                .clipShape(RoundedRectangle(cornerRadius: 12))
                .overlay(RoundedRectangle(cornerRadius: 12).stroke(MaudeTheme.line, lineWidth: 1))
                .padding(.horizontal, 20)
                .padding(.top, 20)
        ...
        .task {
            ...
            if ProcessInfo.processInfo.environment["MAUDE_OPEN_DONATE"] == "1" { showDonate = true }
            if ProcessInfo.processInfo.environment["MAUDE_OPEN_TOKENHOW"] == "1" { showHow = true }
            #endif
        }
    }
    ...
    private var heroCard: some View {
        ZStack(alignment: .topTrailing) {
            ...
            MaudeApertureMark(size: 150, reversed: true)
                .opacity(0.18)
                .offset(x: 40, y: -50)

            VStack(alignment: .leading, spacing: 0) {
                Text("Your DfG tokens".uppercased())
                    .font(.maudeKicker(10)).tracking(1.4)
            ...
            if appState.tokenTransactions.isEmpty {
                ...
                VStack(spacing: 0) {
                    ForEach(Array(appState.tokenTransactions.enumerated()), id: \.element.id) { idx, tx in
                        if idx > 0 { Divider().background(MaudeTheme.line2).padding(.leading, 56) }
                        transactionRow(tx)
                    }
                }
                .background(MaudeTheme.paper2)
        ...
        return HStack(spacing: 12) {
            ZStack {
                RoundedRectangle(cornerRadius: 8)
                    .fill(positive ? MaudeTheme.moss2 : MaudeTheme.brass2)
                    .frame(width: 30, height: 30)
                Image(systemName: positive ? "plus" : "globe.europe.africa")
                    .font(.lato(13, .semibold))
                    .foregroundStyle(positive ? MaudeTheme.moss : MaudeTheme.brass)
            }
            ...
            Text(positive ? "+\(tx.amount)" : "−\(tx.amount)")
                .font(.maudeMono(14))
                .foregroundStyle(positive ? MaudeTheme.moss : MaudeTheme.ink2)
        }
...
private struct DonateSheet: View {
    ...
    private func causeCard(_ cause: TokenCause) -> some View {
        ...
        return HStack(spacing: 14) {
            ...
            if confirmedId == cause.id {
                ...
                Button {
                    ...
                    .padding(.vertical, 6)
                    .background(canAfford ? MaudeTheme.invertBG : MaudeTheme.primaryOffFill)
                    .foregroundStyle(canAfford ? MaudeTheme.invertFG : MaudeTheme.primaryOffLabel)
                    .cornerRadius(8)
                }
                .disabled(!canAfford)
            }
        ...
        .overlay(RoundedRectangle(cornerRadius: 12).stroke(
            confirmedId == cause.id ? MaudeTheme.moss3 : MaudeTheme.line2, lineWidth: 1))
    }
}
