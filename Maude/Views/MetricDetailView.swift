...
enum WellnessPillar: String, Hashable, CaseIterable {
    ...
    // the MAUDE_OPEN_PILLAR screenshot hook for free (rawValue-based).
...
struct MetricDetailView: View {
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    HStack(spacing: 6) {
                        Image(systemName: pillar.icon).font(.system(size: 12)).foregroundStyle(MaudeTheme.moss)
                        Text(pillar.title.uppercased()).font(.maudeKicker(10.5)).tracking(0.8)
    ...
    private func dayCurveCard(_ day: [Double]) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                Text("Today").font(.maudeKicker(9.5)).tracking(0.8).foregroundStyle(MaudeTheme.ink3)
                Spacer()
                HStack(spacing: 5) {
                    RoundedRectangle(cornerRadius: 2)
                        .fill((clinicalTIRZones ? MaudeTheme.tirTarget : MaudeTheme.moss).opacity(0.4))
                        .frame(width: 14, height: 9)
                    Text(clinicalTIRZones ? "TARGET 3.9–10.0" : "YOUR RANGE")
                        .font(.maudeKicker(8)).tracking(0.6).foregroundStyle(MaudeTheme.ink4)
                }
            }
            GlucoseCurveView(values: day, showsClinicalZones: clinicalTIRZones)
        }
        .padding(14).background(MaudeTheme.paper2)
    ...
    private var trendCard: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("This week").font(.maudeKicker(9.5)).tracking(0.8).foregroundStyle(MaudeTheme.ink3)
            ...
            switch pillar {
            case .sleep:
                DailyBarsChart(values: weekValues, tint: MaudeTheme.moss, xTicks: ticks,
                               unit: pillar.unit, daySlots: slots)
            case .glucose:
                DailyBarsChart(values: weekValues, tint: MaudeTheme.moss, xTicks: ticks, unit: pillar.unit,
                               goal: 70, goalLabel: "70% TARGET", daySlots: slots)
            default:   // recovery + heart today; Area-④ pillars never reach here
                AreaTrendChart(values: weekValues, tint: MaudeTheme.moss, xTicks: ticks,
                               unit: pillar.unit, daySlots: slots)
            }
        }
        .padding(14).background(MaudeTheme.paper2)
    ...
    private var stages: [Stage] {
        if let s = liveSleep, s.hasStageDetail {
            ...
            return [ Stage(name: "Deep",  mins: s.deepMin, color: MaudeTheme.moss),
                     Stage(name: "Light", mins: s.coreMin, color: MaudeTheme.moss3),
                     Stage(name: "REM",   mins: s.remMin,  color: MaudeTheme.amber) ]
        }
        return [ Stage(name: "Deep",  mins: 85,  color: MaudeTheme.moss),
                 Stage(name: "Light", mins: 241, color: MaudeTheme.moss3),
                 Stage(name: "REM",   mins: 110, color: MaudeTheme.amber),
                 Stage(name: "Awake", mins: 60,  color: MaudeTheme.line2) ]
    }
    ...
    private var sleepStagesCard: some View {
        ...
        return VStack(alignment: .leading, spacing: 12) {
            HStack {
                Text("TIME ASLEEP").font(.maudeKicker(9.5)).tracking(0.8).foregroundStyle(MaudeTheme.ink3)
                Spacer()
                Text(asleepText).font(.maudeMono(12)).foregroundStyle(MaudeTheme.ink3)
            }
            ...
            VStack(spacing: 6) {
                ForEach(stages, id: \.name) { s in
                    HStack(spacing: 8) {
                        ...
                        Text(s.name).font(.lato(13)).foregroundStyle(MaudeTheme.ink2)
                        Spacer()
