...
// Design ref: A7.2 "Morning/Evening Edition" (design_handoff_maude_a7 —
...
struct WalletView: View {
    ...
    @State private var toast: MaudeToastData?
    ...
    #if DEBUG
    // Screenshot hook only: opens the share FORM directly (MAUDE_OPEN_NEWSHARE
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 0) {
                ...
                // ── App bar ──
                MaudeAppBar(title: "Privacy", showMark: false)
                ...
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    // ── Your shares ──
                    MaudeSectionHeader(label: "Your shares")
                    ...
                    MaudeSectionHeader(label: "Research")
                    ...
                    VStack(spacing: 0) {
                        ...
                        Divider().background(MaudeTheme.line2).padding(.leading, 14)
                        researchToggleRow(
                    ...
                    // ── Your credential (UC-A — Maude Citizen into My DfG wallet) ──
                    // Sandbox-only rail: hidden on the production backend (per Kim).
                    if Config.walletIssuanceEnabled {
                        MaudeSectionHeader(label: "Your credential")

                        citizenCredentialRow
                    }

                    // ── DfG Tokens ──
                    MaudeSectionHeader(label: "DfG Tokens")

                    tokenEntryRow

                    // ── Consent record ──
                    MaudeSectionHeader(label: "Consent record")

                    ceSpineCard

                    // ── Recent events ──
                    MaudeSectionHeader(label: "Recent events", trailing: "Full log")
        ...
        .task {
            ...
            if ProcessInfo.processInfo.environment["MAUDE_DEMO_CITIZEN_CRED"] == "1", receiptOffer == nil {
            ...
            // Deterministic screenshot of the receipt flow (MAUDE_DEMO_RECEIPT=1):
            // a real offer for the first active grant, else a representative one.
            // MAUDE_RECEIPT_OFFER=1 implies it: that hook expands the demoted
            // wallet offer INSIDE the sheet, so it needs the sheet open to mean
            // anything — on its own it captured this screen unchanged.
            if ProcessInfo.processInfo.environment["MAUDE_DEMO_RECEIPT"] == "1"
                || ProcessInfo.processInfo.environment["MAUDE_RECEIPT_OFFER"] == "1",
               receiptOffer == nil {
            ...
            if ProcessInfo.processInfo.environment["MAUDE_OPEN_REACTIVATE"] == "1",
               !appState.withdrawnGrants.isEmpty {
                showReactivate = true
            }
            // Area ⑥ screenshot hooks (MAUDE_OPEN_WALLET family):
            // new-share sheet, research hub, token wallet.
            if ProcessInfo.processInfo.environment["MAUDE_OPEN_NEWSHARE"] == "1" { showNewShare = true }
            if ProcessInfo.processInfo.environment["MAUDE_OPEN_RESEARCH"] == "1" { pushResearch = true }
            if ProcessInfo.processInfo.environment["MAUDE_OPEN_TOKENS"]   == "1" { pushTokens = true }
            // MAUDE_SHARE_PHASE=preview|done — the share form's own hook only
            // fires once the form is on screen, so open it here (Privacy is the
            // headless entry point: MAUDE_TAB=privacy).
            if ProcessInfo.processInfo.environment["MAUDE_SHARE_PHASE"] != nil { debugShareForm = true }
            #endif
        }
    ...
    private var summaryCard: some View {
        ...
        return ZStack(alignment: .topTrailing) {
            ...
            MaudeApertureMark(size: 120, reversed: colorScheme == .light)
                .opacity(0.16)
                .offset(x: 18, y: -18)
            ...
            VStack(alignment: .leading, spacing: 0) {
                Text("Your shares".uppercased())
                    ...
                    .tracking(1.4)
                    .foregroundStyle(MaudeTheme.invertSub)
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            HStack(alignment: .center) {
                ...
                Text(grant.isActive ? "Active" : "Stopped")
                    ...
                    .padding(.vertical, 4)
                    .background(grant.isActive ? MaudeTheme.moss2 : MaudeTheme.line2)
                    .foregroundStyle(grant.isActive ? MaudeTheme.moss : MaudeTheme.ink2)
                    .clipShape(Capsule())
            }
            ...
            if !chips.isEmpty {
                FlexHStack(items: chips) { chip in
                    Text(chip)
                        ...
                        .lineLimit(1)
                        .fixedSize()
                        .overlay(RoundedRectangle(cornerRadius: 6).stroke(MaudeTheme.line2))
                }
                .padding(.top, 9)
            }
    ...
    /// UC-A — issue the citizen's own "Maude Citizen" credential into My DfG.
        ...
        Button {
            ...
            HStack(spacing: 14) {
                ...
                if issuingCitizenCred {
                    ProgressView().controlSize(.small).tint(MaudeTheme.moss)
                } else {
                    Text("Issue")
                        .font(.lato(12.5, .bold))
                        .foregroundStyle(MaudeTheme.moss)
        ...
        NavigationLink(destination: ConsentLedgerView()) {
            HStack(alignment: .top, spacing: 10) {
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    Text("The plain list of every choice you've made — backed by an independent record. Maude can read it; only you can change it.")
                        .font(.lato(12.5))
                        .lineSpacing(2.5)
            ...
            .background(MaudeTheme.moss2)
            .clipShape(RoundedRectangle(cornerRadius: 10))
            .overlay(RoundedRectangle(cornerRadius: 10).stroke(MaudeTheme.moss3, lineWidth: 1))
        }
        .buttonStyle(.plain)
        .padding(.bottom, 4)
    }
            ...
            if appState.walletEvents.isEmpty {
                ...
                ForEach(Array(appState.walletEvents.enumerated()), id: \.element.id) { idx, event in
                    ...
                    auditRow(dot: dotColor(event),
                             ...
                             titleColor: event.decision == .denied ? MaudeTheme.rust : MaudeTheme.ink)
                }
            }
        }
        .background(MaudeTheme.paper2)
    ...
    private func dotColor(_ event: WalletEvent) -> Color {
        ...
        case .approved: return MaudeTheme.moss
        case .denied:   return MaudeTheme.rust
        case .pending:  return MaudeTheme.ink4
        }
    }
