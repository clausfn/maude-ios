...
// toast, deltas, pills, chips. Pure presentation; reads MaudeTheme tokens only.
...
enum MaudeMotion {
    ...
    @MainActor static func reduced(_ system: Bool) -> Bool {
        system || UserDefaults.standard.bool(forKey: "maudeReduceMotion")
    }
}
...
struct RingView: View {
    ...
    var colors: [Color] = [MaudeTheme.moss, MaudeTheme.clay]
    var glow: Color = MaudeTheme.heroGlow
    ...
    private var shownValue: Double {
        MaudeMotion.reduced(systemReduceMotion) ? clamped : (shown ? clamped : 0)
    }

    var body: some View {
        ZStack {
            Circle().stroke(MaudeTheme.line2, lineWidth: lineWidth)
            Circle()
...
struct MiniRing: View {
    ...
    private var colors: [Color] {
        warn ? [MaudeTheme.clay, MaudeTheme.moss] : [MaudeTheme.moss, MaudeTheme.mossRev]
    }
...
enum DeltaTone { case good, bad, neutral
    var color: Color { switch self { case .good: MaudeTheme.moss; case .bad: MaudeTheme.rust; case .neutral: MaudeTheme.ink4 } }
}

struct StatusPill: View {
    var text: String
    var dot: Color? = nil
    var bg: Color = MaudeTheme.moss2
    var fg: Color = MaudeTheme.moss
    var body: some View {
        HStack(spacing: 6) {
...
struct GlucoseCurveView: View {
    ...
    @Environment(\.accessibilityReduceMotion) private var systemReduceMotion
    @State private var shown = false
    private var t: Double { MaudeMotion.reduced(systemReduceMotion) ? 1 : (shown ? 1 : 0) }
            ...
            VStack(spacing: 6) {
                ...
                HStack {
                    ForEach(Array(xTicks.enumerated()), id: \.offset) { idx, lab in
                        Text(lab).font(.maudeKicker(9)).foregroundStyle(MaudeTheme.ink4)
            ...
            ZStack(alignment: .topLeading) {
                    ...
                    Rectangle()
                        ...
                        .overlay(alignment: .top) {
                            Path { p in p.move(to: .zero); p.addLine(to: CGPoint(x: w, y: 0)) }
                                .stroke(MaudeTheme.moss.opacity(0.30), lineWidth: 0.5)
                                .offset(y: y(high, h))
                        }
                }
                if let targetLabel {
                ...
                if values.count > 1 {
                    ...
                    smoothedArea(pts, h).fill(LinearGradient(
                        colors: [MaudeTheme.clay.opacity(0.26), MaudeTheme.clay.opacity(0.0)],
                        startPoint: .top, endPoint: .bottom))
                    smoothedLine(pts).trim(from: 0, to: max(0.0001, t))
                        .stroke(MaudeTheme.clay, style: StrokeStyle(lineWidth: 2.2, lineCap: .round, lineJoin: .round))
                    ...
                    if redOutOfRange {
                        smoothedLine(pts).trim(from: 0, to: max(0.0001, t))
                            .stroke(MaudeTheme.clinRed, style: StrokeStyle(lineWidth: 2.2, lineCap: .round, lineJoin: .round))
                            .animation(.easeOut(duration: 1.0), value: t)
                    ...
                    if let peakLabel,
                       ...
                       values[maxIdx] > high {
                        ...
                        .stroke(MaudeTheme.clinRed.opacity(0.5), lineWidth: 1)
                        .opacity(t)
                        Circle().fill(MaudeTheme.clinRed).frame(width: 7, height: 7)
                            .position(peak).opacity(t)
                        Text(peakLabel)
                            .font(.maudeMono(10))
                            .foregroundStyle(MaudeTheme.clinRed)
                            .frame(width: w, alignment: .trailing)
                            .padding(.trailing, 2)
                            .offset(y: 1)
                            .opacity(t)
                    }
                    Circle().fill(MaudeTheme.clay).frame(width: 8, height: 8)
                        .overlay(Circle().stroke(MaudeTheme.paper2, lineWidth: 2))
...
struct MiniSparkline: View {
    var values: [Double]
    var tint: Color = MaudeTheme.moss
                ...
                ZStack(alignment: .topLeading) {
                    ...
                    if let g = goal {
                        ...
                        Path { p in p.move(to: CGPoint(x: 0, y: gy)); p.addLine(to: CGPoint(x: W, y: gy)) }
                            .stroke(MaudeTheme.ink4, style: StrokeStyle(lineWidth: 1, dash: [4, 4]))
            ...
            if !xTicks.isEmpty {
                HStack {
                    ForEach(Array(xTicks.enumerated()), id: \.offset) { _, t in
                        Text(t).font(.maudeKicker(8.5)).tracking(0.4)
                            .foregroundStyle(MaudeTheme.ink4)
                            .frame(maxWidth: .infinity)
                    }
                }
...
struct AreaTrendChart: View {
    ...
    private var yAxis: some View {
        VStack(alignment: .trailing) {
            ...
            Spacer()
            if hasBand {
                Text(axisLabel(mean)).foregroundStyle(MaudeTheme.moss).opacity(0.9)
                Spacer()
            }
...
struct MaudeToastData: Equatable {
    var title: String
    var detail: String
    var tone: DeltaTone = .good          // good = grant (moss dot), bad = withdraw (rust dot)
}

private struct MaudeToastView: View {
    let data: MaudeToastData
    var body: some View {
        HStack(spacing: 10) {
            Circle().fill(data.tone == .bad ? MaudeTheme.rust : MaudeTheme.moss)
                .frame(width: 8, height: 8)
            VStack(alignment: .leading, spacing: 2) {
                Text(data.title).font(.lato(13, .bold)).foregroundStyle(MaudeTheme.invertFG)
                Text(data.detail).font(.maudeMono(10)).foregroundStyle(MaudeTheme.invertSub)
            }
            Spacer(minLength: 8)
            Image(systemName: "checkmark").font(.system(size: 12, weight: .bold))
                .foregroundStyle(data.tone == .bad ? MaudeTheme.rust : MaudeTheme.moss)
        }
        .padding(.horizontal, 14).padding(.vertical, 12)
        .background(MaudeTheme.invertBG)
        .overlay(Capsule().stroke(MaudeTheme.invertLine, lineWidth: 1))
        .clipShape(Capsule())
        .shadow(color: MaudeTheme.cardShadow, radius: 14, y: 6)
...
extension View {
    ...
    /// auto-dismiss ~2.8s.
    func maudeToast(_ item: Binding<MaudeToastData?>) -> some View {
        overlay(alignment: .bottom) {
            if let data = item.wrappedValue {
                MaudeToastView(data: data)
    ...
    static func bands(yMin: Double, yMax: Double,
        ...
        [
            Band(lower: yMin, upper: veryLowCeiling,
                 color: MaudeTheme.tirVeryLow, pattern: .hatch, label: "VERY LOW"),
            Band(lower: veryLowCeiling, upper: low,
                 color: MaudeTheme.tirLow, pattern: .none, label: "LOW"),
            Band(lower: low, upper: high,
                 color: MaudeTheme.tirTarget, pattern: .none, label: "IN RANGE"),
            Band(lower: high, upper: veryHighFloor,
                 color: MaudeTheme.tirHigh, pattern: .none, label: "HIGH"),
            Band(lower: veryHighFloor, upper: yMax,
                 color: MaudeTheme.tirVeryHigh, pattern: .dots, label: "VERY HIGH"),
        ]
    }
...
struct IrisDayArc: View {
    ...
    var body: some View {
        ZStack {
            Circle().stroke(MaudeTheme.fjordBright.opacity(0.18), lineWidth: 4)
            Circle().trim(from: 0, to: max(0.001, p))
                .stroke(MaudeTheme.fjordBright, style: StrokeStyle(lineWidth: 4, lineCap: .round))
                .rotationEffect(.degrees(-90))
            Circle().inset(by: 8).stroke(MaudeTheme.ink.opacity(0.10), lineWidth: 3.5)
            Circle().inset(by: 8).trim(from: 0, to: max(0.001, p * 0.8))
                .stroke(MaudeTheme.ink.opacity(0.85), style: StrokeStyle(lineWidth: 3.5, lineCap: .round))
                .rotationEffect(.degrees(-90))
            Circle().inset(by: 16).trim(from: 0, to: 0.16)
                .stroke(MaudeTheme.amber, style: StrokeStyle(lineWidth: 3, lineCap: .round))
                .rotationEffect(.degrees(-70))
            Circle().fill(MaudeTheme.ink).frame(width: 6, height: 6)
        }
        .frame(width: size, height: size)
...
struct ScoreRing: View {
    ...
    var textColor: Color = MaudeTheme.ink
            ...
            GeometryReader { geo in
                ...
                if plotted.count > 1 {
                    ...
                    ZStack(alignment: .topLeading) {
                        ...
                        .stroke(MaudeTheme.ink3.opacity(0.55),
            ...
            if !labels.isEmpty {
                HStack {
                    ForEach(Array(labels.enumerated()), id: \.offset) { i, l in
                        if i > 0 { Spacer() }
                        Text(l).font(.lato(10)).foregroundStyle(MaudeTheme.ink3)
                    }
                }
            }
        }
