...
struct NudgeDetailView: View {
    ...
    private var reportAffordance: some View {
        ...
        .buttonStyle(.plain)
        .accessibilityHint(Text("Tell Maude's safety monitoring this insight felt wrong or unsafe"))
    }
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            if let ev, let pct = ev.chartPercent {
                HStack(alignment: .bottom, spacing: 16) {
                    splitBar(title: "Before", value: 1 - pct, color: MaudeTheme.moss, height: 56)
                    splitBar(title: "After", value: pct, color: MaudeTheme.clay, height: 120)
                }
                .frame(maxWidth: .infinity)
            }
    ...
    private func splitBar(title: String, value: Double, color: Color, height: CGFloat) -> some View {
        VStack(spacing: 6) {
            Text("\(Int((value * 100).rounded()))%")
                .font(.lato(22, .black)).foregroundStyle(color == MaudeTheme.clay ? MaudeTheme.clayText : MaudeTheme.moss)
            RoundedRectangle(cornerRadius: 8).fill(color)
                .frame(height: height)
            Text(title).font(.lato(12, .bold)).foregroundStyle(MaudeTheme.ink)
        }
        .frame(maxWidth: .infinity)
    }
    ...
    private var trendFallback: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text("Your trend".uppercased())
                ...
                .foregroundStyle(MaudeTheme.ink3)
            AreaTrendChart(values: [9.1, 8.2, 7.6, 6.9, 6.4, 6.0], tint: MaudeTheme.moss)
        }
        .padding(14)
        .background(MaudeTheme.paper2)
        .clipShape(RoundedRectangle(cornerRadius: 14))
    ...
    private func stillLearning(_ ev: NudgeEvidence) -> some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            Text(ev.headline ?? "We're still learning your baseline.")
                .font(.maudeSerif(23)).kerning(-0.2)
                .lineSpacing(2)
                .foregroundStyle(MaudeTheme.ink2)
                .fixedSize(horizontal: false, vertical: true)
            ...
            if let note = ev.learningNote {
                HStack(alignment: .firstTextBaseline, spacing: 8) {
                    Text("●").font(.system(size: 9)).foregroundStyle(MaudeTheme.moss)
                    Text(note)
