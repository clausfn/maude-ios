// ChatView.swift — Maude wellness-scope assistant UI. v01 2026-06-09 ·
// v02 2026-08-12 (A7.2 Area ⑨: "Ask Maude" chrome from b-learn.jsx AIChat —
...
struct ChatView: View {
    ...
    var body: some View {
        ...
        #if DEBUG
        .task {
            // Snapshot hook: auto-send one seed question (MAUDE_CHAT_SEED).
            if let seed = ProcessInfo.processInfo.environment["MAUDE_CHAT_SEED"],
    ...
    // MARK: - Header ("Ask Maude" + On-device chip; reuses brand, not the system nav bar)

    private var header: some View {
        HStack(spacing: 10) {
            MaudeApertureMark(size: 22)
                .frame(width: 32, height: 32)
                .background(RoundedRectangle(cornerRadius: 9).fill(MaudeTheme.paper2))
                .overlay(RoundedRectangle(cornerRadius: 9).stroke(MaudeTheme.line, lineWidth: 1))
            Text("Ask Maude").font(.maudeSerif(17)).kerning(-0.3).foregroundStyle(MaudeTheme.ink)
            Spacer()
            OnDeviceChip()
            if consentProcess {
                Menu {
                    ...
                    Image(systemName: "ellipsis.circle").font(.lato(17)).foregroundStyle(MaudeTheme.ink3)
                }
            }
            Button { dismiss() } label: {
                Image(systemName: "xmark").font(.lato(15, .bold)).foregroundStyle(MaudeTheme.ink3)
            }
        }
    ...
    private var aiLabelBar: some View {
        HStack(spacing: 7) {
            Image(systemName: "sparkles").font(.system(size: 11)).foregroundStyle(MaudeTheme.moss)
            Text(MaudeChatCopy.aiLabel).font(.maudeKicker(10)).tracking(0.4).foregroundStyle(MaudeTheme.ink3)
            Spacer()
        }
        .padding(.horizontal, 20).padding(.vertical, 8)
        .background(MaudeTheme.moss2)
        .overlay(alignment: .bottom) { Rectangle().fill(MaudeTheme.line).frame(height: 0.5) }
    }

    // MARK: - Consent (standalone; the rest of Maude works without this)

    private var consentBody: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                Text(MaudeChatCopy.intendedPurpose)
                    .font(.lato(15)).lineSpacing(3).foregroundStyle(MaudeTheme.ink2)
                    .padding(.top, 8)
                ...
                HStack(spacing: 8) {
                    Image(systemName: "info.circle").font(.caption).foregroundStyle(MaudeTheme.ink4)
                    Text("The rest of Maude works without enabling the assistant. You can turn it off anytime, which deletes the conversation.")
                        .font(.lato(12.5)).foregroundStyle(MaudeTheme.ink4).lineSpacing(2)
                }
                .padding(.top, 4)
            }
    ...
    private func consentToggle(title: String, detail: String, isOn: Binding<Bool>, enabled: Bool) -> some View {
        VStack(alignment: .leading, spacing: 6) {
            Toggle(isOn: isOn) {
                Text(title).font(.lato(14.5, .bold)).foregroundStyle(enabled ? MaudeTheme.ink : MaudeTheme.ink4)
            }
            .tint(MaudeTheme.moss)
            .disabled(!enabled)
            Text(detail).font(.lato(12.5)).lineSpacing(2).foregroundStyle(MaudeTheme.ink3)
        }
        .padding(14)
            ...
            ScrollViewReader { proxy in
                ScrollView {
                    LazyVStack(alignment: .leading, spacing: 10) {
                        ...
                        if thinking {
                            Text("…").font(.lato(14)).foregroundStyle(MaudeTheme.ink3)
                                .padding(.horizontal, 12).padding(.vertical, 9)
                                .background(RoundedRectangle(cornerRadius: 14).fill(MaudeTheme.paper2))
                                .id("thinking")
                        }
                    }
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            if !hasSummaryData {
                HStack(alignment: .top, spacing: 7) {
                    Image(systemName: "heart.text.square")
                        .font(.lato(13)).foregroundStyle(MaudeTheme.moss)
                    Text("Maude is still learning your normal — your readings are only starting to come in. You can ask how it works, or connect Apple Health in Settings to bring in your own numbers.")
            ...
            ForEach(starterQuestions, id: \.self) { ex in
                Button { draft = ex; send() } label: {
                    Text(ex).font(.lato(13)).foregroundStyle(MaudeTheme.moss)
                        .padding(.horizontal, 12).padding(.vertical, 9)
                        .frame(maxWidth: .infinity, alignment: .leading)
                        .background(MaudeTheme.moss2)
                        .clipShape(RoundedRectangle(cornerRadius: 10))
    ...
    private func assistantCard(_ m: ChatMessage) -> some View {
            ...
            if m.intent == .explainHRVDrop, let d = engine.summary.hrvDetail,
               d.weekSeries.count >= 2 {
                ChatBaselineSpark(values: d.weekSeries,
                                  band: d.usualBand,
                                  tint: MaudeTheme.accentRecovery)
                    .frame(height: 54)
                    .padding(.top, 10)
            }
        ...
        .shadow(color: MaudeTheme.cardShadow.opacity(0.5), radius: 8, y: 4)
    }
    ...
    private func proofText(for m: ChatMessage) -> String {
        if m.isSafetyLine {
            return m.origin == .onDevice
                ? "Answered on this iPhone · Maude doesn't diagnose, treat, or advise on medication"
                : "Maude doesn't diagnose, treat, or advise on medication"
        }
    ...
    private var composer: some View {
        HStack(spacing: 10) {
            TextField("Ask about your data…", text: $draft, axis: .vertical)
                ...
                .padding(.horizontal, 12).padding(.vertical, 9)
                .background(RoundedRectangle(cornerRadius: 18).fill(MaudeTheme.paper2))
                .overlay(RoundedRectangle(cornerRadius: 18).stroke(MaudeTheme.line, lineWidth: 1))
            Button { send() } label: {
                Image(systemName: "arrow.up").font(.lato(16, .bold)).foregroundStyle(MaudeTheme.invertFG)
                    .frame(width: 38, height: 38)
                    .background(Circle().fill(canSend ? MaudeTheme.moss : MaudeTheme.line))
            }
            .buttonStyle(.plain).disabled(!canSend)
        }
        .padding(.horizontal, 16).padding(.vertical, 10)
        .background(MaudeTheme.paper)
        .overlay(alignment: .top) { Rectangle().fill(MaudeTheme.line).frame(height: 0.5) }
    }
