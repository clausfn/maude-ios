...
// MaudeApp presents it, then this. On completion the caller sets ALL the
...
//
// DEBUG env hook (snapshot family): MAUDE_ONB_STEP=<0..10|declined|lock|literacy>
...
enum OnbFrame: String, CaseIterable {
    ...
    var accent: Color {
        switch self {
        case .health, .healthDeclined: return MaudeTheme.accentGlucose
        case .name:                    return MaudeTheme.accentSleep
        case .passport:                return MaudeTheme.accentRecovery
        case .sharing:                 return MaudeTheme.brass
        case .backup:                  return MaudeTheme.accentHeart
        default:                       return MaudeTheme.moss
        }
    }

    #if DEBUG
    /// MAUDE_ONB_STEP values → frames (0..10 = the 11 handoff frames).
...
struct OnboardingFlowView: View {
    ...
    @AppStorage("maudeReduceMotion") private var appReduceMotion = false
    ...
    private func segmentColor(_ s: Int) -> Color {
        guard let slot = frame.progressSlot else { return MaudeTheme.line }
        if s < slot { return MaudeTheme.moss }
        if s == slot { return frame.accent }
        return MaudeTheme.ink.opacity(0.14)
    }
    ...
    private var stepContent: some View {
        ...
        case .cover, .ready:
            EmptyView()
        case .why:
            WhyMaudeStep(accent: frame.accent, onContinue: { advance() })
        case .signIn:
    ...
    private var coverFrame: some View {
        OnboardingCover {
            VStack(alignment: .leading, spacing: 0) {
                ...
                Text("Maude reads the health data you already have, finds patterns in *your own* normal, and keeps every reading on this device.")
                    .font(.lato(14.5))
                    .lineSpacing(4)
    ...
    private var readyContent: some View {
                VStack(spacing: 0) {
                    ...
                    Text(name.map { "You're set, \($0)." } ?? "You're set.")
                        .font(.maudeSerif(29, .bold, relativeTo: .title1))
                        .foregroundStyle(.white)
                        .multilineTextAlignment(.center)
                        .padding(.top, 26)
                    ...
                    VStack(alignment: .leading, spacing: 7) {
                        ...
                        Text("Because your data never leaves your control, Maude can be a guardian that goes further than Apple or any cloud is allowed to — reading everything together to look out for you, while you alone decide what's ever shared.")
                            .font(.lato(13))
                            .lineSpacing(4)
