...
//  • AppLockSetupView — the onboarding offer ("Lock Maude to your face."),
...
//    its own system passcode fallback. MaudeApp overlays it on scenePhase
...
struct AppLockSetupView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            Text("Your health lives on this phone. Maude asks for Face ID each time it wakes, and falls back to your device passcode — so keep one set. No passcode leaves the device.")
                .font(.lato(14))
                .lineSpacing(4)
...
/// The real lock still runs on background→active (see MaudeApp).
...
struct AppLockScreen: View {
    ...
    private func attempt() {
        ...
        context.evaluatePolicy(.deviceOwnerAuthentication,
                               localizedReason: String(localized: "Unlock Maude.")) { success, _ in
            Task { @MainActor in
                inFlight = false
