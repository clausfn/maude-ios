...
struct AuthView: View {
    ...
    #if DEBUG
    /// Screenshot hook: open the DfG wallet flow immediately (MAUDE_OPEN_DFG=1).
    private var autoOpenDfG: Bool { ProcessInfo.processInfo.environment["MAUDE_OPEN_DFG"] == "1" }
    /// Screenshot hook: open the email surface (MAUDE_OPEN_EMAIL=create|signin).
    private var autoOpenEmail: String? { ProcessInfo.processInfo.environment["MAUDE_OPEN_EMAIL"] }
    #endif
    ...
    private var header: some View {
        VStack(spacing: 0) {
            ...
            Text("A free account — for you first.")
                .font(.maudeSerif(26, .bold, relativeTo: .title2))
            ...
            Text("Maude is yours to use alone, free. An account — held by the non-profit Data for Good Foundation — only stores your name and email, and unlocks a few things when you want them.")
                .font(.lato(13.5))
                .lineSpacing(4)
    ...
    private var signInStack: some View {
        VStack(spacing: 10) {
            ...
            Button {
                ...
                Text(Config.authEnabled ? "Try it without an account →" : "Enter Maude")
    ...
    private var walletGrid: some View {
        VStack(spacing: 9) {
            Text("OR USE AN IDENTITY WALLET")
                .font(.maudeKicker(9)).tracking(1.4).foregroundStyle(MaudeTheme.ink4)
                .padding(.top, 2)
            HStack(spacing: 9) {
    ...
    private func walletCell(logo: String?, tileColor: Color? = nil, swatch: Color?, name: String, _ action: @escaping () -> Void) -> some View {
        Button(action: action) {
            HStack(spacing: 8) {
                ...
                Text(name).font(.lato(14, .bold)).foregroundStyle(MaudeTheme.ink)
        ...
        VStack(spacing: 9) {
            ...
            if emailMode == .create {
                ...
                Text(String(localized: "At least 6 characters."))
                    ...
                    .foregroundStyle(password.isEmpty || password.count >= 6
                                     ? MaudeTheme.ink4 : MaudeTheme.clayText)
                    .frame(maxWidth: .infinity, alignment: .leading)
                    .padding(.leading, 4)
            }
            Button {
            ...
            Button {
                ...
                Text(emailMode == .create
                     ...
                     : String(localized: "New to Maude? Create an account"))
                    .font(.lato(13, .bold))
                    .foregroundStyle(MaudeTheme.moss)
                    .frame(maxWidth: .infinity)
                    .padding(.vertical, 6)
            ...
            VStack(spacing: 16) {
                ...
                VStack(spacing: 10) {
                    ...
                    Text(String(localized: "Set a new password"))
                        .font(.maudeSerif(22))
                        .foregroundStyle(MaudeTheme.ink)
                    Text(String(localized: "Choose a new password for your Maude account."))
                ...
                if let hint {
                    Text(hint.text)
                        .font(.lato(11.5))
                        .foregroundStyle(hint.warn ? MaudeTheme.clayText : MaudeTheme.ink4)
                        .frame(maxWidth: .infinity, alignment: .leading)
                        .padding(.leading, 4)
                }
                if let error {
                ...
                Button {
                    ...
                    Group {
                        if isSaving {
                            ProgressView().tint(MaudeTheme.invertFG)
                        } else {
