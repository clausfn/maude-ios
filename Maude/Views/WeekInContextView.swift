...
struct WeekInContextView: View {
    ...
    // this path — see `Maude/Context/CalendarLoad.swift`.
    //
    ...
    private func fill(_ level: CorrelationLevel) -> Color {
        if gradedHeatmap {
            ...
            switch level {
            case .noData:  return MaudeTheme.gridEmpty
            case .low:     return MaudeTheme.moss2      // just like your usual
            case .medium:  return MaudeTheme.devMed     // a little off
            case .high:    return MaudeTheme.devHigh    // clearly off your usual
            case .outlier: return MaudeTheme.devOutlier // worth noticing (+ ring)
            }
        }
        switch level {
        case .noData:  return MaudeTheme.gridEmpty
        case .low:     return MaudeTheme.moss2     // in range
        case .medium:  return MaudeTheme.moss3     // in range (firmer)
        case .high:    return MaudeTheme.clay2     // mild
        case .outlier: return MaudeTheme.clay      // worth noticing
        }
    }
    ...
    private func neutralFill(_ level: CorrelationLevel) -> Color {
        level == .noData ? MaudeTheme.gridEmpty : MaudeTheme.accentFinance.opacity(0.16)
    }
    ...
    private var weekScroll: some View {
        ScrollView(.vertical, showsIndicators: false) {
            VStack(alignment: .leading, spacing: 0) {
                ...
                MaudeAppBar(title: "Insights", showMark: false)
                ...
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    // 2. Section header
                    MaudeSectionHeader(label: "7 days in context")
                        .padding(.horizontal, 20)
                    ...
                    MaudeSectionHeader(label: "This week")
                        .padding(.horizontal, 20)
                    ...
                    MaudeSectionHeader(label: "Your data, in depth")
                        .padding(.horizontal, 20)
                        .padding(.top, 10)

                    VStack(spacing: 0) {
                        depthRow(pillar: .activity)
                        Divider().background(MaudeTheme.line2).padding(.leading, 44)
                    ...
                    if crossSourceCards {
                        MaudeSectionHeader(label: "Cross-source patterns")
                            .padding(.horizontal, 20)
                            .padding(.top, 10)
                        CrossSourcePatterns()
                            .padding(.horizontal, 16)
        ...
        .maudeScrollEdgeSoft()   // iOS 26 + flag: chrome dissolves into the trend feed
    ...
    private var dayScrubEntry: some View {
        HStack(spacing: 12) {
            Image(systemName: "hand.draw").font(.system(size: 18)).foregroundStyle(MaudeTheme.moss)
            VStack(alignment: .leading, spacing: 2) {
                Text("Replay your day").font(.lato(15, .semibold)).foregroundStyle(MaudeTheme.ink)
                Text("Drag the line to relive any moment of today")
                    .font(.lato(12)).foregroundStyle(MaudeTheme.ink3)
            }
            Spacer()
            Image(systemName: "chevron.right").font(.system(size: 13, weight: .semibold)).foregroundStyle(MaudeTheme.ink4)
        }
        .padding(14)
        .background(MaudeTheme.paper2)
        ...
        VStack(alignment: .leading, spacing: 10) {
            HStack {
                Text("GLUCOSE · TIME IN RANGE")
                    .font(.maudeKicker(10.5)).tracking(0.8)
                    .foregroundStyle(MaudeTheme.ink3)
                Spacer()
                if hasRealTIR {
                    Text(tirHeadline).font(.maudeMono(15)).foregroundStyle(MaudeTheme.ink)
                }
            }
            if hasRealTIR {
                let slots = weekSlots(.inRange)
                AreaTrendChart(values: tirValues, tint: MaudeTheme.moss,
    ...
    private var recoveryCard: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack {
                HStack(spacing: 5) {
                    Image(systemName: "waveform.path.ecg").font(.system(size: 11)).foregroundStyle(MaudeTheme.moss)
                ...
                Spacer()
                if hasRealHRV {
                    Text(hrvHeadline).font(.maudeMono(15)).foregroundStyle(MaudeTheme.ink)
                }
            }
            if hasRealHRV {
                let slots = weekSlots(.hrv)
                AreaTrendChart(values: hrvValues, tint: MaudeTheme.moss,
    ...
    private var correlationGridCard: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 0) {
                ...
                ForEach(Array(week.days.enumerated()), id: \.offset) { dayIndex, day in
                    Text(day.localizedDayLetter)
                        .font(.maudeKicker(9))
                        .tracking(0.5)
                        .foregroundStyle(markedDays.contains(dayIndex) ? MaudeTheme.accentFinance
                                         : (dayIndex == clusterDay ? MaudeTheme.clayText
                                            : (selected?.day == dayIndex ? MaudeTheme.ink : MaudeTheme.ink3)))
                        .frame(maxWidth: .infinity)
                }
            }
                ...
                ForEach(Array(metricLabels.enumerated()), id: \.offset) { rowIndex, label in
                    HStack(spacing: 0) {
                        ...
                        Text(label)
                            .font(.maudeKicker(8))
                            .tracking(0.4)
                            .foregroundStyle(selected?.metric == rowIndex ? MaudeTheme.ink2 : MaudeTheme.ink4)
            ...
            if calendarState != .ready {
                HStack(alignment: .top, spacing: 6) {
                    Image(systemName: "calendar")
                        .font(.system(size: 10)).foregroundStyle(MaudeTheme.ink4)
            ...
            Divider().overlay(MaudeTheme.line).padding(.top, 4)
            gridReadout
        }
        .padding(.horizontal, 16)
        .padding(.vertical, 14)
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 12)
                .stroke(MaudeTheme.line2, lineWidth: 1)
        )
        .shadow(color: MaudeTheme.cardShadow, radius: 8, y: 2)
    }
                ...
                HStack(spacing: 8) {
                    ...
                    if let kind {
                        StatusPill(text: kind.label,
                                   dot: MaudeTheme.accentFinance,
                                   bg: MaudeTheme.accentFinance.opacity(0.12),
                                   fg: MaudeTheme.accentFinance)
                    } else {
                        StatusPill(text: level.accessibilityLabel.capitalized,
                                   dot: fill(level),
                                   bg: level == .outlier ? MaudeTheme.clay2 : MaudeTheme.moss2,
                                   fg: level == .outlier ? MaudeTheme.clay : MaudeTheme.moss)
                    }
                }
                Text(readoutText(sel: sel, level: level, dayName: dayName, kind: kind))
                    .font(.lato(13)).lineSpacing(2).foregroundStyle(MaudeTheme.ink2)
            }
            .transition(.opacity)
        } else {
            Text("Tap any square to read that day's signal.")
                .font(.lato(13)).foregroundStyle(MaudeTheme.ink3)
                .padding(.top, 2)
        }
    }
    ...
    private func readoutText(sel: SelectedCell, level: CorrelationLevel,
        ...
        if sel.metric == CorrelationDeriver.calendarIndex {
            ...
            return base + " " + String(localized: "You marked that day, so Maude isn't reading it as a drift from your usual.")
        }
        return kind == nil
    ...
    private func markedReadout(metric: Int, level: CorrelationLevel, day: String) -> String {
        ...
        return String(localized: "\(day)'s \(m) was recorded and kept. You marked that day, so Maude isn't reading it as a drift from your usual.")
    }
    ...
    private var gridLegend: some View {
        HStack(spacing: 12) {
            ...
            if !markedDays.isEmpty {
                LegendSwatch(color: MaudeTheme.accentFinance.opacity(0.16),
    ...
    private var regulatoryNoteCard: some View {
        HStack(alignment: .top, spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                Text("Maude describes patterns in your own data. It does not diagnose or treat.")
                    .font(.caption)
                    .foregroundStyle(MaudeTheme.ink2)
                    .lineSpacing(1.5)
                    .fixedSize(horizontal: false, vertical: true)
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 10)
                .stroke(MaudeTheme.clay.opacity(0.5), lineWidth: 0.5)
        )
    }
    ...
    private func weeklyCard(series: [Double], unit: String, label: String,
        ...
        if series.count >= 2, let first = series.first, let last = series.last {
            ...
            WeeklyMetricCard(value: value(avg), unit: unit, label: label,
                             deltaLabel: delta(d),
                             deltaColor: d >= 0 ? MaudeTheme.moss : MaudeTheme.ink2)
        } else {
            WeeklyMetricCard(value: "—", unit: "", label: label,
                             deltaLabel: "No data yet",
                             deltaColor: MaudeTheme.ink2)
        }
    }
    ...
    var body: some View {
        RoundedRectangle(cornerRadius: 6)
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 6)
                    .inset(by: -2)
                    .stroke(selected ? MaudeTheme.ink
                            : (cluster ? MaudeTheme.clay.opacity(0.55) : .clear),
