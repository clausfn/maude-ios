...
//   `MaudeMotion.reduced(_:)`. Honour the `liquidGlass` flag at the call site.
import SwiftUI
...
struct GlassScrubberThumb: View {
    ...
    private var reduceMotion: Bool { MaudeMotion.reduced(reduceMotionSystem) }
    ...
    var body: some View {
        ...
        let content = HStack(spacing: 7) {
            Circle().fill(inRange ? MaudeTheme.moss : MaudeTheme.clay).frame(width: 7, height: 7)
            Text(label).font(.maudeMono(12)).foregroundStyle(MaudeTheme.ink)
        }
        ...
        return Group {
            if solid {
                content
                    .background(shape.fill(MaudeTheme.paper2))
                    .overlay(shape.strokeBorder(inRange ? MaudeTheme.moss : MaudeTheme.ink3, lineWidth: 1))
            } else if glassOn, #available(iOS 26, *) {
                content
                    .glassEffect(inRange
                        ? .regular.tint(MaudeTheme.moss.opacity(0.5)).interactive()
                        : .regular.interactive(), in: shape)
                    .glassEffectID("scrubber", in: namespace)
            } else {
                content
                    .background(shape.fill(.ultraThinMaterial))
                    .overlay(shape.strokeBorder(MaudeTheme.line, lineWidth: 0.5))
                    .shadow(color: MaudeTheme.cardShadow, radius: 10, y: 3)
            }
        }
...
struct ScrubReadout: View {
    ...
    var body: some View {
        HStack(alignment: .firstTextBaseline, spacing: 6) {
            Text(value).font(.maudeMono(22)).foregroundStyle(MaudeTheme.ink)
            Text(unit).font(.maudeMono(11)).foregroundStyle(MaudeTheme.ink3)
            Spacer()
            HStack(spacing: 6) {
                ...
                Circle().fill(inRange ? MaudeTheme.moss : MaudeTheme.clay).frame(width: 6, height: 6)
                Text(inRange ? "in your range" : "worth noticing")
                    .font(.lato(11, .semibold))
                    .foregroundStyle(inRange ? MaudeTheme.moss : MaudeTheme.clayText)
                Text("· \(time)").font(.maudeMono(11)).foregroundStyle(MaudeTheme.ink3)
            }
        }
        .padding(14)
        ...
        VStack(spacing: 12) {
            ...
            GeometryReader { geo in
                ...
                ZStack(alignment: .topLeading) {
                    ...
                    Rectangle().fill(MaudeTheme.line).frame(width: 1, height: h).offset(x: progress * w)
    ...
    private func trace(in size: CGSize) -> some View {
        ...
        return ZStack {
            Path { p in p.move(to: pt(0)); for i in 1..<samples.count { p.addLine(to: pt(i)) } }
                .stroke(MaudeTheme.moss.opacity(0.85), style: .init(lineWidth: 2, lineCap: .round, lineJoin: .round))
            Path { p in
            ...
            .fill(LinearGradient(colors: [MaudeTheme.moss.opacity(0.18), .clear], startPoint: .top, endPoint: .bottom))
        }
    }
}
...
struct MorphActionCluster: View {
    ...
    private var solid: Bool { reduceTransparency || contrast == .increased }
    private var reduceMotion: Bool { MaudeMotion.reduced(systemReduceMotion) }
    ...
    @ViewBuilder private func action(_ title: String, _ icon: String, id: String,
        ...
        Button { run(); withAnimation(anim) { expanded = false } } label: {
            Label(title, systemImage: icon)
                ...
                .foregroundStyle(prominent ? MaudeTheme.invertFG : MaudeTheme.ink)
...
private struct GlassPill: ViewModifier {
    ...
    func body(content: Content) -> some View {
        ...
        return Group {
            if solid {
                content
                    .background(shape.fill(prominent ? MaudeTheme.moss : MaudeTheme.paper2))
                    .overlay(shape.strokeBorder(prominent ? MaudeTheme.moss : MaudeTheme.ink3, lineWidth: 1))
            } else if glassOn, #available(iOS 26, *) {
                content
                    .glassEffect(prominent ? .regular.tint(MaudeTheme.moss) : .regular, in: shape)
                    .glassEffectID(id, in: ns)
                    .glassEffectTransition(.matchedGeometry)
            } else {
                content
                    .background(shape.fill(prominent ? AnyShapeStyle(MaudeTheme.moss) : AnyShapeStyle(.ultraThinMaterial)))
                    .overlay(shape.strokeBorder(MaudeTheme.line, lineWidth: 0.5))
                    .shadow(color: MaudeTheme.cardShadow, radius: 8, y: 2)
            }
        }
...
struct TidelineField: View {
    ...
    private var still: Bool { MaudeMotion.reduced(systemReduceMotion) }
                ...
                Canvas { ctx, size in
                    ...
                    ctx.fill(Path(CGRect(origin: .zero, size: size)), with: .linearGradient(
                        Gradient(stops: [
                            ...
                            .init(color: MaudeTheme.heroGlow.opacity(a), location: max(0, tideY / h - 0.2)),
                            .init(color: MaudeTheme.moss.opacity(a * 0.7), location: 1),
                        ]),
                    ...
                    let tones = [MaudeTheme.heroGlow, MaudeTheme.moss, MaudeTheme.amber, MaudeTheme.heroGlow]
                    for i in 0..<4 {
...
struct DaySummaryGlass<Content: View>: View {
    ...
    @Environment(\.colorSchemeContrast) private var contrast
    private var opaqueFallback: Bool { reduceTransparency || contrast == .increased }
    private let radius = MaudeTheme.Radius.hero
        ...
        return Group {
            if opaqueFallback {
                inner
                    ...
                    .overlay(RoundedRectangle(cornerRadius: radius).stroke(MaudeTheme.line, lineWidth: 0.5))
            } else if glassOn, #available(iOS 26, *) {
...
struct ConcentricGlassCard<Content: View>: View {
    ...
    var body: some View {
        ...
        return Group {
            if #available(iOS 26, *) {
                inner
                    ...
                    .overlay(ConcentricRectangle(corners: .concentric).stroke(MaudeTheme.line, lineWidth: 0.5))
            } else {
                inner
                    .clipShape(RoundedRectangle(cornerRadius: MaudeTheme.Radius.card))
                    .overlay(RoundedRectangle(cornerRadius: MaudeTheme.Radius.card).stroke(MaudeTheme.line, lineWidth: 0.5))
        ...
        .shadow(color: solid ? .clear : MaudeTheme.cardShadow, radius: solid ? 0 : 8, y: solid ? 0 : 2)
    }
}
