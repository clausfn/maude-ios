...
//
// ANATOMY (benchmark: Apple Health sleep, done in Maude's own language):
...
struct SleepDetailView: View {
    ...
    private func hero(_ m: Model) -> some View {
        MetricHero(tint: MaudeTheme.accentSleep,
    ...
    private func figuresRow(_ m: Model) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            HStack(alignment: .firstTextBaseline, spacing: 8) {
                Text("TIME IN BED")
                    .font(.maudeKicker(9.5)).tracking(MaudeTheme.Tracking.kicker)
                    .foregroundStyle(MaudeTheme.ink3)
                Text(m.inBedText)
                    .font(.maudeMono(14)).foregroundStyle(MaudeTheme.ink)
                Spacer()
                if let span = m.nightSpanText {
                    Text(span).font(.maudeMono(11)).foregroundStyle(MaudeTheme.ink3)
                }
            }
                ...
                VStack(spacing: 5) {
                    ForEach(Array(m.stageRows.enumerated()), id: \.offset) { i, s in
                        HStack(spacing: 8) {
                            ...
                            Text(s.name).font(.lato(12.5)).foregroundStyle(MaudeTheme.ink2)
                            Spacer()
    ...
    private func bedtimeCard(_ m: Model) -> some View {
        MetricDCard(kicker: "Bedtime · this week",
                    ...
                    foot: m.bedtimeFoot) {
            MetricCompareBars(rows: m.bedtimeRows, color: MaudeTheme.accentSleep)
        }
    }

    private func compareCard(_ m: Model, sentence: String) -> some View {
        MetricDCard(kicker: "Sleep · vs last week", headline: sentence) {
            MetricCompareBars(rows: m.compareRows, color: MaudeTheme.accentSleep)
        }
    }
    ...
    struct Model {
        ...
        init(derived d: SleepWeekDetail, isDemoSeed: Bool = false) {
            ...
            let rows: [StageRow] = d.hasStageDetail
                ? [StageRow(name: "Deep", minutes: d.deepMin, color: MaudeTheme.accentSleep),
                   StageRow(name: "REM", minutes: d.remMin, color: MaudeTheme.accentSleep.opacity(0.62)),
                   StageRow(name: "Core", minutes: d.coreMin, color: MaudeTheme.accentSleep.opacity(0.34))]
                : []
            stageRows = rows
...
// lives in Maude/Intelligence/SleepSampleFixture.swift — view files must never
