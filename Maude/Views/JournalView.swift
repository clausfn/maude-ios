...
struct JournalView: View {
    ...
    var body: some View {
          ...
          ScrollViewReader { proxy in
            ScrollView {
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    MaudeAppBar(title: "Journal", showMark: false)
        ...
        .task {
            if ProcessInfo.processInfo.environment["MAUDE_DEMO_EPRO"] == "1", provenanceOffer == nil,
               let first = journalEntries.first {
                issueProvenanceReceipt(for: first)
            }
            // A7.2 Area ⑧ snapshot hook: MAUDE_TAB=journal MAUDE_OPEN_VOICE=1
            // → the full-screen voice recorder.
            if ProcessInfo.processInfo.environment["MAUDE_OPEN_VOICE"] == "1" {
                showVoiceNote = true
            }
        }
        #endif
                        ...
                        Button {
                            ...
                            VStack(spacing: 4) {
                                Text(dayLetter(day))
                                    .font(.maudeKicker(9))
                                    .kerning(0.5)
                                    .foregroundStyle(isSelected ? MaudeTheme.invertFG : MaudeTheme.ink4)

                                Text(dayNumber(day))
                                    .font(.maudeMono(13))
                                    .foregroundStyle(isSelected ? MaudeTheme.invertFG : MaudeTheme.ink2)

                                Circle()
                                    .fill(hasContent ? (isSelected ? MaudeTheme.invertFG.opacity(0.7) : MaudeTheme.moss) : Color.clear)
                                    .frame(width: 4, height: 4)
                            }
                            .frame(width: 36, height: 58)
                            .background(isSelected ? MaudeTheme.invertBG : Color.clear)
                            .cornerRadius(10)
                        }
                        .buttonStyle(.plain)
                        .id(day)
            ...
            ForEach(TimelineFilter.allCases, id: \.self) { f in
                Button {
                    ...
                    Text(f.rawValue)
                        .font(.maudeKicker(10))
                        .kerning(0.5)
                        .foregroundStyle(activeFilter == f ? MaudeTheme.invertFG : MaudeTheme.ink3)
                        .padding(.horizontal, 12)
                        .padding(.vertical, 6)
                        .background(activeFilter == f ? MaudeTheme.invertBG : MaudeTheme.paper2)
                        .cornerRadius(20)
                        .overlay(Capsule().stroke(MaudeTheme.line, lineWidth: activeFilter == f ? 0 : 0.5))
                }
                .buttonStyle(.plain)
            }
            Spacer()
    ...
    private var captureGrid: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(spacing: 8) {
                captureButton("Log mood", icon: "face.smiling", tint: MaudeTheme.moss, bg: MaudeTheme.moss2) {
                    showMoodSheet = true
                }
                captureButton("Add meal", icon: "fork.knife", tint: MaudeTheme.moss, bg: MaudeTheme.moss2) {
            ...
            HStack(spacing: 8) {
                captureButton("Add symptom", icon: "waveform.path.ecg", tint: MaudeTheme.clay, bg: MaudeTheme.clay2) {
                    expandComposer(tag: "Symptom", prompt: "What are you noticing?")
                }
                captureButton("Voice note", icon: "mic.fill", tint: MaudeTheme.moss, bg: MaudeTheme.moss2) {
                    #if os(iOS)
                    showVoiceNote = true
                    #endif
                }
            ...
            captureButton("Log a supplement", icon: "pills.fill",
                          tint: MaudeTheme.moss, bg: MaudeTheme.moss2, wide: true) {
                showSupplementSheet = true
            }
            captureButton("Upload a document or photo", icon: "arrow.up.doc",
                          tint: MaudeTheme.ink3, bg: MaudeTheme.line2, wide: true) {
                #if os(iOS)
            ...
            HStack(spacing: 8) {
                ForEach(contextSuggestions, id: \.self) { s in
                    Button { expandComposer(tag: "Note", prompt: s) } label: {
                        HStack(spacing: 7) {
                            Circle().fill(MaudeTheme.clay).frame(width: 6, height: 6)
                        ...
                        .clipShape(Capsule())
                        .overlay(Capsule().stroke(MaudeTheme.clay3, lineWidth: 1))
                    }
                    .buttonStyle(.plain)
                }
                Spacer(minLength: 0)
    ...
    private var moodSheet: some View {
        VStack(spacing: 0) {
            Capsule().fill(MaudeTheme.line).frame(width: 38, height: 4).padding(.top, 12).padding(.bottom, 18)
            Text("How's your energy?")
                .font(.maudeSerif(20)).kerning(-0.2).foregroundStyle(MaudeTheme.ink)
            Text("One tap. You can add a note after — or not.")
                .font(.lato(13)).foregroundStyle(MaudeTheme.ink3).padding(.top, 4)
    ...
    private func moodFace(_ label: String, _ icon: String, _ logged: String) -> some View {
        Button { logMood(logged) } label: {
            VStack(spacing: 7) {
                ZStack {
                    Circle().stroke(MaudeTheme.line, lineWidth: 1.5).frame(width: 54, height: 54)
                    Image(systemName: icon).font(.system(size: 22)).foregroundStyle(MaudeTheme.ink3)
                }
                Text(label).font(.lato(11, .bold)).foregroundStyle(MaudeTheme.ink3)
            }
        }
        .buttonStyle(.plain)
    }
    ...
    private var supplementSheet: some View {
        VStack(alignment: .leading, spacing: 0) {
            Capsule().fill(MaudeTheme.line).frame(width: 38, height: 4)
                .frame(maxWidth: .infinity)
            ...
            Button { } label: {   // HONEST-STUB (disabled + SOON chip)
                HStack(spacing: 11) {
                    ...
                    VStack(alignment: .leading, spacing: 1) {
                        Text("Scan the label").font(.lato(14, .bold)).foregroundStyle(MaudeTheme.ink)
    ...
    private func supplementField(_ label: String, placeholder: String, text: Binding<String>) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            Text(label.uppercased())
                .font(.maudeKicker(9)).tracking(0.6).foregroundStyle(MaudeTheme.ink4)
                .lineLimit(1).minimumScaleFactor(0.8)
            TextField(placeholder, text: text)
                .font(.lato(14)).foregroundStyle(MaudeTheme.ink)
    ...
    private var composerCard: some View {
                ...
                ForEach(["Glucose", "Sleep", "Mood", "Activity"], id: \.self) { tag in
                    ...
                    Button {
                        ...
                        Text(tag)
                            ...
                            .kerning(0.4)
                            .foregroundStyle(selected ? tagColor(tag) : MaudeTheme.ink3)
                            .padding(.horizontal, 8)
                            .padding(.vertical, 5)
                            .background(selected ? tagBg(tag) : MaudeTheme.paper)
                            .overlay(Capsule().stroke(selected ? tagColor(tag).opacity(0.4) : MaudeTheme.line, lineWidth: 0.5))
                            .clipShape(Capsule())
                    }
                    .buttonStyle(.plain)
                }
        ...
        .overlay(RoundedRectangle(cornerRadius: 12)
            .stroke(composerExpanded ? MaudeTheme.moss3 : MaudeTheme.line2, lineWidth: 1))
        .shadow(color: MaudeTheme.cardShadow, radius: composerExpanded ? 12 : 4, y: 2)
    ...
    private func entryCard(_ entry: JournalEntry) -> some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            HStack(spacing: 0) {
                ...
                HStack(spacing: 3) {
                    ...
                    Text("on device").font(.maudeKicker(8.5)).tracking(0.3)
                }
                .foregroundStyle(MaudeTheme.moss)
        ...
        .background(MaudeTheme.paper2)
        .cornerRadius(12)
        .overlay(RoundedRectangle(cornerRadius: 12).stroke(MaudeTheme.line2, lineWidth: 0.5))
        .overlay(alignment: .leading) {
            RoundedRectangle(cornerRadius: 2)
                .fill(entry.tags.first.map { tagColor($0) } ?? MaudeTheme.ink4)
                .frame(width: 3)
                .padding(.vertical, 6)
        }
    ...
    private func tagColor(_ tag: String) -> Color {
        ...
        case "Mood":    return MaudeTheme.ink3
        default:        return MaudeTheme.ink3
        }
    }

    private func tagBg(_ tag: String) -> Color {
        switch tag {
        case "Glucose": return MaudeTheme.clay2
        case "Sleep":   return MaudeTheme.moss2
        default:        return MaudeTheme.line2
        }
    }

    private func vaultColor(_ key: VaultDocType.VaultColor) -> Color {
        switch key {
        case .amber: return MaudeTheme.clay
        case .moss:  return MaudeTheme.moss
        case .ink:   return MaudeTheme.ink
        case .blue:  return MaudeTheme.accentRecovery
        case .ink3:  return MaudeTheme.ink3
        }
    }
