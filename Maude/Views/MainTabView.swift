...
// Design ref: Maude_App_UI_Aperture_v01_20260521.html (tab bar)
import SwiftUI
#if canImport(UIKit)
...
enum MaudeTab: String, CaseIterable {
...
private struct TabFrameKey: PreferenceKey {
    static let defaultValue: [MaudeTab: CGRect] = [:]
    static func reduce(value: inout [MaudeTab: CGRect], nextValue: () -> [MaudeTab: CGRect]) {
...
struct MainTabView: View {
    ...
    @State private var tab: MaudeTab = {
        #if DEBUG
        if let raw = ProcessInfo.processInfo.environment["MAUDE_TAB"],
           let t = MaudeTab(rawValue: raw) { return t }
        #endif
        return .home
    }()
    ...
    @State private var tabFrames: [MaudeTab: CGRect] = [:]   // measured slots (bar space) → hit-test + rest pill
    ...
    private let tabBarSpace = "maudeTabBar"
    #if DEBUG
    @State private var debugOpenChat = false
    @State private var debugOpenThread = false
    @State private var debugOpenSources = false   // MAUDE_OPEN_SOURCES=1 (Area ⑦ screenshots)
    @State private var debugOpenVault = false     // MAUDE_OPEN_VAULT=1
    @State private var debugOpenLearn: LearnTopic? = nil   // MAUDE_OPEN_LEARN=hrv (Area ⑨)
    #endif
    ...
    private var showConnectHintResolved: Bool {
        #if DEBUG
        if ProcessInfo.processInfo.environment["MAUDE_FORCE_HINT"] == "1" { return true }
        #endif
    ...
    var body: some View {
                ...
                case .home:
                    NavigationStack {
                        ...
                        .navigationDestination(item: $selectedNudge) { nudge in
                            NudgeDetailView(nudge: nudge).maudeDetail()
                        }
                    }
                case .insights:
        ...
        .maudeSampleModeBanner(appState,
        ...
        .onReceive(NotificationCenter.default.publisher(for: .maudeOpenEdition)) { _ in
            tab = .home
        }
        ...
        .onReceive(NotificationCenter.default.publisher(for: .maudeOpenAttention)) { _ in
            tab = .home
        }
        ...
        // space directly (MAUDE_OPEN_VAULT wins when both are set).
        ...
        // directly (MAUDE_OPEN_LEARN=hrv; add MAUDE_LEARN_TIER=clinical for tier B).
        .sheet(item: $debugOpenLearn) { topic in
            LearnArticleView(
                topic: topic, appState: appState,
                startTier: ProcessInfo.processInfo.environment["MAUDE_LEARN_TIER"] == "clinical"
                    ? .clinical : nil)
        }
        .task {
            if ProcessInfo.processInfo.environment["MAUDE_OPEN_CHAT"] == "1" { debugOpenChat = true }
            if ProcessInfo.processInfo.environment["MAUDE_OPEN_THREAD"] == "1" { debugOpenThread = true }
            if let raw = ProcessInfo.processInfo.environment["MAUDE_OPEN_LEARN"],
               let topic = LearnTopic(rawValue: raw) { debugOpenLearn = topic }
            if ProcessInfo.processInfo.environment["MAUDE_OPEN_VAULT"] == "1" { debugOpenVault = true }
            else if ProcessInfo.processInfo.environment["MAUDE_OPEN_SOURCES"] == "1" { debugOpenSources = true }
            // UC-RSCH deterministic screenshots: SHOW_RESEARCH=1 → Home card (s09);
            // OPEN_STUDY=1 → consent (s10); OPEN_STUDY=joined → joined (s11).
            let study = ProcessInfo.processInfo.environment["MAUDE_OPEN_STUDY"]
            if ProcessInfo.processInfo.environment["MAUDE_SHOW_RESEARCH"] == "1" || study != nil {
        ...
        .sheet(isPresented: Binding(
            ...
            let startJoined: Bool = {
                #if DEBUG
                return ProcessInfo.processInfo.environment["MAUDE_OPEN_STUDY"] == "joined"
                #else
                return false
                #endif
            }()
    ...
    /// Every Maude font resolves through `UIFontMetrics`, which reads the
    ...
    private var barTier: MaudeBarTier {
        maudeBarTier(glassOn: glassOn,
    ...
    private func select(_ item: MaudeTab) {
    ...
    private func tabAt(_ x: CGFloat) -> MaudeTab? {
    ...
    private var tabRow: some View {
        ...
        HStack(spacing: 2) {
            ForEach(MaudeTab.allCases, id: \.self) { item in
                tabCell(item)
            }
        }
    }

    @ViewBuilder private func tabCell(_ item: MaudeTab) -> some View {
        let active = tab == item
        let k = barTypeScale
        VStack(spacing: 3) {
        ...
        .foregroundStyle(active ? MaudeTheme.ink : MaudeTheme.ink3)
    ...
    private var lens: (cx: CGFloat, cy: CGFloat, w: CGFloat, h: CGFloat, capR: CGFloat, halfLen: CGFloat)? {
        ...
        let w = (b.width / CGFloat(MaudeTab.allCases.count)) * 1.6
        let h = b.height * 0.92
        let capR = h / 2
    ...
    private var tabBar: some View {
        ...
        return tabRow
            ...
            .background {
                ZStack {
                    Capsule().fill(MaudeTheme.paper2)
                    restChip
                }
            }
            .compositingGroup()
            ...
            .overlay(Capsule().strokeBorder(MaudeTheme.line, lineWidth: 0.5))   // floating-bar edge
            ...
            // is NOT enough — Maude's own fonts scale through UIFontMetrics and
