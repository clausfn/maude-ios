...
struct LabReportImportView: View {
    ...
    private var verdictBand: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text(stage == .review
                 ? "Check what Maude read, then decide."
    ...
    private var howCard: some View {
        VStack(alignment: .leading, spacing: 11) {
            ...
            step(2, "Maude reads it here on this phone and lists what it recognised — with what it could not read, and why.")
            step(3, "You edit or drop any row. Nothing enters your record until you tap Save.")
            Text("Maude reads 15 common results: HbA1c, glucose, cholesterol (total, LDL, HDL), triglycerides, creatinine, eGFR, ALAT, TSH, haemoglobin, ferritin, CRP, vitamin D and B12. Anything else is listed, not guessed.")
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                Image(systemName: "doc.text.fill")
                    .font(.system(size: 14)).foregroundStyle(MaudeTheme.ink3)
            ...
            VStack(alignment: .leading, spacing: 4) {
                DatePicker(selection: $sampleDate, in: ...Date(), displayedComponents: .date) {
                    Text("Sample date").font(.lato(13)).foregroundStyle(MaudeTheme.ink2)
                }
                Text(printedDate == nil
                     ? String(localized: "No date was printed on the file that Maude could read — today's date is filled in. Change it if you know the day the sample was taken.")
    ...
    private var nothingReadCard: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Text("Maude didn't find any of the 15 results it can read in this file, so there is nothing to save. A straight-on photo of the results table, or the lab's own PDF, usually works better.")
        ...
        return VStack(alignment: .leading, spacing: 7) {
            HStack(alignment: .firstTextBaseline, spacing: 10) {
                ...
                TextField("", text: row.text)
                    .font(.maudeMono(17))
                    .foregroundStyle(r.value == nil ? MaudeTheme.ink4 : MaudeTheme.ink)
            ...
            if let v = r.value {
                ...
                Text("Type a number to save this row.")
                    .font(.lato(11.5)).foregroundStyle(MaudeTheme.brass)
            }

            if r.isLowConfidence {
                HStack(alignment: .top, spacing: 6) {
                    Image(systemName: "eye.trianglebadge.exclamationmark")
                        .font(.system(size: 11)).foregroundStyle(MaudeTheme.brass)
    ...
    private var unreadCard: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(String(localized: "Not read (\(unread.count))").uppercased())
                .font(.maudeKicker(10)).tracking(1.2)
                .foregroundStyle(MaudeTheme.ink3)
            Text("Maude leaves these alone rather than guessing at them. They are not saved.")
                .font(.lato(12)).lineSpacing(2)
                .foregroundStyle(MaudeTheme.ink2)
                .fixedSize(horizontal: false, vertical: true)
            ForEach(unread.prefix(12)) { line in
                VStack(alignment: .leading, spacing: 2) {
                    Text(line.text)
                        .font(.maudeMono(11)).foregroundStyle(MaudeTheme.ink2)
                        .lineLimit(2)
    ...
    private func reason(_ r: UnreadLabLine.Reason) -> String {
        switch r {
        case .unknownAnalyte:       return String(localized: "Not one of the results Maude reads yet.")
        case .unknownUnit:          return String(localized: "The unit on this line isn't one Maude can convert exactly.")
        ...
        HStack(spacing: 10) {
            ...
            Button { save() } label: {
                Text(includedCount == 0 ? "Nothing selected" : "Save \(includedCount) to my record")
                    ...
                    .foregroundStyle(.white)
                    .frame(maxWidth: .infinity).padding(.vertical, 12)
                    .background(includedCount == 0 ? MaudeTheme.ink4 : MaudeTheme.moss)
                    .clipShape(RoundedRectangle(cornerRadius: 12))
            }
            .buttonStyle(.plain)
    ...
    private func errorBanner(_ message: String) -> some View {
        HStack(alignment: .top, spacing: 8) {
            Image(systemName: "exclamationmark.triangle.fill")
                .font(.system(size: 13)).foregroundStyle(MaudeTheme.brass)
