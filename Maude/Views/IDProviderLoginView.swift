...
// AltID uses zero-knowledge proofs) → verified → back to Maude. Brand-coloured
...
struct IDProvider: Identifiable, Equatable {
    ...
    static let altID = IDProvider(
        ...
        unlockSubtitle: "Sign in to Maude with the official Danish identity wallet — you choose what to share.",
        disclose: [
...
struct IDProviderLoginView: View {
    ...
    init(provider: IDProvider, onComplete: @escaping () -> Void, onCancel: @escaping () -> Void = {}) {
        ...
        #if DEBUG
        switch ProcessInfo.processInfo.environment["MAUDE_IDP_STEP"] {
    ...
    private var footer: some View {
        HStack(spacing: 6) {
            ...
            Text(provider.footer).font(.maudeKicker(9.5)).tracking(0.5).foregroundStyle(sub)
        }
    }
    ...
    private var presentStep: some View {
        VStack(spacing: 16) {
            ...
            card {
                Text("YOU'LL SHARE").font(.maudeKicker(9.5)).tracking(1).foregroundStyle(cream.opacity(0.9))
    ...
    private var verifiedStep: some View {
        VStack(spacing: 16) {
            ...
            VStack(spacing: 8) {
                ...
                Text("Your identity is confirmed. Only the facts you approved were shared with Maude.")
                    .font(.lato(13.5)).lineSpacing(2).foregroundStyle(sub)
                    .multilineTextAlignment(.center).padding(.horizontal, 6)
            }
            HStack(spacing: 8) {
                Text(provider.refLabel).font(.maudeKicker(9)).tracking(1).foregroundStyle(sub)
                Text(ref).font(.maudeMono(12)).foregroundStyle(cream)
            }
            .padding(.horizontal, 14).padding(.vertical, 9)
            .background(Color.white.opacity(0.08)).clipShape(Capsule())
            .overlay(Capsule().stroke(line, lineWidth: 1))
            primaryButton("Enter Maude", icon: "arrow.right") { onComplete() }
        }
    }
