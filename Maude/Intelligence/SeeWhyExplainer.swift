...
// Maude prints, in the user's own terms (Bevel absorb ②).
//
...
// ask what went into it. Maude's structural answer is that EVERY verdict surface
...
//    The two places Maude does lean on a shared reference (the 70 % time-in-range
...
public nonisolated enum SeeWhyExplainer {
    ...
    static func stillLearning(id: String, surface: String, verdict: String,
        ...
        SeeWhyExplanation(
            id: id, surface: surface, verdict: verdict,
            unexplained: String(localized: "There is nothing to decompose yet — Maude has \(have) of the \(need) days it needs before it will compare anything to your usual. Until then it says so instead of guessing."),
            method: .usualBand)
    }
    ...
    public static func todayHero(tone: TodayTone, verdict: String,
        ...
        if coldStart {
            return SeeWhyExplanation(
                id: id, surface: surface, verdict: verdict,
                unexplained: String(localized: "There is no verdict to take apart yet — Maude is still collecting your first days. It will not compare you to anything until it has your own days to compare against."),
                method: .usualBand)
        }
        ...
        case .uneven:
            ...
            rows.append(SeeWhyRow(lReference, String(localized: "Glucose is the one signal Maude also holds against a shared clinical mark — 70% of readings in range. Every other word on this page is measured against your own usual.")))
        ...
        if rows.isEmpty {
            return SeeWhyExplanation(
                id: id, surface: surface, verdict: verdict,
                unexplained: String(localized: "This line needs at least four days with readings before it can be split into an earlier and a later half. Maude has fewer than that right now."),
                method: .usualBand)
        }
    ...
    public static func signalCard(domain: SignalDomain, verdict: String, value: String,
        ...
        switch domain {
        case .sleep:
            rows.append(SeeWhyRow(lWord, String(localized: "Maude compares the later half of your week with the earlier half. About 25 minutes more reads as “A little longer”; anything smaller stays “As usual”.")))
        case .glucose:
    ...
    public static func markedDay(verdict: String, kindLabel: String,
        ...
        SeeWhyExplanation(
            ...
            verdict: verdict,
            rows: [
                SeeWhyRow(lNumbers, String(localized: "Nothing was measured to produce this line — you told Maude these days are \(kindLabel.lowercased()).")),
                SeeWhyRow(lWindow, isOpen
                          ? String(localized: "From \(startedText), still open until you clear it.")
                          : String(localized: "From \(startedText).")),
                SeeWhyRow(lWord, String(localized: "While a day is marked, Maude stops reading it as drifting from your usual. It can only ever remove a comparison — it never adds one, and it never changes a number.")),
    ...
    public static func glucoseHero(verdict: String, inRangePct: Int, prevWeekPct: Int?,
        ...
        rows.append(SeeWhyRow(lReference, String(localized: "The target band here is the shared clinical one (3.9–10.0 mmol/L) — glucose is the single place Maude uses a clinical range rather than your own usual.")))
        if let gmi = gmiPct {
    ...
    public static func nudge(verdict: String, n: Int, nUnit: String, baselineDays: Int,
            ...
            return SeeWhyExplanation(
                ...
                rows: [
                    ...
                    SeeWhyRow(lWindow, baselineNeeded.map {
                        String(localized: "\(n) of the \($0) needed before Maude will assert anything.")
                    } ?? String(localized: "Still below the number of days Maude needs.")),
                    SeeWhyRow(lWord, String(localized: "Below the gate Maude refuses to assert a pattern. Saying “still learning” is the honest answer, and it is the one you get.")),
                ],
                method: .evidenceGate)
        }
        var rows: [SeeWhyRow] = [
        ...
        rows.append(SeeWhyRow(lBaseline, String(localized: "Your own paired days only. Maude never pools your data with anyone else's to reach this.")))
