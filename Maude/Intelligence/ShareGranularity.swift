...
// because `MaudeBackendService.createGrant` fills `granularity` in with a
...
// summaries, so `summary` is the only level Maude is able to honour today.
import Foundation

enum ShareGranularity {
    /// The only detail level Maude can actually package (FR-SHARE-02).
