...
// so the model can never move Maude across the non-MDSW line. v01 2026-06-09.
//
// PR-102 (launch audit): the Mistral key is DEBUG-only. Debug builds call the EU
// Mistral endpoint directly with the local gitignored key (MaudeSecrets); Release
...
// the key server-side (route prepared in maude-backend in parallel).
import Foundation
...
enum MistralClient {
    ...
    static var hasKey: Bool { !MaudeSecrets.mistralAPIKey.isEmpty }
    #else
    ...
    static func complete(system: String, user: String) async throws -> String {
        ...
        let key = MaudeSecrets.mistralAPIKey
