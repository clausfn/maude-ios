...
// calls POST /ingest/sundhed (Bearer = the citizen's Maude GoTrue JWT).
//
...
//   • the Maude GoTrue JWT (bearer for POST /ingest/sundhed) — held by the app;
...
struct SundhedWebSessionView: View {
            ...
            HStack(spacing: 10) {
                Button { dismiss() } label: {
                    Image(systemName: "chevron.left")
                        ...
                        .overlay(Circle().stroke(MaudeTheme.line2, lineWidth: 1))
                }
                .buttonStyle(.plain)
                .accessibilityLabel("Back")
                Text("Connect Sundhed.dk")
                    .font(.lato(17, .bold)).foregroundStyle(MaudeTheme.ink)
                Spacer()
            }
    ...
    private var controlBar: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            if let lp = lastPull, phase == .idle || phase == .failed {
                HStack(spacing: 10) {
                    Image(systemName: "checkmark.seal.fill")
                        .font(.system(size: 15)).foregroundStyle(MaudeTheme.moss)
                ...
                .padding(10)
                .background(RoundedRectangle(cornerRadius: 10).fill(MaudeTheme.moss2.opacity(0.5)))
            }

            if let message {
                Text(message)
                    .font(.lato(12.5)).lineSpacing(2)
                    .foregroundStyle(phase == .failed ? MaudeTheme.rust : MaudeTheme.ink3)
            }
            ...
            Text(loggedIn
                 ? (lastPull == nil
                    ? "You're signed in to Sundhed.dk. Maude is opening your Medicine, Lab results, Diagnoses and Hospital-journal pages and reading only summaries and codes. Your journal text is never read."
                 ...
                 : "Sign in with MitID above. Maude never sees or stores your MitID login — that happens directly with Sundhed.dk.")
            ...
            Button { startPull() } label: {
                ...
                .background(RoundedRectangle(cornerRadius: 12)
                    .fill(canHarvest ? MaudeTheme.moss : MaudeTheme.ink4))
            }
            .buttonStyle(.plain)
            .disabled(!canHarvest)
            ...
            if phase == .done {
                Button {
                    ...
                    .background(RoundedRectangle(cornerRadius: 12).fill(MaudeTheme.moss2))
                ...
                HStack(spacing: 9) {
                    ZStack {
                        RoundedRectangle(cornerRadius: 8)
                            .fill(st == .done ? MaudeTheme.moss : MaudeTheme.ink.opacity(0.10))
                        Image(systemName: row.icon)
                            .font(.system(size: 12, weight: .semibold))
                            .foregroundStyle(st == .done ? .white : MaudeTheme.ink)
                    }
                    .frame(width: 26, height: 26)
                    Text(row.title)
                        .font(.lato(13, .semibold))
                        .foregroundStyle(st == .done ? MaudeTheme.moss
                                         : st == .active ? MaudeTheme.ink
                                         : MaudeTheme.ink3)
                    Spacer()
                    switch st {
                    case .done:
    ...
    private var disabledNote: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text("Coming soon")
                .font(.maudeKicker(10.5)).tracking(0.6)
                .foregroundStyle(MaudeTheme.ink3)
    ...
    private func ingest(_ harvest: SundhedWebHarvest) async {
        guard let citizenId, !citizenId.isEmpty else {
            fail("Sign in to Maude first, then try again.")
            return
        }
...
private struct SundhedWebView: UIViewRepresentable {
    ...
    final class Coordinator: NSObject, WKScriptMessageHandler, WKNavigationDelegate, WKUIDelegate {
        static let channel = "maudeSundhed"
        ...
        func runHarvest() {
            webView?.evaluateJavaScript("window.__maudeSundhedHarvest && window.__maudeSundhedHarvest();") { [weak self] _, err in
        ...
        func clearCaps() {
            webView?.evaluateJavaScript("window.__maudeSundhedClear && window.__maudeSundhedClear();", completionHandler: nil)
        }
        ...
        func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
            webView.evaluateJavaScript("window.__maudeSundhedProbe && window.__maudeSundhedProbe(); window.__maudeSundhedScan && window.__maudeSundhedScan();", completionHandler: nil)
        }
    ...
    //   3. exposes __maudeSundhedProbe()   → posts {type:"session", loggedIn};
    //   4. exposes __maudeSundhedHarvest()  → REDUCES the accumulated caps to codes
    //      + aggregates, posts {type:"harvest", payload:<SundhedWebHarvest>};
    //   5. exposes __maudeSundhedClear()    → drops the caps after a successful ingest.
    //
    ...
    (function () {
      "use strict";
      // ===== MAUDE SUNDHED INTERCEPTOR + REDUCER — BEGIN (injected, documentStart) =====
      var CH = "maudeSundhed";
      function post(msg) {
      ...
      var CAP = { labs: "maude.cap.labs", meds: "maude.cap.meds", conditions: "maude.cap.conditions" };
      ...
      (function patchXHR() {
        ...
        XMLHttpRequest.prototype.open = function (method, url) {
          try { this.__maudeURL = url; } catch (e) {}
      ...
      window.__maudeSundhedScan = function () {
        try {
      ...
      window.__maudeSundhedProbe = function () {
      ...
      window.__maudeSundhedHarvest = function () {
        try {
          // One last DOM scan in case we're sitting on a code page right now.
          try { if (onCodePage()) window.__maudeSundhedScan(); } catch (e) {}
      ...
      // ===== MAUDE SUNDHED INTERCEPTOR + REDUCER — END =====
    })();
    """#
}
