...
// else in this app. Read `Maude_DonorDataProgramme_v01_20260813.md` before
...
//      performs. Nothing in `Maude/Donation/` opens a network connection, and
...
//   2. The flow exists only in a DONOR BUILD (`-D MAUDE_DONOR`). In every
...
enum DonationProgramme {
    ...
    static let payloadSchema = "maude.donation.v1"
    ...
    /// TRUE only in a build compiled with `-D MAUDE_DONOR`. Every shipped
    ...
    static var configuredPublicKeyBase64: String? {
        #if DEBUG
        if let raw = ProcessInfo.processInfo.environment["MAUDE_DONATION_PUBKEY"],
           !raw.isEmpty {
            return raw
        }
        #endif
    ...
    static let excludedItems: [String] = [
        ...
        "Anything you have written in Maude — journal entries, notes, voice notes, chat",
