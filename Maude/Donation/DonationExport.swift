...
// `Maude/Donation/`. There is no URLSession, no URLRequest, no upload task, no
...
enum DonationExport {
    ...
    static func defaultStagingDirectory() -> URL {
        FileManager.default.temporaryDirectory
            .appendingPathComponent("maude-donations", isDirectory: true)
    }
    ...
    /// `maude-donation-<grant reference>-<yyyyMMdd>.lvdn`. The reference is the
    ...
    static func fileName(grantReference: String, sealedAt: Date) -> String {
        ...
        return "maude-donation-\(safe(grantReference))-\(fmt.string(from: sealedAt)).\(DonationProgramme.fileExtension)"
    }
