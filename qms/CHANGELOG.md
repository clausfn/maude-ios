...
Suite: **T-VIZ-04 = `MaudeTests/DayAxisIntegrityTests`** (6 — sleep fallback axis, activity gaps, energy absent-vs-gap, fitness rest week, fitness pre-record weeks, copy lint). **Full unit run 879 tests / 109 suites — all green** (873 before this PR); UI target 130/130; `guard_provenance.sh` green.
...
**584 tests / 81 suites PASS**; Debug + donor-flag (`-D MAUDE_DONOR`) builds green; provenance guard green; new blocking donation-egress guard green and mutation-checked.
...
- **FR-DIAG-01 — the instrument.** Settings → "Sleep diagnostics · 14-night report": every raw HK sample (source + bundle id + device, stage, start–end, drops marked) against every derived value with the exact computed-from and excluded-with-reason lists. Share-sheet export only (transport constructs lint-banned in Maude/Diagnostics/), Release-enabled so CN can run it on TestFlight, blocked in sample mode. RK-DIAG-01 covers the instrument's own leak hazard.
...
PR-109's `maudeScrollEdge()` was merged believing it worked; the sweep proved it did not (content at full opacity under the Dynamic Island in all six scrolled frames). Root causes, both reproduced on device before changing anything:
...
### Other
- **Scroll edge:** `maudeScrollEdgeSoft()` was a no-op below iOS 26 / with the glass flag off — which is why the sweep saw content under the Dynamic Island. New `maudeScrollEdge()` fades from the paper token, sized from the real safe-area inset, firmer under Reduce Transparency; an overlay with hit-testing off, so content size, offsets and the snapshot hook are untouched.
- **Consult stage placeholder:** the black rectangle is replaced by a derived stage (`CallStageDeriver`): initials, name · organisation, an honest status line, self-tile caption and a retry affordance. `.live` renders nothing extra — when media is up, Maude claims nothing.
...
- **① FR-WID-01** — source + `MaudeWidgets/SETUP.md` prepared; `project.pbxproj` untouched per repo rule, so the two Widget-Extension targets remain a short Xcode-GUI task for CN.
...
- **Posture:** signing = Fonden Data For Good PS258XSNL8, bundle `xyz.ppcn.maude` (ASC app 6776228205), export `app-store-connect`/automatic; the exported .ipa is additionally gated by `scripts/guard_release_posture.sh` (get-task-allow=false, Apple Distribution cert, beta-reports-active) — a failure ABORTS the upload.
- **Distribution scope:** **INTERNAL TestFlight only.** Promotion to the external group "Maude beta tester" remains blocked pending the standing gates — DPIA v02 signed (RK-SUND-01 covers the shipped in-app sundhed flows + diagnoses + external cohort), written Trifork/sundhed.dk sanction artifact (or `sundhedWebConnectEnabled` gated off for external builds), and the ClickUp decisions task z8nrz7c35v (self-signup lock).
...
- **feat(onboarding) — Area ①, the designed flow (f-onboarding.jsx).** New `Maude/Views/Onboarding/`: flow machine + shared chrome (9-segment progress, back chevron, n/9 counter, ambient accent glow), Canvas dawn-fjord cover + Ready finale (stroke-drawn iris, Reduce-Motion static), Why-Maude, governance sub-pager + "What's kept, anonymously" data-minimisation card, name capture (device-local `displayName` overlay), passport step over real `healthContext` → ProfileSheet editors, sharing explainer, backup posture persisted (`BackupPreference`; sovereign = visible non-selectable "Coming soon"), literacy step (`literacyLevel` plain|both|clinical, default both — consumers land with FR-LIT-01), how-Maude-learns. `AuthView`/`HealthKitPrimerView` restyled IN PLACE — every real GoTrue/HealthKit state kept. Satellites: HealthKit-declined honest screen, Face ID app lock (`.deviceOwnerAuthentication`, fails open, Settings toggle, NSFaceIDUsageDescription), MitID calm prompt fronting the Sundhed session, study-enrollment IdentityVerify (ID_METHODS seed, dfg_mpc preferred, UI-only stub, decline always available). PrivacyDeclarationView remains the first gate (FR-REG-01).
...
Skin only — IA/copy/navigation unchanged; dark "Evening" args untouched except the two documented exceptions. Source: the A7.2 design handoff (`design_handoff_maude_a7`, revised package) — the outcome of Christel's review rounds 3–4 (52 of 57 threads resolved by the package; 5 partial → copy pass / feature backlog).
...
- **feat(assets).** AccentColor (was empty) light/dark set; LaunchBackground → paper/marine; DfG colour logo from the package; MaudeMark → cobalt+`00B5AC`; reversed marks + WatchMark → white+`00E0D0`; AppIcon.icon → mid-cobalt ground (gradient refinement deferred to Icon Composer GUI), white iris, `00E0D0` ring.
...
- **feat(sundhed) — Path A: live in-app connect (10.88).** `SundhedWebSessionView`: the citizen signs in to sundhed.dk with MitID inside a WKWebView (credentials never visible to Maude); an injected document-start interceptor captures the SPA's own same-origin API responses across four Min Sundhedsjournal pages (medicine, labs, diagnoses, hospital journal) + a DOM read for ICD-10 codes, reduced in-page to codes/aggregates. **Terminal sink is the on-device store ONLY** — the earlier automatic upload was removed; nothing leaves the device from this flow.
- **feat(record) — canonical on-device health record store (10.89).** `Maude/Health/HealthRecordStore.swift` + schema (`HealthObservation`/`HealthCondition`/`HealthMedication`): source-agnostic (Sundhed live/PDF today; OCR/HealthKit later), per-source dedup/supersession, file-protected SwiftData, cloudKit `.none`. Off-device ONLY via the explicit "contribute to research" action (coded catalog v0.4.0 body → existing consented ingest client).
...
- **feat(rails) — sandbox wallet-rail detour retired.** `Config.walletRailBaseURL` no longer hard-codes the Scaleway sandbox container for prod builds: ALL rails (grants, ledger, wallet issuance, care surface) ride `api.maude.app` in Release. A DEBUG-only `MAUDE_WALLET_RAIL_URL` env override remains for local dev against a split backend. **Deploy gating is the owner's step — route parity (journal, care, issuance routes) must land on `api.maude.app` before this branch ships.**
- **feat(posture) — Release-posture enforcement.** New `Maude/ReleasePosture.swift`: `#if !DEBUG` runtime preconditions executed at every Release launch (simulated logins off, wallet rail on the main backend, default backend = sovereign prod, no dev seed token) — a regression crashes the first launch, not a review. Plus `MaudeTests/ReleasePostureTests.swift`: a source-lint suite (conditional-compilation walker) verifying every demo gate (`dfgWalletLoginEnabled`/`nationalIDLoginEnabled`, `.mock` default, LV001 injection, mock wallet/care/research seeding, AuthView demo button, Journal/Vault/Privacy demo seeds, ColdStart seed table, sandbox rail host absence) — fails the unit-test run in any configuration.
...
- **Verification:** `guard_provenance.sh` green · Debug build green · Release build (`-configuration Release`, iPhone 17 sim) green · `MaudeTests` green incl. 4 new suites (`ReleasePostureTests`, `JournalSyncMappingTests`, `ReceiptDecodeTests`, `EraseOrderingTests`).
...
- **fix(sec) — no Mistral key compiles into Release binaries.** `MaudeSecrets.mistralAPIKey` is `#if DEBUG`-gated (empty string in Release, referenced nowhere on the Release path); `MistralClient` now branches: Debug = direct EU Mistral call with the local gitignored key (unchanged); Release = backend proxy `POST {apiBase}/ai/chat` `{system, user}` → `{content}` on the sovereign API base, authorised with the Keychained session bearer (NFR-SEC-01) — the key stays server-side (proxy route prepared in maude-backend in parallel). `hasKey` in Release now means "proxy reachable" (sovereign backend), keeping the ChatView consent-toggle + on-device fallback semantics intact. **Owner action (open): rotate the Mistral key** — it compiled into earlier builds.
...
- **docs(qms) — 8 missing RTM rows added (FR-QMS-03 cardinal violation closed):** FR-RSCH-03, FR-WAL-08, FR-PROV-02, FR-PAT-01, FR-PAT-02, FR-PAS-03, FR-ING-08, NFR-RSCH-04 — each row names its design element and its covering test (`WorkoutDedupTests` T-DED-01..05, `PatternEngineTests` T-PAT-01..04, `tirRespectsCustomBand`) or an explicit named test gap. RTM version bumped to 2026-07-03. **Found in passing:** the PR-99 changelog entry below claims `T-RSCH-01`/`T-CONSENT-REACT-01` were authored — neither exists in `MaudeTests/`, and `RK-CONSENT-REACT` (referenced from `ConsentReactivation.swift`) is absent from RISK.md; recorded as test gaps in the new RTM rows, tests still to be authored (consent-critical).
...
- **chore(ci) — dead-CTA guard added to `build-ios.sh` (blocking, before xcodebuild):** the build fails on any `Button { }` empty closure in `Maude/` Swift sources unless the line carries an explicit `// HONEST-STUB` marker (all six disabled stubs, incl. the pre-existing `JournalView` one, are marked). Verified: guard green on the tree, red on an offending fixture. Still to author (owner): the audit's `noDeadPrimaryCTAs` XCUITest tap-sweep (test plan row 8).
...
- **docs(release) — `GOLIVE_CHECKLIST.md` rewritten against reality (audit cross-check verdict: FAIL).** The old file claimed build 10.20 / 114 tests (actual at audit @ 0558bdd: **10.70 / 126 tests in 28 suites**), said the Watch target "must be created in Xcode" (it is **built AND embedded** — `ValidateEmbeddedBinary …/Watch/Maude Watch.app` in the audited build log), and asserted "Release build green" without evidence. The rewrite states only what the 2026-07-03 audit verified, lists Release build / MaudeUITests / device archive / Watch runtime explicitly as **NOT yet run**, and adds a §0 release-gates block: (1) demo-login flags compile-time OFF outside DEBUG (enforced in code, wave 1 — Release spot-check pending); (2) Mistral key rotated + backend `/ai/chat` proxy live before any public build (key burned — compiled into builds ≤ 10.70); (3) post-deploy route parity green via `maude-backend/scripts/route-parity.sh` (8 client-called routes currently 404 on api.maude.app); (4) `guard_provenance.sh` green as the blocking `build-ios.sh` step (real CI still absent); (5) owner review of the 83 × 5 `needs_review` localization drafts from wave 4.
...
- **Progressive enhancement, iOS 17 floor kept** (CN: "no exclusive phones"). New `Maude/Views/LiquidGlass.swift` + `GlassComponents.swift`; all effects behind `@AppStorage("liquidGlass")` (default ON, **Settings → Liquid Glass**) and `if #available(iOS 26, *)`. Three-tier ladder on every surface: iOS 26 real `.glassEffect` → iOS 17–25 `.ultraThinMaterial` → opaque `paper2` under Reduce Transparency / Increase Contrast. Motion resolves to rest via `MaudeMotion.reduced`. Flag off / iOS < 26 = prior app byte-for-byte. Builds on PR-96 (Daylight).
- **Chrome promoted:** floating tab bar → real Liquid Glass (`.maudeBarGlass`); soft scroll-edge dissolve on Today + Insights (`.maudeScrollEdgeSoft`).
...
- **fix(a11y) FB-AEkAWxal** — *"changed text size and it did not change; font is small."* Root cause: the `Theme.swift` font helpers built every face from a FIXED point size (`.system(size:)` and `.custom(_:size:)` **without** `relativeTo:`), so nothing scaled with the iOS text-size setting — and NFR-A11Y-01 had been marked "implemented" on the false belief that `Font.custom(_:size:)`/SF Pro `.system(size:)` auto-scale (they do not; the PR-96 Daylight type switch kept the same fixed-size pattern). Fixed in ONE place: SF Pro helpers (`lato`/`maudeH1/H2/Body/Caption`) now route through `UIFontMetrics(forTextStyle:).scaledFont(for:)`, and the IBM Plex Mono helpers (`maudeKicker`/`maudeMono`) gain `relativeTo:`. All ~566 call-sites scale from this single change; the default (Large) look is unchanged. The two intentional chrome clamps (tab bar `…xLarge`, Home chips `…xxLarge`) are kept.
...
- **feat(theme):** `Theme.swift` recoloured to the locked **six-colour Daylight palette** (Punch Red · Honeydew · Frosted Blue · Cerulean · Oxford Navy · Amber Flame). Retires fern-green/lime, brown-clay, near-black; dark mode = deep navy. Every `MaudeTheme.*` call site recolours automatically. No green hue in the UI (Cerulean carries consent/in-range).
...
- Stage F of `Maude_VideoConsult_Spec_v01`; the citizen-side flow (request → check-in + device test → calendar/reminders → waiting room → auto-launch) is now complete.
- **Verified:** build green.

## PR-93 — Video consult Stage C/D (iOS): calendar invite + reminder ladder (2026-06-15, FB-AOIWoD6l)
- **feat(consult):** the "Add to my calendar" EventKit event now carries the **app deep link** (`maude://consult`, in `url` + notes) and the spec **reminder ladder as calendar alarms** — day-before (-24h), -1h, and -10m — reused so calendar alarms and (future) push don't double-fire. Title clarified to "Maude video consultation".
- iOS side of Stage C/D. The server-generated `.ics` email (stable `UID`/`SEQUENCE`/`VTIMEZONE`) + push delivery remain backend follow-ups; a deep-link URL handler for `maude://consult/{id}` is a small follow-up.
- **Verified:** build green.
...
- Addresses the "confusing start on video" feedback (FB-AOUGIncO) and is Stage E of `Maude_VideoConsult_Spec_v01`.
...
- **fix(brand):** `DfGOnboardingView` Page 1 sits on `MaudeTheme.paper` (cream) but used the white/negative DfG mark → invisible. Locked logo rule is light bg → colour variant; swapped `dfg-logo-negative` → `dfg-logo`. Other negative-mark usages audited and left as-is (white-on-navy tiles / inverted token card are correct).
...
- **Guardrails held**: brand-locked terms untouched (Maude, DfG, Apple Health/Watch, Health Vault, Sundhed.dk, Sundhedsplatformen, MitID, MyHealth@EU, FMK, drug names, Yourcoach.health, Rigshospitalet); glucose stays mmol/L; GMI headline untouched; provenance still never rendered.
...
- **BRAND CHANGE, signed off explicitly:** palette navy #112744 (ink), paper #F8F5EC / card
  ...
  fallback assets. Mark: IRIS replaces aperture (app icon 1024, MaudeMark/Reversed imagesets —
  asset-embedded, never redrawn). lato()/maudeKicker helper names retained so 300+ call sites
...
- **feat(l10n):** `Maude/Localizable.xcstrings` (Apple String Catalog, en source) added;
...
- **fix(onboarding):** the DfG screen opens on the full navy canvas with the **inverted white
  ...
  wider tracking). Snapshot hook `MAUDE_SHOW_DFG_ONBOARDING=1`. Sim-verified. 114 tests ✅.
...
- **fix(consult):** Jitsi call is now titled **"Maude video call"** (no raw room id) and the
...
### PR-66 — DfG wallet login: real OID4VP rail + Maude Citizen credential check (2026-06-11)
- **feat(auth):** the first-screen DfG Wallet flow now (a) frames the present step as
  **presenting the Maude Citizen credential** from the My DfG wallet (sign-in IS the proof the
  credential is stored), (b) offers **"Get your Maude Citizen credential"** inline (real Partisia
  ...
  `MAUDE_OPEN_DFG=1`; `MAUDE_WALLET_STEP=verifying` now runs the live verification.
...
- **feat(UC-24b):** per-journal-entry **ePRO provenance receipt** (checkmark-seal action on entry
  ...
  content (ALCOA+). DEBUG `MAUDE_DEMO_EPRO=1`. Sim-verified vs local backend (real Partisia QR).
- 114 tests ✅.

### PR-64 — UC-A: Maude Citizen credential issuance into My DfG (2026-06-10)
- **feat(wallet):** "Your credential" row on Privacy → issues the citizen's own **Maude Citizen**
  ...
  `AppState.issueCitizenCredential`. DEBUG `MAUDE_DEMO_CITIZEN_CRED=1`.
...
- **feat(auth):** `DfGWalletLoginView` — in-app, high-fidelity simulation of the DfG Wallet identity
  ...
  Partisia" + CE ref → into Maude. DfG-branded navy authority ground; descriptive, no "simulated" labels.
...
- **feat(ingestion):** real on-device HealthKit is now the DEFAULT provider on a Health-capable
  ...
  Simulator / no-Health / `-uiTestAutoDemo` / `MAUDE_DATA=mock` fall back to demo. `usingRealData`
...
- **fix(ui):** `MessageThreadView` — explicit composer text colour (never follows the
  ...
  separation; selectable text. `DEBUG MAUDE_OPEN_THREAD` deep-link for verification.
...
### PR-54/55 — Maude Watch app (watchOS) — descriptive wellness glance (2026-06-10)
- **feat(watch):** `MaudeWatch/` scaffold — `WatchHomeView` (calm affirming line +
  wellness pillars Glucose/Sleep/Recovery/Heart, two-state moss/clay), `WatchModels`
  (descriptive snapshot), `WatchTheme` (locked Midnight palette), `MaudeWatchApp`.
...
- **feat(watch):** `MaudeComplication` (WidgetKit watchOS) — descriptive in-range
...
- NudgeDetailView gains a "Discuss in the assistant" secondary CTA; Insights recovery
  ...
  Maude's descriptive-only version of Oura's "Dive in with Advisor".
...
- **security:** API key in gitignored `Maude/Secrets.swift` (template committed); NOT in
  git. Key still ships in the binary → move behind an api.maude.app proxy before public
...
- **feat(chat):** `Maude/Chat/MaudeChat.swift` — wellness-scope assistant: fixed
...
- **test(chat):** `MaudeTests/ChatGuardTests` — 10 acceptance tests green (red-team
...
- **fix(ui) [beta feedback]:** read the 4 TestFlight `betaFeedbackScreenshotSubmissions`
  ...
  - **DfG Tokens (`TokenWalletView`):** the cost pills + In-app/Charity toggle used
    `MaudeTheme.ink` (a TEXT colour, light in Midnight) as a *background* with white
    text → white-on-cream, unreadable. Swapped to `invertBG`/`invertFG`. Balance card
    now shows the **DfG logo** (`dfg-logo`/`dfg-logo-negative`), not the Maude mark.
...
- **feat(console) [sandbox.maude.app]:** real GoTrue login replacing the rejected
...
- **infra:** self-hosted Jitsi white-labelled to "Maude" (watermark off, runtime patch).
...
- **feat(ui) [NFR-UI-*]:** built the remaining v2 screens from the locked design
  explorations (`_maude_design_reference/.../explorations/`), reusing the v2 token
...
- **feat(consult) [NFR-SEC-07 SATISFIED]:** live video for the Care/consult flow,
  on a **self-hosted EU Jitsi** (Scaleway fr-par; instance `maude-jitsi`, public
...
- **feat(consult):** `Config.videoConsultEnabled = true`; `Config.jitsiDomain` →
  ...
  in-app. Deterministic room `maude-consult-<id>` unchanged.
- **feat(console):** B2B console `VITE_JITSI_DOMAIN` repointed meet.jit.si → the
  sovereign host; rebuilt + redeployed to `sandbox.maude.app` (rclone sync + Edge
...
- **Hardening (open):** move behind `meet.maude.app` (A record), lock SG to
...
- **build(release):** `CURRENT_PROJECT_VERSION` 10.4 → **10.5**; archived + uploaded
  ...
  (PS258XSNL8), bundle `xyz.ppcn.maude`. `Config/ExportOptions.plist` team confirmed
...
- **Ships:** PR-46 full HealthKit capture + Connect→authorize wiring + PR-47 Paper/
  clay. Release config = `sovereignProd` (api.maude.app, health 200), `authEnabled
...
- **feat(theme):** added the Design System v2 signal tokens to `Maude/Theme.swift` —
  ...
  `confLearning`), the semantic type scale (`maudeH1/H2/Body/Caption`) and
  `MaudeTheme.Tracking`. Base palette already matched the design's `colors.css`
  (same `Maude_Design_Tokens_v01` source); Lato + IBM Plex Mono already bundled.
- **feat(theme) [NFR-UI-CLAY-01 / FR-REG-02]:** swept the patient view layer from
  `MaudeTheme.amber` → `MaudeTheme.clay` (`amber2`→`clay2`) across
  `Maude/Views/*.swift`. Rationale: amber reads as a warning light; the app is
...
- **feat(theme) [NFR-UI-THEME-01]:** default theme flipped Midnight → **Paper**
  (`MaudeApp.themeModeRaw`), the design ground. Midnight still selectable in
  Settings; dynamic tokens flip the whole app via the single root scheme.
- Source: `claude.ai/design` handoff bundle (`_maude_design_reference/`), chat
...
- Verified: `xcodebuild` (iOS Sim) BUILD SUCCEEDED; MaudeTests 78/78 green;
...
- **feat(config):** `Config.authEnabled` (default **false**). While false the auth
  screen hides the live Apple + email sign-in and shows a single **"Enter Maude"**
...
- **fix(ui) [fb3 "Check Color on buttons"]:** `NudgePrimaryButtonStyle` used
  `MaudeTheme.ink` as a surface + white text → cream-on-cream (invisible) on
...
- **fix(ui) sweep (same root cause, reachable secondary screens):**
  ...
  replaced with the locked `MaudeApertureMark` asset (theme-aware variant).
...
- **fix(brand):** `MaudeApertureMark` now auto-selects the asset variant from
...
- **feat(ui):** SettingsView **DISPLAY** zone — Theme segmented (Midnight/Paper →
  `maudeThemeMode`, drives root tokens), Reduce-motion toggle (`maudeReduceMotion`,
  read by `MaudeMotion`), Demo-data-chip toggle (`maudeShowDemoChip`). All three
  are real (no dead toggles); each writes `@AppStorage` consumed by live code.
- **fix(brand):** `maudeShowDemoChip` **defaults OFF** — the "Demo data" chip is a
...
- **fix(ui):** JournalView selected calendar-day pill, active filter chip, and FAB
  used `MaudeTheme.ink` as a *surface* + white text → invisible on Midnight.
...
- **feat(ui):** Wallet summary card now an **invert surface** (was `ink`+white →
  ...
  toast with shared `MaudeToastData` + `.maudeToast` (withdraw → rust dot).
...
- **feat(dev):** `MAUDE_TAB` env (DEBUG) to deep-link the initial tab for snapshots.
...
- **feat(ui):** `OuraComponents.swift` — `RingView` (gradient moss→amber arc +
  ...
  OR in-app `maudeReduceMotion`), `MiniRing` (vitals + delta), `GlucoseCurveView`
...
- **feat(theme):** `Theme.swift` v02 — every `MaudeTheme.*` colour is now a
  **dynamic token** (`Color.dyn(light,dark)`): light = Paper, dark = **Midnight**
  ...
  `.preferredColorScheme` flip (MaudeApp, `@AppStorage("maudeThemeMode")`,
...
- **feat(brand):** converted **all ~199** non-mono `.font(.system(size:…))` calls
  ...
  (headlines/body), matching maude.app. Monospaced/rounded faces preserved
...
### PR-32 — Brand alignment to maude.app (mark asset, Lato + IBM Plex Mono, launch) (2026-06-03)
- **feat(brand):** embed the **locked aperture mark** as an image asset
  (`MaudeMark`/`MaudeMarkReversed`, the same SVGs as the website) and rewrite
  `MaudeApertureMark` to render it — no more code-drawn mark (brand kit §1:
...
- Verified: builds for the iOS Simulator; fonts in the bundle, mark + launch color
  ...
  matches maude.app (paper, aperture mark, Lato wordmark).

### PR-31 — First TestFlight upload (2026-06-03)
- **release:** Maude **1.0 (1)** archived (Release/live-data), signed with a
  ...
  `xyz.ppcn.maude`, team PS258XSNL8.
- **fix(appstore):** added `NSHealthUpdateUsageDescription` — Apple rejects upload
  (90683) without the HealthKit *write* purpose string even though Maude is
  read-only.
- Found via the existing DfG App Store Connect record (Maude / `xyz.ppcn.maude`)
  + App ID `486FL49R3M` — so the earlier `app.maude.ios` plan was dropped; reused
  the existing identifier (no new App ID / app record).
- **State:** `api.maude.app` LIVE (TLS, `/me`→401 auth-gated). `auth.maude.app`
  (GoTrue) NOT yet deployed → sign-in won't work for testers until it's up
  (see `maude-backend/docs/Deploy_Prod_Hosts_Prompt_v01.md`). Hold external
  testers until auth is live.

### PR-30 — TestFlight prep: live-data Release config + App Store gaps (2026-06-03)
- **Decisions (locked):** DfG team `PS258XSNL8`; bundle `app.maude.ios`; canonical
  hosts on `maude.app`; video consult **gated off** in v1; external testers,
...
- **feat(config):** `Config.backend` now ships **live in Release/TestFlight** —
  Release → `sovereignProd` (`api.maude.app` + Supabase GoTrue `auth.maude.app`);
  Debug → `.mock`; `MAUDE_BACKEND` env still overrides. New `sovereignProd`
  preset; `sovereignStaging` repointed to `api.maude.app`.
...
- **chore(signing):** `Config/Signing.xcconfig` (local, gitignored) → team
  `PS258XSNL8`, bundle `app.maude.ios`, group `group.app.maude.ios`.
...
- Verified: `xcodebuild build` (iPhone 17 Pro sim) succeeds; privacy manifest in
  the .app; bundle id `app.maude.ios`.
- **Still blocked (needs your Apple login):** no DfG signing identity in this
  ...
  Apple Distribution cert); register App ID `app.maude.ios` + App Group + the
...
- **fix(brand):** the `AppIcon.appiconset` declared icon slots but contained **no
  ...
  (`Brand_Assets/maude_appicon_aperture_1024.png`), flattened onto the brand
...
- **feat(auth):** new `SupabaseAuthClient` (GoTrue) — `login` (password grant),
  ...
  with pure body builders + response parsers. `MaudeBackendService` now takes a
...
- **feat(config):** `Config.supabaseAuthURL` (self-hosted GoTrue) + `.sovereign(…,
  ...
  seed token (no Supabase needed locally). `MAUDE_BACKEND` env override unchanged.
...
- **feat(auth):** `AppleSignInCoordinator` runs the native `ASAuthorizationController`
  ...
  `AppState.signInWithApple` → `MaudeBackendService.signInWithApple` (the PR-22
...
- **chore(e2e):** local-QA hooks so the Care tab can be exercised against the
  ...
  read `MAUDE_BACKEND` / `MAUDE_JITSI_DEMO` env (defaults stay `.mock` / `nil`),
  ...
  is joinable out of the box (maude-backend `prisma/seed.ts`).
...
- **feat(services):** `MaudeBackendService.signInWithApple` (was a `notAvailable`
...
- Backend change lives in **maude-backend** (commit `test(consult): …`). The
  ...
  direction (maude-b2b-console `dd7090d`): button "Request recording" → sets
...
- Backend change lives in **maude-backend** (commit `feat(citizen): secure-
  ...
  `CareConnect`/`MaudeBackendService` already consume them (PR-18). Backend
...
- **feat(ui):** `ConsultView` joins the deterministic EU room
  (`https://<jitsiDomain>/maude-consult-<id>`) in a `WKWebView` (inline media);
...
- **docs:** `docs/Video_and_OAuth_Contract_v01.md` — cross-repo contract for the
  citizen side of the recipient workflow already in `maude-backend`/
  `maude-b2b-console` (Ory bearer model, deterministic Jitsi room
  `maude-consult-<id>`, recording-consent direction, citizen endpoint set).
- **feat(services):** `CareConnect` capability (sovereign-only, off the shared
  protocol) + `MaudeBackendService` conformance — `GET /notifications`,
...
- **feat(security):** `SessionTokenStore` (Keychain, device-only) persists the
  Ory session token (NFR-SEC-01 / FR-AUTH-03). `MaudeBackendService` restores it
...
- Verified `MaudeBackendService` against the actual `maude-backend` repo
...
- **feat(services):** `MaudeBackendService` takes an optional `OryAuthClient`.
...
- **feat(services):** `MaudeBackendService` implements `SupabaseServiceProtocol`
  against the EU-sovereign backend (NestJS/Scaleway+Ory) per `openapi.yaml` /
  `Maude_iOS_Backend_Contract_v01` — `GET /me`, `GET /recipients`, `GET /grants`,
...
- **docs(qms):** re-anchored RTM to actual SRS IDs — FR-SHARE-01 (DerivedShareBuilder),
  ...
  must be EU-sovereign — `maude-backend`/Scaleway+Ory, not US-parented Supabase),
  FR-PROV-01 (SourceArbiter). Root cause of the divergence: building from the
  derived RTM instead of `Maude_SRS_v05`/`Maude_UseCases_v05` and
...
- The PR-5 engine type `Nudge` collided with the locked prototype's
  presentation-layer `Nudge` card view-model (`Maude/Models/MockData.swift`),
...
- **test:** `MaudeTests/IngestionTests.swift` (T-ING-01..05),
  `MaudeTests/ProvenanceGuardTests.swift` (T-PROV-02/03).
...
- **feat(store):** on-device `SwiftData` container (`MaudeStore`) with
...
- **test:** `MaudeTests/DataModelTests.swift` — clinical/SIMULATED rejection,
...
- **chore:** relocate locked Phase-1 prototype to non-synced canonical clone
  `~/Developer/DataForGood/maude-ios` (baseline, no code change).
