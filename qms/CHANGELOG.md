...
## PR-101 — Travelling glass magnifier tab bar (Flighty-style) (2026-06-17, build 10.63, CN reference: Flighty)
- **feat(chrome) — one travelling Liquid Glass lens on the floating tab bar.** Replaces the flat moss selection pill with a SINGLE persistent glass lens that glides between tabs (CN's Flighty reference). iOS 26: real `.glassEffect` lens with a tasteful chromatic rim that flares mid-travel (`TravellingTabPill`, `LiquidGlass.swift`), merged into the bar capsule via `GlassEffectContainerCompat` — the fix for the prior "glass-on-glass is muddy" retreat (the merge needs a container; stacking two blurs doesn't). Positioned from measured tab frames (`TabFrameKey` preference) + `.offset` so ONE element travels — `.position` was tried first and ballooned the bar to full-screen (caught + fixed on the Simulator).
- **legibility decision:** the lens sits BEHIND the row (a frosted lens ON TOP hid the selected tab on the Simulator); the active glyph is magnified 1.14× so the selection still reads as under a magnifier. Calm/on-brand: no garish spectrum, moss + heroGlow rim through clear gaps.
- **design note (cross-fade avoided):** the iOS 26 `glassEffectID` + `.glassEffectTransition(.matchedGeometry)` morph CROSS-FADED between endpoints on a 6-slot bar (verified on the Simulator) instead of gliding; switched to the single-measured-pill strategy, which travels correctly.
- **three-tier ladder kept:** iOS 26 glass → iOS 17–25 `.ultraThinMaterial` pill (travels via the same `LiviqaBarMotion.travel` spring) → opaque moss pill under Reduce Transparency / Increase Contrast. Flag off / iOS < 26 = the prior bar (moss dot), byte-for-byte. Reduce Motion = no glide. All behind the existing `liquidGlass` flag.
- **Verified:** build green; pill places + stays legible at Home/Care/Settings, single-element travel confirmed via slow-motion burst, flag-off regression intact (moss dot, no pill). **Shipped TestFlight 10.63.** KNOWN: real glass refraction renders only on a physical iOS 26 device — the Simulator shows it frosted, so the on-device Flighty look needs device confirmation.
