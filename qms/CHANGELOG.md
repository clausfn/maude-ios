...
## PR-101 — Flighty drag-magnifier tab bar (2026-06-17, build 10.64, CN reference: Flighty)
- **feat(chrome) — interactive glass magnifier on the floating tab bar.** Not a selection pill that slides (that earlier same-day take was wrong, per CN). It is the Flighty interaction: a `DragGesture(minimumDistance: 0)` on the bar raises a LARGE glass bubble (`GlassMagnifierBubble` — a clear iOS 26 `.glassEffect` lens with a chromatic rim, `LiquidGlass.swift`) under the finger on touch; the bubble tracks the finger and shows a MAGNIFIED copy of the tab it is over (the in-row glyph dims beneath it); on lift it selects the tab under the finger and the bubble vanishes. Bubble Ø 64 (≫ a tab slot), pops up above the bar line. Finger hit-tested against measured tab frames (`TabFrameKey`).
- **rest state:** clean bar — selected tab marked with a small moss dot, no pill, no slide. The magnifier exists only while pressing.
- **accessibility:** selection works without the gesture — each cell is an `.isButton` accessibility element with `.accessibilityAction(.default)` → `select(item)` (VoiceOver double-tap).
- **three-tier ladder kept:** iOS 26 clear glass lens → iOS 17–25 `.ultraThinMaterial` bubble → opaque `paper2` bubble under Reduce Transparency / Increase Contrast. Flag off / iOS < 26 = the prior bar (tap-to-select, moss dot), no bubble. All behind the existing `liquidGlass` flag.
- **Verified on the iOS 26 Simulator (real gesture via computer-use):** touch raises the bubble; it tracks Home→Care→Privacy magnifying each; tap selects; drag-release selects; rest shows the dot with no bubble. Earlier intermediate takes caught + fixed on-device-of-record: `glassEffectID` morph cross-faded on a 6-slot bar; `.position` ballooned the bar full-screen; a frosted lens ON TOP hid the selected tab. **Shipped TestFlight 10.64.** KNOWN: the real glass refraction renders only on a physical iOS 26 device — the Simulator draws it frosted.
