...
| FR-ARCH-04 | Provider is read-only by contract (auth + fetch only, no write surface) | `Liviqa/Ingestion/HealthDataProvider.swift` | T-ING-05 | in-progress (HK empty write set in PR-4) | PR-3 |
...
|---|---|---|
| FR-ING-02..13 | Concrete `HealthKitService` read queries + LV001 export reader | PR-4 |
