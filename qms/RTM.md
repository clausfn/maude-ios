...
| UC-consult / video + messaging | Citizen care-team surface: notifications, joinable consults (Jitsi room), citizen-authoritative recording consent, secure messaging threads | `Liviqa/Services/CareConnect.swift`, `LiviqaBackendService` (CareConnect), `AppState.refreshCareInbox`; contract `docs/Video_and_OAuth_Contract_v01.md` | (service typechecks; UI + backend `/threads` routes pending) | in-progress | PR-18 |
| UC-consult (UI) | Care tab: messaging chat + consult join (EU Jitsi WebView) + citizen recording-consent toggle | `Liviqa/Views/MessagesView.swift`, `Liviqa/Views/ConsultView.swift`, `Liviqa/Views/MainTabView.swift` (Care tab) | SourceKit-clean; Xcode/simulator build pending | implemented (UI) | PR-19 |
